...
class VOT(object):
    ...
    def __init__(self, region_format, channels=None):
        ...
        self._trax = trax.Server(
            [region_format], [trax.Image.PATH], channels,
            metadata=dict(vot="python"))

        request = self._trax.wait()
        assert(request.type == trax.TraxStatus.INITIALIZE)
        if len(request.objects) != 1:
            raise RuntimeError('Single-object VOT wrapper received {} objects'.format(
                len(request.objects)))
    ...
    def frame(self):
        ...
        if request.type == trax.TraxStatus.FRAME:
