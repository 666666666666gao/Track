#!/usr/bin/env python3
"""Metric-blind CUDA parity check for the template-only transaction tracker.

The check runs the direct identity-only tracker and the transaction tracker on
the same real RGB-D frames.  It is deliberately restricted to a trajectory
prefix with no template candidate: every bbox, score, and writer decision must
then match.  Only the initialization ground-truth box is read.
"""
...
from lib.test.parameter.sutrack_transaction import (
    parameters as transaction_parameters,
)
...
def run_prefix(
        ...
        init_bbox, init_nlp, transaction_mode):
    ...
    frames = []
    for index in range(anchor_index + 1, anchor_index + frame_count + 1):
        ...
        if transaction_mode:
            ...
            if event_kind is not None:
                raise RuntimeError(
                    'Parity prefix opened transaction {} at frame {}'.format(
                        event_kind, index))
...
def compare_frames(baseline, candidate):
    ...
    for base, current in zip(baseline, candidate):
        ...
        if bbox_error > 1.0e-6 or score_error > 1.0e-7:
...
def main():
    ...
    parser.add_argument('--frames-after-init', type=int, default=35)
    ...
    baseline, baseline_devices, unused_trace = run_prefix(
        ...
        args.frames_after_init, init_bbox, init_nlp, False)
    del baseline_params
    gc.collect()
    torch.cuda.empty_cache()
    ...
    candidate_params = create_params(transaction_parameters, trace_root)
    candidate, candidate_devices, trace_path = run_prefix(
        SUTRACKProtectedTransaction, candidate_params, sequence_root,
        args.anchor_index, args.frames_after_init, init_bbox, init_nlp, True)
    max_bbox_error, max_score_error = compare_frames(baseline, candidate)

    payload = {
        'schema': 'sutrack_template_transaction_cuda_parity_v1',
        'status': 'passed',
