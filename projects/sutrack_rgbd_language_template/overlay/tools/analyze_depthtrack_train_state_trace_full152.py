...
def main():
    ...
    if (trace_plan.get('schema') !=
            'sutrack-depthtrack-train-state-trace-plan/v1' or
            trace_plan.get('complete') is not True or
            trace_plan.get('dataset') != 'DepthTrack Train only' or
            trace_plan.get('sequence_count') != 152 or
            trace_plan.get('ground_truth_available_to_tracker') is not False or
            trace_plan.get('future_frame_text_used') is not False or
            trace_plan.get('public_evaluation') is not False or
            trace_plan.get('analysis_sequence_order') != expected_sequences):
        raise ValueError('full152 trace plan contract failed')
    ...
    source_snapshot = {str(trace_plan_path): sha256_file(trace_plan_path)}
    ...
    if (trace_plan['config']['sha256'] !=
            common_contract['config']['sha256'] or
            trace_plan['checkpoint']['sha256'] !=
            common_contract['checkpoint']['sha256'] or
            trace_plan['language_manifest']['sha256'] !=
            common_contract['language_manifest']['sha256']):
        raise ValueError('full152 plan/source provenance mismatch')
    ...
    result = {
        ...
        'scope': 'DepthTrack Train full152 immediate-action labels',
        ...
        'trace_plan_sha256': source_snapshot[str(trace_plan_path)],
