...
class STTrack(BaseTracker):
    def __init__(self, params):
        super(STTrack, self).__init__(params)
        network = build_sttrack(params.cfg, training=False)

        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        depth = 12
        self.keep_rate = [x for x in torch.linspace(0.7, 1, depth//4)][::-1]
        # self.keep_rate = keep_rate.reverse()
        self.cfg = params.cfg
        ...
        # else:
        #     self.update_threshold = conf_thr
    ...
    def initialize(self, image, info: dict):
        ...
        # self.template_list_v = [template.tensors[:, :3, :, :]] * self.num_template
        # self.template_list_i = [template.tensors[:, 3:, :, :]] * self.num_template
        ...
        with torch.no_grad():
            ...
            if self.track_query_before != None:
                out_dict = self.network.forward(
                    ...
                    search=x_dict, ce_template_mask=self.box_mask_z, track_query_before=self.track_query_before,keep_rate=self.keep_rate, semantic_context=self.semantic_context )
            else:
                out_dict = self.network.forward(
                    template= self.z_dict ,
                    search=x_dict, ce_template_mask=self.box_mask_z, keep_rate=self.keep_rate, semantic_context=self.semantic_context )
