...
def main():
    ...
    if args.analyze:
        ...
        for item in receipts:
            ...
            values,metrics=independent_overlap(boxes,gt);per[name]=metrics
            base=sorted(baseline[name],key=lambda x:x['frame_index']);assert [r['frame_index'] for r in base]==list(range(len(rows)))
            baseboxes=np.asarray([r['public_bbox'] for r in base]);basevalues,basemetrics=independent_overlap(baseboxes,gt)
