"""Evaluate one frozen scale-triggered template policy on Train development."""
import argparse
import csv
import json
import math
...
def main():
    ...
    if args.analyze:
        ...
        for item in receipt['sequences']:
            ...
            per[name] = metrics
            baseline = np.asarray([r['bbox'] for r in case['expected_rows']])
            ...
            assert all(r['score'] == case['expected_rows'][r['frame']]['score'] for r in rows[1:prefix_end])
    ...
    tracker = STTrackScaleTemplate(params,plan['scale_change'])
    folder = root/'recursive'; folder.mkdir()
    ...
    for name in plan['sequences']:
        ...
        rows=[dict(frame=0,bbox=case['init_bbox'],score=1.,template_update=None,template_scale_ratio=1.,
                   template_reference_scale=math.sqrt(case['init_bbox'][2]*case['init_bbox'][3]))]
        for frame in range(1,case['frames']):
            ...
            rows.append(dict(frame=frame,bbox=out['target_bbox'],score=float(out['best_score']),
                             **{k:out[k] for k in ['template_update','template_scale_ratio','template_reference_scale']}))
        path=folder/(name+'.json'); path.write_text(json.dumps(dict(sequence=name,rows=rows),allow_nan=False)+'\n')
        item=dict(sequence=name,frames=len(rows),native_updates=sum(r['template_update']=='native' for r in rows),
                  scale_updates=sum(r['template_update']=='scale' for r in rows),sha256=sha(path),elapsed_seconds=time.time()-started)
    ...
    (root/'recursive_receipt.json').write_text(json.dumps(dict(status='complete',sequences=receipts,
        spec_sha256=sha(root/'spec.json'),checkpoint_sha256=spec['checkpoint_sha256'],ground_truth_files_opened=False),indent=2)+'\n')
