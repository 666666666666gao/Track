#!/usr/bin/env python3
"""M29 utility-conditioned candidate-only versus paired temporal harm veto."""
...
SPEC_SCHEMA = "sttrack-lachtt-m29-utility-conditioned-temporal-harm-spec/v1"
PREFLIGHT_SCHEMA = "sttrack-lachtt-m29-preflight-binding/v1"
PREAUDIT_SCHEMA = "sttrack-lachtt-m29-preexecution-audit/v1"
FOLDS = (2, 3, 4, 5)
CONDITIONS = ("candidate_temporal", "paired_temporal")
...
EXPECTED_CLAIM_CEILING = {
    "scope": "DepthTrack Train folds 2-5 post-M28 action-conditioned safety diagnostic only",
    "permitted": "one fixed 3072-step M29 CAND-TEMP-versus-PAIR-TEMP result",
    "forbidden": [
...
def load_components():
    ...
    return (torch, m22, m23, m25, UtilityConditionedTemporalHarmHead,
...
def validate_training_contract(spec):
    ...
    if spec.get("training") != expected:
        raise ContractError("M29 training constants drifted")
...
def validate_spec(spec, args, spec_record, runner_record, m23):
    if (spec.get("schema") != SPEC_SCHEMA or
        ...
        raise ContractError("M29 spec identity drifted")
    if (args.output.resolve() != Path(spec["output"]["root"]).resolve() or
            args.output.exists()):
        raise ContractError("M29 output precondition drifted")
    ...
    expected_architecture = {
        ...
        "head": "linear_58_to_32_tanh_gru_32_linear_32_to_3_tanh",
    ...
    if spec.get("architecture") != expected_architecture:
        raise ContractError("M29 architecture constants drifted")
    expected_policy = {
    ...
    if spec.get("oof_policy") != expected_policy:
        raise ContractError("M29 policy drifted")
    expected_gates = {
    ...
    if spec.get("scientific_gates") != expected_gates:
        raise ContractError("M29 scientific gates drifted")
    if spec.get("claim_ceiling") != EXPECTED_CLAIM_CEILING:
        raise ContractError("M29 claim ceiling drifted")
    ...
    if spec.get("forbidden_runtime_roots") != expected_forbidden_roots:
        raise ContractError("M29 forbidden runtime roots drifted")
...
def validate_preflight(path, spec_record, runner_record, spec, m23):
    ...
    if (binding.get("schema") != PREFLIGHT_SCHEMA or
        ...
        raise ContractError("M29 preflight binding drifted")
    return record
...
def validate_preaudit(path, spec_record, runner_record, preflight_record,
    ...
    if (audit.get("schema") != PREAUDIT_SCHEMA or
            ...
            audit.get("authorization", {}).get("m29_execution") is not True or
            audit.get("audited_identity") != expected or
            audit.get("claim_ceiling") != spec["claim_ceiling"]):
        raise ContractError("M29 preaudit did not authorize exact execution")
    return record
...
def target_tensors(torch, m23, rows, valid):
    ...
    if (tuple(target["harm"].shape) != (6, 3) or
        ...
        raise ContractError("M29 harm target drifted")
    return target
    ...
    for key in sorted(m27_index):
        ...
        for condition in CONDITIONS:
            ...
            if (tuple(identity.shape) != (5, 6, 9) or
                ...
                raise ContractError("M29 temporal identity relation drifted")
...
def make_batch(torch, keys, actions, relations, identities, targets,
    ...
    for key in keys:
        ...
        fields["harm"].append(targets[key]["harm"][role])
...
def forward_loss(torch, model, batch):
    ...
    if not math.isfinite(float(loss.detach())):
        raise ContractError("M29 loss is non-finite")
    return outputs, loss
...
def condition_predictions(torch, model, eval_fold, keys, actions, offset,
    ...
    for key in keys:
        ...
        rows.append({
            "record_type": "m29_utility_conditioned_temporal_harm_prediction",
            "condition": condition,
            ...
            "predicted_top_action_harm_h3_h5_h10": [
...
def permutation_audit(torch, m23, model, keys, actions, relations, identities,
    ...
    for key in keys:
        ...
        restored = model_outputs(
            torch, model, permuted)["predicted_harm"][0]
...
def publish(output, result, trace, predictions, m23):
    ...
    try:
        ...
        manifest = {
            "schema": "sttrack-lachtt-m29-output-manifest/v1",
            "complete": True,
        ...
        if tuple(sorted(path.name for path in temporary.iterdir())) != tuple(
                sorted(OUTPUT_FILES)):
            raise ContractError("M29 output set drifted")
...
def load_development(torch, m22, m23, m22_spec_record, m22_binding_record,
    ...
    if (len(training_groups) != 507 or
        ...
        raise ContractError("M29 target closure drifted")
    ...
    for key, rows in training_groups.items():
        ...
        if split_entries[key]["partition"] != "training" or fold not in FOLDS:
            raise ContractError("M29 training fold drifted")
    ...
    if any(fold_sequences[left] & fold_sequences[right]
        ...
        raise ContractError("M29 sequence folds overlap")
    ...
    if sum(unique_counts.values()) != 2106:
        raise ContractError("M29 unique hypothesis census drifted")
...
def smoke_main(args):
    ...
    evidence_diff = float((
        identities["candidate_temporal"][key] -
        identities["paired_temporal"][key]
    ).abs().max())
    result = {
        "schema": "sttrack-lachtt-m29-no-optimizer-smoke/v1",
        "complete": True,
    ...
    if (not result["identical_initial_state"] or evidence_diff <= 0 or
        ...
        raise ContractError("M29 smoke gate failed")
...
def formal_main(args):
    ...
    if (git_output("rev-parse", "HEAD") != repository["commit"] or
        ...
        raise ContractError("repository identity drifted before M29")
    FORBIDDEN_RUNTIME_ROOTS[:] = [
    ...
    for condition in CONDITIONS:
        for fold in FOLDS:
            ...
            model = Head()
            if parameter_count(model) != 8323:
                raise ContractError("M29 parameter count drifted")
    ...
    if len(set(initial_states.values())) != 1:
        raise ContractError("M29 initial states differ")
            ...
            for epoch in range(spec["training"]["epochs_per_member"]):
                ...
                for epoch_step, keys in enumerate(batches):
                    ...
                    if post_nonfinite or postclip > 5.000001:
                        raise ContractError("M29 postclip gradient gate failed")
                    optimizer.step()
                    trace.append({
    ...
    if total_steps != 3072 or len(trace) != 3072:
        raise ContractError("M29 optimizer step count drifted")
    ...
    if (not condition_pass["candidate_temporal"] and
            condition_pass["paired_temporal"]):
        interpretation = "paired_temporal_supported_candidate_temporal_failed"
    elif (condition_pass["candidate_temporal"] and
          condition_pass["paired_temporal"]):
        interpretation = "both_pass_paired_reference_necessity_unsupported"
    elif (condition_pass["candidate_temporal"] and
          not condition_pass["paired_temporal"]):
        interpretation = "candidate_temporal_pass_paired_temporal_failed"
    else:
    ...
    engineering_conditions = {
        ...
        "canonical_role_event_replay_exact": permutation_exact,
    ...
    accepted = engineering_pass and condition_pass["paired_temporal"]
    result = {
        "schema": "sttrack-lachtt-m29-result/v1",
        "complete": True,
        "accepted": accepted,
        ...
        "decision": (
            "m29_pair_temporal_pass_authorize_separate_integration_plan_only"
            if accepted else "m29_stop_according_to_frozen_interpretation"),
    ...
    if (m23.file_record(args.spec) != spec_record or
            ...
            git_output("status", "--porcelain")):
        raise ContractError("M29 source changed during execution")
