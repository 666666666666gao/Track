#!/usr/bin/env python3
"""One bound step for learned RoI association with a safe gradient path."""
...
def validate_binding(args, spec):
    ...
    expected = {
        "schema": "sttrack-lachtt-m14a-learned-bounded-roi-smoke-binding/v1",
    ...
    if (plan_audit.get("path") !=
            ...
            "EXPERIMENT_AUDIT_M14A_PREEXECUTION_PLAN_20260901.json" or
            plan_audit.get("sha256") !=
            "d3897a8668ca156049a30cf797fdbb9905b1cdaf6cdd44caaf8c9a276a0c78f9" or
    ...
    if (authorizations.get("pre_execution_plan_audit_passed") is not True or
            authorizations.get("one_engineering_optimizer_step") is not True or
            authorizations.get("independent_result_audit_after_run") is not True or
            authorizations.get("m14b_capacity_plan_if_pass") is not True):
        raise ValueError("binding does not authorize M14a")
    for name in (
            "m14b_capacity_execution", "sequence_disjoint_pilot",
...
def source_records(spec, source_spec, native_rows, binding):
    ...
    for name in ("m10_scalar_relation", "m13_fixed_relation_source"):
        item = spec[name]
        records.append((name, REPOSITORY_ROOT / item["path"],
                        item["sha256"], None))
    item = binding["pre_execution_plan_audit"]
    records.append(("pre_execution_plan_audit", Path(item["path"]),
                    item["sha256"], None))
...
def main():
    ...
    if (authorization.get(
            ...
            authorization.get(
                "one_engineering_optimizer_step_after_plan_audit_pass") is not True or
            authorization.get("independent_result_audit_after_run") is not True or
            authorization.get("m14b_capacity_plan_if_pass") is not True):
        raise ValueError("spec does not authorize the bounded M14a workflow")
    for name in (
            "m14b_capacity_execution", "sequence_disjoint_pilot",
    ...
    predecessor_audit = json_file(
        spec["m13b_negative_boundary"]["result_audit"]["path"])
    if (predecessor.get("accepted") is not False or
            predecessor.get("decision") != "m13b_fail_stop_without_rescan" or
            str(predecessor_audit.get(
                "integrity_verdict", "")).lower() != "pass"):
        raise ValueError("M13b failure boundary drifted")
    ...
    if (str(plan_audit.get("protocol_gate", "")).lower() != "pass" or
            ...
            plan_audit.get("m14a_executed") is not False):
        raise ValueError("M14a pre-execution audit boundary drifted")
    ...
    difference_nonfinite = int(
        not torch.isfinite(differences).all().item())
    ...
    expected_relation_contract = (
        tuple(raw_spec["families"]) == tuple(FAMILY_NAMES) and
        tuple(raw_spec["family_block_counts"]) == FAMILY_BLOCK_COUNTS and
        tuple(raw_spec["family_index_per_block"]) == BLOCK_FAMILY_INDICES and
        int(spec["learned_projector"]["input_dim"]) == EMBEDDING_WIDTH and
        int(spec["learned_projector"]["output_dim"]) == 8 and
        int(model_spec["relation_dim"]) == 177)
    ...
    conditions = {
        ...
        "projector_nonzero_gradient_tensors_min":
            projector_nonzero_gradient_tensors >= int(
                gates["projector_nonzero_gradient_tensors_min"]),
        "preclip_gradient_norm": preclip_safe,
        "postclip_gradient_norm": postclip_safe,
        "optimizer_step_executed": optimizer_step_executed,
        "projector_changed_tensors_min": changed_projector_tensors >= int(
            gates["projector_changed_tensors_min"]),
        "changed_trainable_tensors_min": changed_tensors >= int(
            gates["changed_trainable_tensors_min"]),
    }
    accepted = all(conditions.values())
    result = {
        "schema": "sttrack-lachtt-m14a-learned-bounded-roi-smoke-result/v1",
        "complete": True,
        "accepted": accepted,
        "decision": ("m14a_pass_freeze_capacity_plan_only"
                     if accepted else "m14a_fail_stop_without_rescan"),
        ...
        "relations": {
            "raw_difference_shape": list(differences.shape),
            "raw_difference_maximum_absolute_value": difference_max_abs,
            "raw_difference_nonfinite": difference_nonfinite,
            "scalar_shape": list(scalar.shape),
            "scalar_maximum_absolute_value": scalar_max_abs,
            "scalar_nonfinite": scalar_nonfinite,
        ...
        "model": {
            ...
            "projector_nonzero_gradient_tensors":
                projector_nonzero_gradient_tensors,
        ...
        "authorization": {
            ...
            "m14b_capacity_plan": accepted,
            "m14b_capacity_execution": False,
    ...
    manifest = {
        "schema": "sttrack-lachtt-m14a-learned-bounded-roi-smoke-manifest/v1",
        "complete": True,
        "accepted": accepted,
