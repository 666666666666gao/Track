"""Evaluate fixed continuity admission with the existing M45 trained head."""
import argparse
...
def main():
    ...
    assert sha(control_root/'spec.json')==plan['control_spec_sha256']
    assert sha(control_root/'geometry_result.json')==plan['control_training_result_sha256']
    training=json.loads((control_root/'geometry_result.json').read_text());assert training['status']=='complete'
    assert training['m45_spec_sha256']==sha(control_root/'spec.json') and training['matched_control_initialization_and_sample_order']
    checkpoint=control_root/'geometry_final.pth';assert sha(checkpoint)==training['checkpoint_sha256']==plan['checkpoint_sha256']
    ...
    if args.analyze:
        ...
        control=json.loads((parent/'recursive_result.json').read_text());assert sha(parent/'recursive_result.json')==plan['source_recursive_result_sha256']
        control_root=Path(plan['control_root']);assert sha(control_root/'recursive_result.json')==plan['control_recursive_result_sha256']
        paired=json.loads((control_root/'recursive_result.json').read_text());assert paired['integrity_pass']
        receipts=[]
        for shard in [0,1]:
        ...
        for item in receipts:
            ...
            for i,r in enumerate(rows):
                assert r['choice']==(0 if r['vetoed'] else r['proposal'])
                if i>=2:
                    assert r['previous_choice_input']==rows[i-1]['choice']
                    assert len(r['continuity_delta'])==2 and all(math.isfinite(x) for x in r['continuity_delta'])
                    assert r['vetoed']==(r['proposal']!=0 and any(x<0 for x in r['continuity_delta']))
                else:assert r['proposal']==0 and not r['vetoed'] and r['continuity_delta'] is None
            assert sum(r['proposal']!=0 for r in rows)==item['proposals'] and sum(r['vetoed'] for r in rows)==item['vetoed']
            ...
            table.append(dict(sequence=name,**metrics,changes=len(changed),proposals=item['proposals'],vetoed=item['vetoed'],mean_iou_gain=metrics['mean_iou']-basemetrics['mean_iou'],
        ...
        saved=torch.load(checkpoint,map_location='cpu');assert saved['variant']=='geometry' and saved['m45_spec_sha256']==sha(control_root/'spec.json')
        assert saved['optimizer_steps']==960 and saved['base_checkpoint_sha256']==spec['checkpoint_sha256']
        result=dict(status='complete',integrity_pass=True,primary='native_continuity_admission',primary_pass=all(gates.values()),gates=gates,
            aggregates=dict(default=base,m45=paired['aggregates']['m45'],m48=totals),per_sequence=per,
            positive_sequences=positive,new_failure_sequences=broken,first_overrides=details,frames=33130,
            proposals=sum(r['proposals'] for r in receipts),vetoed=sum(r['vetoed'] for r in receipts),admitted_changes=sum(r['changes'] for r in receipts),
            ...
            analysis_source_sha256=sha(__file__),training_result_sha256=sha(control_root/'geometry_result.json'),control_result_sha256=sha(control_root/'recursive_result.json'),
            admission_effect_mean_gain=totals['mean_iou']-paired['aggregates']['m45']['mean_iou'],
            next='freeze low22' if all(gates.values()) else 'stop this fixed admission policy; no public evaluation',
            scope='Existing repeatedly used Train development sequences. Reuse exactly the M45 DepthTrack-trained head without new training or parameters; only admission requires nondecreasing native continuity in both streams. No margin sweep or public metric.')
    ...
    tracker=STTrackCandidateContinuity(params,checkpoint);outdir=root/'recursive';outdir.mkdir(exist_ok=True);receipts=[];started=time.time()
    for name in plan['shards'][args.shard]:
        ...
        tracker.initialize(image_at(0),dict(init_bbox=case['init_bbox']));rows=[dict(frame=0,bbox=case['init_bbox'],score=1.,choice=0,none=False,proposal=0,vetoed=False,continuity_delta=None,previous_choice_input=0)]
        for frame in range(1,case['frames']):
            out=tracker.track(image_at(frame));rows.append(dict(frame=frame,bbox=out['target_bbox'],score=float(out['best_score']),choice=out['association_candidate'],none=out['association_none'],proposal=out['association_proposal'],vetoed=out['association_vetoed'],continuity_delta=out['continuity_delta'],previous_choice_input=out['previous_choice_input']))
        path=outdir/(name+'.json');path.write_text(json.dumps(dict(sequence=name,rows=rows))+'\n')
        item=dict(sequence=name,frames=len(rows),changes=sum(r['choice']!=0 for r in rows),proposals=sum(r['proposal']!=0 for r in rows),vetoed=sum(r['vetoed'] for r in rows),sha256=sha(path),elapsed_seconds=time.time()-started);receipts.append(item);print(json.dumps(item),flush=True)
    check_binding(parent,spec)
    for name,digest in plan['source_sha256'].items():assert sha(repo/name)==digest
