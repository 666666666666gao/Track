"""Replay the first choice in the two largest M44 geometry regressions."""
import argparse
import json
...
def main():
    ...
    audit=json.loads((root/'terminal_audit.json').read_text());assert audit['integrity_pass'] and not audit['primary_pass']
    result=json.loads((root/'recursive_result.json').read_text())
    harms=sorted(result['per_sequence']['geometry'],key=lambda n:result['per_sequence']['geometry'][n]['iou_sum']-result['per_sequence']['default'][n]['iou_sum'])[:2]
    assert harms==['egg_indoor','glass03_indoor']
    plans={x['sequence']:x for x in json.loads((root/'inference_inputs.json').read_text())}
    ...
    for name in harms:
        event=[r for r in audit['first_overrides'] if r['arm']=='geometry' and r['sequence']==name][0]
        stop=event['first_override_frame_zero_based'];case=plans[name];folder=Path(spec['dataset_root'])/name
        sealed=json.loads((root/'recursive'/('geometry_'+name+'.json')).read_text())['rows']
    ...
    check_binding(root,spec)
    output=dict(status='complete',frames=frames,rows=rows,source_sha256=sha(__file__),terminal_audit_sha256=sha(root/'terminal_audit.json'),
