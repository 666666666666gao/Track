#!/usr/bin/env python3
"""Build the frozen M18-0 causal-survival target and split closure.
...
PARTITIONS = ("training", "heldout", "quarantine")
...
def maximum_low_run(values, threshold):
    maximum = 0
    current = 0
    for value in values:
        if value <= threshold:
...
def trajectory_metrics(
        branch_ious, public_ious, horizon, threshold, risk_denominator):
    ...
    result = {
        ...
        "max_consecutive_low_overlap_run_fraction": (
            maximum_low_run(branch, threshold) / float(risk_denominator)),
    }
...
def validate_binding(args, spec, binding):
    if binding.get("schema") != (
            "sttrack-lachtt-m18-0-causal-survival-target-closure-binding/v1"):
    ...
    if binding.get("authorization", {}).get(
            "m18_0_readonly_target_closure") is not True:
        raise ContractError("binding does not authorize M18-0 closure")
    ...
    exact_authorization = (
        "run exactly one M18-0 read-only target closure if the "
        "binding passes its own preflight")
    audited_identity = audit.get("audited_identity", {})
    expected_identity = {
    ...
    if (audit.get("overall_verdict") != "PASS" or
            ...
            any(audited_identity.get(name) != value
    ...
    if (repo_path != spec_repo_path or
            runner_relative != Path(
                "tools/run_sttrack_lachtt_m18_0_causal_survival_target_closure.py")):
...
def build_closure(spec):
    expected = spec["frozen_inputs"]["global_counts"]
    target_contract = spec["target_contract"]
    tolerance = float(target_contract["h10_recalculation_tolerance"])
    threshold = float(target_contract["low_overlap_threshold"])
    risk_denominator = int(target_contract["risk_denominator"])
    if (tuple(int(value) for value in target_contract["horizons"]) !=
            HORIZONS or tuple(target_contract["metrics"]) != METRICS or
            not finite_number(tolerance) or tolerance < 0.0 or
            not finite_number(threshold) or not 0.0 <= threshold <= 1.0 or
            risk_denominator != 10):
        raise ContractError("target contract constants drifted")
    ...
    m17_result = load_json(spec["frozen_inputs"]["m17_0_result"]["path"])
    m17_split = load_json(
        spec["frozen_inputs"]["m17_0_split_ledger"]["path"])

    if (m17_result.get("accepted") is not True or
            m17_result.get("complete") is not True):
        raise ContractError("accepted M17-0 source closure is required")
    ...
    m17_split_rows = m17_split.get("events", [])
    m17_split_by_event = {event_key(row): row for row in m17_split_rows}
    duplicate_m17_split_events = (
        len(m17_split_rows) - len(m17_split_by_event))
    ...
    risk_monotonicity_mismatches = 0
    m17_split_parity_mismatches = 0
    partial_availability_events = 0
    folds = int(spec["sequence_split"]["fold_count"])
    salt = spec["sequence_split"]["salt"]
    heldout_fold = int(spec["sequence_split"]["heldout_fold"])
    quarantine_fold = int(spec["sequence_split"]["quarantine_fold"])
    training_folds = set(int(value) for value in
                         spec["sequence_split"]["training_folds"])
    ...
    for closure in sorted(closure_rows, key=event_key):
        ...
        if fold == quarantine_fold:
            partition = "quarantine"
        elif fold == heldout_fold:
            partition = "heldout"
        elif fold in training_folds:
            partition = "training"
        else:
            raise ContractError("unassigned fold")
        labels = []
        available_flags = []
        event_targets = []
        for role_id, action_stub in enumerate(actions):
            ...
            utility_fields_finite = (
                isinstance(utility, dict) and
                all(finite_number(utility.get(field)) for field in (
                    "branch_mean_iou", "public_mean_iou", "mean_iou_gain")))
            if (not utility_fields_finite or
            ...
            if not all(finite_number(value) for horizon in horizon_targets.values()
                ...
                nonfinite_targets += 1
            risks = [horizon_targets[str(horizon)][
                "max_consecutive_low_overlap_run_fraction"]
                for horizon in HORIZONS]
            if (not (0.0 <= risks[0] <= risks[1] <= risks[2] <= 1.0) or
                    any(abs(value * risk_denominator - round(
                        value * risk_denominator)) > tolerance
                        for value in risks)):
                risk_monotonicity_mismatches += 1
        ...
        m17_row = m17_split_by_event.get(key)
        if (m17_row is None or int(m17_row.get("fold", -1)) != fold or
                bool(m17_row.get("strict_h10_available")) != available_event):
            m17_split_parity_mismatches += 1
        if available_event:
    ...
    overlap = sorted(
        (set(split_sequences["training"]) & set(split_sequences["heldout"])) |
        (set(split_sequences["training"]) & set(split_sequences["quarantine"])) |
        (set(split_sequences["heldout"]) & set(split_sequences["quarantine"])))
    internal_summary = {
        ...
        "committed_quarantine_target_rows": len(targets["quarantine"]),
    }
    target_commitments = {
        name: {
    ...
    partition_spec = split_spec["partitions"]
    conditions = {
        ...
        "sequence_count_exact": (
            internal_summary["sequences"] == int(expected["sequences"])),
        ...
        "risk_monotonicity_mismatches_zero": (
            risk_monotonicity_mismatches == 0),
        "m17_split_rows_exact": (
            len(m17_split_rows) == len(closure_rows) and
            duplicate_m17_split_events == 0 and
            len(set(m17_split_by_event) - set(closure_by_event)) == 0),
        "m17_split_fold_parity_mismatches_zero": (
            m17_split_parity_mismatches == 0),
        "target_action_rows_exact": (
            internal_summary["target_action_rows"] ==
            int(expected["available_actions"])),
        "sequence_overlap_zero": not overlap,
    }
    for partition in PARTITIONS:
        expected_partition = partition_spec[partition]
        conditions[partition + "_split_exact"] = (
            sum(partition_event_counts[partition].values()) ==
            int(expected_partition["available_events"]) and
            sum(partition_action_counts[partition].values()) ==
            int(expected_partition["available_actions"]) and
            len(available_sequences[partition]) ==
            int(expected_partition["available_sequences"]) and
            len(split_sequences[partition]) ==
            int(expected_partition["all_event_sequences"]))
        conditions[partition + "_event_classes_exact"] = jsonable_equal(
            dict(partition_event_counts[partition]),
            expected_partition["event_classes"])
        conditions[partition + "_action_labels_exact"] = jsonable_equal(
            dict(partition_action_counts[partition]),
            expected_partition["action_labels"])
    diagnostics = {
        "duplicate_events": duplicate_events,
        ...
        "risk_monotonicity_mismatches": risk_monotonicity_mismatches,
        "m17_split_parity_mismatches": m17_split_parity_mismatches,
        "duplicate_m17_split_events": duplicate_m17_split_events,
        "ledger_audit": ledger_audit,
        "sequence_overlap": overlap,
    }
    summary = {
        "source_events": internal_summary["events"],
        "source_actions": internal_summary["actions"],
        "source_sequences": internal_summary["sequences"],
        "partition_all_sequence_counts":
            internal_summary["partition_all_sequence_counts"],
        "training": {
            "available_events": sum(
                partition_event_counts["training"].values()),
            "available_actions": len(targets["training"]),
            "available_sequences": len(available_sequences["training"]),
            "event_classes": dict(sorted(
                partition_event_counts["training"].items())),
            "action_labels": dict(sorted(
                partition_action_counts["training"].items())),
            "numeric_target_rows": len(targets["training"]),
        },
        "heldout": {
            "committed_action_rows": target_commitments[
                "heldout"]["action_rows"],
            "numeric_target_rows": 0,
        },
        "quarantine": {
            "committed_action_rows": target_commitments[
                "quarantine"]["action_rows"],
            "numeric_target_rows": 0,
        },
    }
    persisted_split_entries = []
    for row in split_entries:
        ...
        if row["partition"] == "training":
            persisted["strict_h10_available"] = row[
                "strict_h10_available"]
        persisted_split_entries.append(persisted)
    split_ledger = {
        "schema": "sttrack-lachtt-m18-0-sequence-split-ledger/v1",
        "fold_function": spec["sequence_split"]["fold_function"],
        "salt": salt,
        "fold_count": folds,
        "training_folds": sorted(training_folds),
        "heldout_fold": heldout_fold,
        "quarantine_fold": quarantine_fold,
        "target_storage_contract": (
            "trajectory_targets serializes training numeric targets only; "
            "heldout and quarantine numeric targets are omitted and each is "
            "represented solely by a canonical SHA256 commitment. A future "
            "authorized evaluator may rederive heldout targets only after "
            "optimization is complete. Fold0 remains permanently quarantined"),
        "target_commitments": target_commitments,
        "summary": summary,
        "all_sequences": split_sequences,
        "training_available_sequences": available_sequences["training"],
        "events": persisted_split_entries,
    }
    ...
    with Path(path).open("wb") as raw:
        with gzip.GzipFile(filename="", mode="wb", fileobj=raw, mtime=0) as zipped:
            with io.TextIOWrapper(zipped, encoding="utf-8", newline="\n") as stream:
                ...
                stream.write(json.dumps({
                    "record_type": "quarantine_target_commitment",
                    "partition": "quarantine",
                    "action_rows": target_commitments[
                        "quarantine"]["action_rows"],
                    "canonical_jsonl_sha256": target_commitments[
                        "quarantine"]["canonical_jsonl_sha256"],
                    "numeric_targets_serialized": False,
                }, sort_keys=True, allow_nan=False) + "\n")
        raw.flush()
        os.fsync(raw.fileno())
...
def inspect_serialized_targets(path):
    counts = Counter()
    with gzip.open(path, "rt", encoding="utf-8") as stream:
        for line in stream:
            row = json.loads(line)
    ...
    return {name: counts[name] for name in PARTITIONS}
...
def main():
    ...
    if spec.get("schema") != (
            "sttrack-lachtt-m18-0-causal-survival-target-closure-spec/v1"):
        raise ContractError("M18-0 spec schema drifted")
    if spec.get("complete") is not True:
        raise ContractError("M18-0 spec is incomplete")
    ...
    conditions["source_records_checked_exact"] = len(checked_sources) == 14
    ...
    try:
        ...
        serialized_counts = inspect_serialized_targets(target_path)
        conditions["training_numeric_targets_serialized_exact"] = (
            serialized_counts["training"] ==
            int(spec["sequence_split"]["partitions"]["training"][
                "available_actions"]))
        conditions["heldout_numeric_targets_not_serialized"] = (
            serialized_counts["heldout"] == 0)
        conditions["quarantine_numeric_targets_not_serialized"] = (
            serialized_counts["quarantine"] == 0)
        accepted = all(conditions.values())
        result = {
            "schema": (
                "sttrack-lachtt-m18-0-causal-survival-target-closure-"
                "result/v1"),
            "complete": True,
            "accepted": accepted,
            "evaluation_type": (
                "real_gt_train_only_readonly_causal_survival_target_closure"),
            "claim_ceiling": (
                "Sealed DepthTrack Train target closure only; no model, "
                "training, checkpoint, Test, CDTB, VOT, Qwen or benchmark "
                "claim."),
            "conditions": conditions,
            "summary": summary,
            "diagnostics": diagnostics,
            "serialized_numeric_target_rows": serialized_counts,
            "source_records_checked": len(checked_sources),
            "decision": (
                "m18_0_pass_authorize_independent_result_audit_only"
                if accepted else "m18_0_fail_stop_no_model"),
            "authorization": {
                "independent_m18_0_result_audit": accepted,
                "m18a_implementation": False,
                "m18a_execution": False,
                "m18b_training": False,
        ...
        manifest = {
            ...
            "inputs": {
                "plan": file_record(
                    spec["frozen_controls"]["experiment_plan"]["path"]),
        ...
        expected_files = set(spec["outputs"]["files"])
