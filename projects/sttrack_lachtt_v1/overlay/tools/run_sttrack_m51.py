"""Run the frozen M51 weight in two sequence shards, then audit both outputs."""
import argparse
...
def main():
    ...
    assert training['m51_spec_sha256']==sha(root/'spec.json') and training['matched_control_initialization_and_sample_order']
    ...
    if args.analyze:
        ...
        paired_path=Path(plan['paired_root'])/'recursive_result.json'
        assert sha(paired_path)==plan['paired_recursive_result_sha256'];paired=json.loads(paired_path.read_text())
        receipts=[]
        for shard in [0,1]:
        ...
        saved=torch.load(checkpoint,map_location='cpu');assert saved['variant']=='geometry' and saved['m51_spec_sha256']==sha(root/'spec.json')
        assert saved['optimizer_steps']==960 and saved['base_checkpoint_sha256']==spec['checkpoint_sha256']
        result=dict(status='complete',integrity_pass=True,primary='relative_geometry',primary_pass=all(gates.values()),gates=gates,
            aggregates=dict(default=base,m45=paired['aggregates']['m45'],m51=totals),per_sequence=per,
            ...
            geometry_effect_mean_gain=totals['mean_iou']-paired['aggregates']['m45']['mean_iou'],
            next='freeze low22' if all(gates.values()) else 'stop this relative geometry variant; no public evaluation',
            scope='Existing Train development sequences. M45 has matched labels, initialization, sample order, parameters and optimizer; only geometry representation changes. Native templates retained. No public metric.')
    ...
    from lib.test.tracker.sttrack_relative_candidate_set import STTrackRelativeCandidateSet as STTrackCandidateSet
