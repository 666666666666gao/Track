#!/usr/bin/env python3
"""Bound fixed-batch test for non-recurrent target--distractor memory."""
...
def validate_binding(args, spec):
    ...
    relation_extractor = REPOSITORY_ROOT / spec["relation_extractor"]["path"]
    commit = git_output("rev-parse", "HEAD")
    expected = {
        "schema": "sttrack-lachtt-m11-fixed-pool-overfit-binding/v1",
    ...
    if authorization.get("one_fixed_batch_capacity_run") is not True:
        raise ValueError("binding does not authorize M11")
    for name in (
...
def bound_records(spec, source_spec, selected_native_rows):
    ...
    for name, item in spec["m10a"].items():
        records.append(("m10a_" + name, Path(item["path"]),
                        item["sha256"], None))
    for name, item in spec["m10b"].items():
        records.append(("m10b_" + name, Path(item["path"]),
                        item["sha256"], None))
    item = spec["relation_extractor"]
...
def main():
    ...
    if (authorization.get("implementation") is not True or
        ...
        raise ValueError("spec does not authorize M11")
    for name in (
            "sequence_disjoint_pilot_execution", "formal_training",
            "tracking_checkpoint", "online_replay", "depthtrack_test",
            "cdtb", "vot_low22", "vot_full127", "qwen",
    ...
    m10b_result = json_file(spec["m10b"]["result"]["path"])
    if (m10b_result.get("accepted") is not False or
            m10b_result.get("decision") != "m10b_fail_stop_without_rescan"):
        raise ValueError("M10b negative boundary drifted")
    m10_audit = json_file(spec["m10b"]["audit"]["path"])
    if (m10_audit.get("integrity_verdict") != "PASS" or
            m10_audit.get("m10b", {}).get("accepted") is not False):
        raise ValueError("M10 independent audit boundary drifted")
    ...
    relation_spec = spec["relation_extractor"]
    ...
    model = TargetDistractorFixedPoolRouter(
    ...
    result = {
        "schema": "sttrack-lachtt-m11-fixed-pool-overfit-result/v1",
        "complete": True,
        "accepted": accepted,
        "decision": ("m11_pass_freeze_sequence_disjoint_pilot_spec_only"
                     if accepted else "m11_fail_stop_without_rescan"),
    ...
    manifest = {
        "schema": "sttrack-lachtt-m11-fixed-pool-overfit-manifest/v1",
        "complete": True,
        "accepted": accepted,
        "payload": {
