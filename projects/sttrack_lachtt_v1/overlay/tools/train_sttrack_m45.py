"""M45: retain an already-correct default in the identity training targets."""
import argparse
...
def main():
    ...
    torch.save(dict(model=model.state_dict(),variant=variant,spec_sha256=sha(root/'spec.json'),binding_sha256=sha(root/'training_binding.json'),
        base_checkpoint_sha256=spec['checkpoint_sha256'],epochs=opt['epochs'],optimizer_steps=steps,m45_spec_sha256=experiment_sha,target_rule='default_if_iou_at_least_half'),checkpoint)
    ...
    result=dict(m45_spec_sha256=experiment_sha,trainer_sha256=sha(__file__),target_rule='default_if_iou_at_least_half',
        ...
        claim='M45 static Train diagnostics. Same M44 geometry architecture/data/optimizer; only current and preceding identity targets preserve a correct default. Matching accuracy uses this changed target definition. Complete recursion remains decisive.')
