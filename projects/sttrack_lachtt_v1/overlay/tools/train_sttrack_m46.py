"""M46 adds initialization-stage Train inputs at the fixed M45 update budget."""
import argparse
...
def main():
    ...
    assert not (root/'geometry_final.pth').exists()
    ...
    optimizer=torch.optim.AdamW(model.parameters(),lr=opt['lr'],weight_decay=opt['weight_decay']);order=torch.Generator().manual_seed(opt['seed'])
    ...
    for round_id in range(opt['rounds']):
        model.train();total=main_total=pair_total=0.
        shuffled=fit[torch.randperm(len(fit),generator=order)[:opt['examples_per_round']]];digest.update(shuffled.numpy().tobytes());seen.update(shuffled.tolist())
        ...
        row=dict(round=round_id+1,loss=total/len(shuffled),identity_loss=main_total/len(shuffled),matching_loss=pair_total/len(shuffled));losses.append(row)
        print(json.dumps(dict(elapsed=time.time()-started,optimizer_steps=steps,**row)),flush=True)
    assert steps==control['optimizer_steps']==960
    checkpoint=root/'geometry_final.pth'
    torch.save(dict(model=model.state_dict(),variant='geometry',spec_sha256=sha(parent/'spec.json'),binding_sha256=sha(parent/'training_binding.json'),
        base_checkpoint_sha256=spec['checkpoint_sha256'],optimization_rounds=opt['rounds'],optimizer_steps=steps,m46_spec_sha256=sha(root/'spec.json'),target_rule='default_if_iou_at_least_half'),checkpoint)
    ...
    check_binding(parent,spec)
    for name,digest in plan['source_sha256'].items():assert sha(repo/name)==digest,name
    result=dict(status='complete',variant='geometry',m46_spec_sha256=sha(root/'spec.json'),trainer_sha256=sha(__file__),
        ...
        fit=fitstats,development=devstats,current_target_counts_fit=dict(Counter(current_target[fit].tolist())),elapsed_seconds=time.time()-started,
