"""M47: keep M45 actions and accept multiple valid correspondence destinations."""
import argparse
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--root',type=Path,required=True);p.add_argument('--variant',choices=['geometry'],default='geometry')
    args=p.parse_args();output_root=args.root;variant=args.variant;experiment=json.loads((output_root/'spec.json').read_text());experiment_sha=sha(output_root/'spec.json')
    assert sha(__file__)==experiment['trainer_sha256']
    root=Path(experiment['source_root']);spec=json.loads((root/'spec.json').read_text());repo=Path(spec['repository']);sys.path.insert(0,str(repo))
    assert sha(root/'spec.json')==experiment['source_spec_sha256']
    assert sha(root/'training_result.json')==experiment['source_training_result_sha256']
    assert sha(root/'recursive_result.json')==experiment['source_recursive_result_sha256']
    assert not json.loads((root/'recursive_result.json').read_text())['primary_pass']
    assert not (output_root/(variant+'_final.pth')).exists()
    binding=check_binding(root,spec)
    control_root=Path(experiment['control_root'])
    assert sha(control_root/'geometry_result.json')==experiment['control_training_result_sha256']
    assert sha(control_root/'recursive_result.json')==experiment['control_recursive_result_sha256']
    ...
    for source_name,source_digest in experiment['source_sha256'].items():assert sha(repo/source_name)==source_digest,source_name
    from lib.models.sttrack.lachtt_candidate_set import CandidateSetAssociation,select_candidate
    from lib.models.sttrack.lachtt_candidate_multipositive_loss import supervised_loss
    ...
    max_iou_current=current_target.clone();max_iou_previous=previous_target.clone()
    current_target[ious[:,0]>=.5]=0;previous_target[previous_ious[:,0]>=.5]=0
    ...
    control_rows={r['key']:r for split in ['fit','development'] for r in control[split]['rows']}
    assert set(control_rows)==set(keys)
    assert all(int(current_target[i])==control_rows[k]['target'] and int(previous_target[i])==control_rows[k]['previous_target'] for i,k in enumerate(keys))
    ...
    optimizer=torch.optim.AdamW(model.parameters(),lr=opt['lr'],weight_decay=opt['weight_decay']);order=torch.Generator().manual_seed(opt['seed']);order_digest=hashlib.sha256()
    ...
    for epoch in range(opt['epochs']):
        model.train();total=main_total=pair_total=0.;shuffled=fit[torch.randperm(len(fit),generator=order)];order_digest.update(shuffled.numpy().tobytes())
        for index in shuffled.split(opt['batch_size']):
            logits,affinity=model(*inputs(index));loss,main,pair=supervised_loss(logits,affinity,current_target[index].cuda(),previous_target[index].cuda(),ious[index].cuda(),previous_ious[index].cuda())
    ...
    checkpoint=output_root/(variant+'_final.pth')
    torch.save(dict(model=model.state_dict(),variant=variant,spec_sha256=sha(root/'spec.json'),binding_sha256=sha(root/'training_binding.json'),
        base_checkpoint_sha256=spec['checkpoint_sha256'],epochs=opt['epochs'],optimizer_steps=steps,m47_spec_sha256=experiment_sha,target_rule='default_if_iou_at_least_half'),checkpoint)
    def evaluate(net,index):
        ...
        with torch.no_grad():
            for b in index.split(opt['batch_size']):
                ...
                positive=previous_ious[b]>=.5;positions=torch.cat([positive,~positive.any(1,keepdim=True)],dim=1)
                matching_correct+=int((positions.gather(1,predicted[:,None]).flatten()&valid).sum());matching_count+=int(valid.sum())
        ...
        stats=dict(events=len(index),mean_iou=float(value.mean()),default_mean_iou=float(default.mean()),correct=int((value>=.5).sum()),default_correct=int((default>=.5).sum()),
            ...
            positive_sequences=sum(x['gain']>0 for x in seqstats.values()),target_match_membership_correct=matching_correct,target_match_membership_count=matching_count,sequence_stats=seqstats,
    ...
    for source_name,source_digest in experiment['source_sha256'].items():assert sha(repo/source_name)==source_digest,source_name
    assert sha(__file__)==experiment['trainer_sha256'] and sha(output_root/'spec.json')==experiment_sha
    control=json.loads((control_root/'geometry_result.json').read_text())
    assert initial==control['initial_state_sha256'] and order_digest.hexdigest()==control['sample_order_sha256']
    assert steps==control['optimizer_steps']==960
    assert int((current_target[fit]!=max_iou_current[fit]).sum())==121
    result=dict(m47_spec_sha256=experiment_sha,trainer_sha256=sha(__file__),target_rule='default_if_iou_at_least_half',matching_rule='multiple_iou_valid_destinations_for_selected_queries',action_labels_match_m45=True,
        current_fit_labels_changed=int((current_target[fit]!=max_iou_current[fit]).sum()),previous_fit_labels_changed=int((previous_target[fit]!=max_iou_previous[fit]).sum()),
        matched_control_initialization_and_sample_order=True,status='complete',variant=variant,parameters=sum(x.numel() for x in model.parameters()),optimizer_steps=steps,epochs=opt['epochs'],losses=losses,
        ...
        changed_tensors=[name for name,v in model.state_dict().items() if tensor_sha(v)!=initial[name]],sample_order_sha256=order_digest.hexdigest(),
        ...
        claim='M47 static Train diagnostics. Same M45 actions, architecture, data, initialization and sample order; only auxiliary correspondence destination sets change. Membership matching accuracy is not comparable with old exact-slot accuracy. Complete recursion remains decisive.')
    (output_root/(variant+'_result.json')).write_text(json.dumps(result,indent=2)+'\n')
