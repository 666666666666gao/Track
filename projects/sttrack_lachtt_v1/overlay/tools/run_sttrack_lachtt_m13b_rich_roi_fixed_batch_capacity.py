#!/usr/bin/env python3
"""Bound capacity test for detached richer RoI target--distractor relations."""
...
def validate_binding(args, spec):
    ...
    relation_extractor = REPOSITORY_ROOT / spec["relation"]["path"]
    commit = git_output("rev-parse", "HEAD")
    expected = {
        "schema": "sttrack-lachtt-m13b-rich-roi-capacity-binding/v1",
    ...
    if (plan_audit.get("path") !=
            ...
            "EXPERIMENT_AUDIT_M13B_PREEXECUTION_PLAN_20260901.json" or
            plan_audit.get("sha256") !=
            "a017f2e41e5b36ac8baa51fdb69a116de7af7de23926a420fd6e51b84954ffbe" or
    ...
    if (authorization.get("one_fixed_batch_capacity_run") is not True or
        ...
        raise ValueError("binding does not authorize M13b")
    for name in (
    ...
    return binding, runner, model, relation_extractor, commit
...
def bound_records(spec, source_spec, selected_native_rows, binding):
    ...
    records.append((
        "m11_reference_result",
        Path(spec["m11_reference"]["result"]["path"]),
        spec["m11_reference"]["result"]["sha256"], None))
    for boundary_name in ("m12_negative_boundary",
                          "m13a_positive_boundary"):
    ...
    records.append(("pre_execution_plan_audit", Path(audit["path"]),
                    audit["sha256"], None))
    item = spec["relation"]
    records.append(("relation_extractor", REPOSITORY_ROOT / item["path"],
                    item["sha256"], None))
...
def main():
    ...
    if (authorization.get("implementation_after_plan_audit_pass") is not True or
            ...
            authorization.get("independent_result_audit_after_run") is not True):
        raise ValueError("spec does not authorize M13b")
    for name in (
            "sequence_disjoint_pilot_execution", "formal_training",
            "tracking_checkpoint", "online_replay", "depthtrack_test",
            "cdtb", "vot_low22", "vot_full127", "qwen",
    ...
    binding, runner_path, model_path, relation_extractor_path, commit = \
        validate_binding(args, spec)
    ...
    m12_result = json_file(
        spec["m12_negative_boundary"]["result"]["path"])
    if (m12_result.get("accepted") is not False or
            m12_result.get("decision") != "m12_fail_stop_without_rescan"):
        raise ValueError("M12 negative boundary drifted")
    m13a_result = json_file(
        spec["m13a_positive_boundary"]["result"]["path"])
    m13a_audit = json_file(
        spec["m13a_positive_boundary"]["audit"]["path"])
    if (m13a_result.get("accepted") is not True or
            m13a_result.get("decision") !=
            "m13a_pass_freeze_capacity_plan_only" or
            str(m13a_audit.get("integrity_verdict", "")).lower() != "pass"):
        raise ValueError("M13a positive boundary drifted")
    ...
    if (str(plan_audit.get("protocol_gate", "")).lower() != "pass" or
            ...
            plan_audit.get("m13b_executed") is not False):
        raise ValueError("M13b pre-execution audit boundary drifted")
    ...
    if list(relations.shape) != spec["gates"]["relation_shape"]:
    ...
    if (tuple(relation_spec["families"]) != tuple(FAMILY_NAMES) or
            int(relation_spec["projection_width"]) != PROJECTION_WIDTH or
            int(relation_spec["difference_blocks"]) != DIFFERENCE_BLOCKS or
            int(relation_spec["total_width"]) != RICH_RELATION_DIM):
        raise ValueError("rich relation source contract drifted")
    ...
    if parameter_count != int(model_spec["expected_parameters"]):
        raise ValueError("model parameter count drifted")
    parameter_relative_difference = abs(
        parameter_count - int(model_spec["m11_parameters"])) / float(
            model_spec["m11_parameters"])
    if parameter_relative_difference > float(
            model_spec["parameter_relative_difference_max"]):
        raise ValueError("M11 parameter budget match drifted")
    ...
    conditions = {
        ...
        "extractor_permutation_error_max":
            extractor_permutation_error <= float(
                gates["extractor_permutation_error_max"]),
        ...
        "parameter_relative_difference_max":
            parameter_relative_difference <= float(
                gates["parameter_relative_difference_max"]),
    ...
    result = {
        "schema": "sttrack-lachtt-m13b-rich-roi-capacity-result/v1",
        "complete": True,
        "accepted": accepted,
        "decision": ("m13b_pass_freeze_sequence_disjoint_pilot_plan_only"
                     if accepted else "m13b_fail_stop_without_rescan"),
        ...
        "model": {
            ...
            "parameter_absolute_difference": abs(
                parameter_count - int(model_spec["m11_parameters"])),
            ...
            "relation_nonfinite": relation_nonfinite,
    ...
    manifest = {
        "schema": "sttrack-lachtt-m13b-rich-roi-capacity-manifest/v1",
        "complete": True,
        "accepted": accepted,
        "payload": {
