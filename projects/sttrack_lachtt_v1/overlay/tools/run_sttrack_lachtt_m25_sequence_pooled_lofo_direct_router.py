#!/usr/bin/env python3
"""M25 sequence-pooled leave-one-fold-out direct router."""
...
SPEC_SCHEMA = "sttrack-lachtt-m25-sequence-pooled-lofo-direct-router-spec/v1"
PREFLIGHT_SCHEMA = "sttrack-lachtt-m25-preflight-binding/v1"
PREAUDIT_SCHEMA = "sttrack-lachtt-m25-preexecution-audit/v1"
FOLDS = (2, 3, 4, 5)
EXPECTED_FOLD_COUNTS = {
    2: {"sequences": 20, "events": 132, "unique": 530,
        "training_events": 375, "steps": 564},
    3: {"sequences": 17, "events": 103, "unique": 434,
        "training_events": 404, "steps": 612},
    4: {"sequences": 18, "events": 73, "unique": 306,
        "training_events": 434, "steps": 660},
    5: {"sequences": 21, "events": 199, "unique": 836,
        "training_events": 308, "steps": 468},
}
...
def validate_spec(spec, args, spec_record, runner_record, m23):
    ...
    expected_training = {
        ...
        "optimizer_steps_per_member": {"2": 564, "3": 612,
                                       "4": 660, "5": 468},
        "optimizer_steps_total": 2304,
    ...
    if spec.get("training") != expected_training:
        raise ContractError("M25 training constants drifted")
    expected_architecture = {
        "class": "UniqueHypothesisSelectiveRouter",
        "leave_one_fold_out_models": 4,
    ...
    if spec.get("architecture") != expected_architecture:
        raise ContractError("M25 architecture constants drifted")
    expected_policy = {
        "evaluation": "sequence-pooled leave-one-fold-out",
        "models_per_event": 1,
        "model_trained_on_other_three_folds": True,
    ...
    if spec.get("oof_policy") != expected_policy:
        raise ContractError("M25 OOF policy drifted")
    expected_gates = {
    ...
    if spec.get("scientific_gates") != expected_gates:
        raise ContractError("M25 scientific gates drifted")
    if set(spec.get("m22_inputs", {})) != {"spec", "binding"}:
        raise ContractError("M25 input set expanded beyond frozen Train sources")
...
def validate_preflight(path, spec_record, runner_record, spec, m23):
    ...
    if (binding.get("schema") != PREFLIGHT_SCHEMA or
        ...
        raise ContractError("M25 preflight binding drifted")
...
def validate_preaudit(path, spec_record, runner_record, preflight_record,
    ...
    if (audit.get("schema") != PREAUDIT_SCHEMA or
            ...
            audit.get("authorization", {}).get("m25_execution") is not True or
            audit.get("audited_identity") != expected or
            audit.get("claim_ceiling") != spec.get("claim_ceiling")):
        raise ContractError("M25 preaudit did not authorize exact execution")
    return audit, record
...
def publish(output, result, trace, predictions, m23):
    ...
    try:
        ...
        manifest = {
            "schema": "sttrack-lachtt-m25-output-manifest/v1",
            "complete": True,
        ...
        if tuple(sorted(path.name for path in temporary.iterdir())) != tuple(
                sorted(OUTPUT_FILES)):
            raise ContractError("M25 output set drifted")
...
def main():
    ...
    if (git_output("rev-parse", "HEAD") != repository["commit"] or
        ...
        raise ContractError("repository identity drifted before M25")
    ...
    if len(training_groups) != 507:
        raise ContractError("M25 training event count drifted")
    ...
    for key, rows in training_groups.items():
        ...
        if entry["partition"] != "training" or fold not in FOLDS:
            raise ContractError("M25 training fold identity drifted")
    ...
    if (min(unique_counts.values()) != 2 or
        ...
        raise ContractError("M25 unique hypothesis census drifted")
    for fold in FOLDS:
        if sum(unique_counts[key] for key in fold_groups[fold]) != (
                EXPECTED_FOLD_COUNTS[fold]["unique"]):
            raise ContractError("M25 per-fold unique census drifted")
    ...
    for fold in FOLDS:
        ...
        if (count != architecture["expected_parameters_per_member"] or
            ...
            raise ContractError("M25 model identity drifted")
        models[fold] = model
    ...
    if len(set(initial_states.values())) != 1:
        raise ContractError("M25 models did not share identical initialization")
    ...
    for fold in FOLDS:
        ...
        training_folds = tuple(value for value in FOLDS if value != fold)
        groups = {
            key: rows for training_fold in training_folds
            for key, rows in fold_groups[training_fold].items()
        }
        if len(groups) != EXPECTED_FOLD_COUNTS[fold]["training_events"]:
            raise ContractError("M25 pooled training event count drifted")
        ...
        for epoch in range(training["epochs_per_member"]):
            ...
            if len(batches) != expected_batches:
                raise ContractError("M25 epoch batch count drifted")
            for epoch_step, keys in enumerate(batches):
                ...
                if any(not torch.isfinite(value).all().item()
                    ...
                    raise ContractError("M25 output is non-finite")
                ...
                if (nonfinite or not math.isfinite(preclip) or
                    ...
                    raise ContractError("M25 preclip gradient gate failed")
                ...
                if (post_nonfinite or not math.isfinite(postclip) or
                        postclip > 5.000001):
                    raise ContractError("M25 postclip gradient gate failed")
                optimizer.step()
                trace.append({
    ...
    if (global_step != training["optimizer_steps_total"] or
            ...
            any(member_steps[fold] != EXPECTED_FOLD_COUNTS[fold]["steps"]
                for fold in FOLDS)):
        raise ContractError("M25 optimizer step count drifted")
    ...
    for eval_fold in FOLDS:
        ...
        training_folds = tuple(fold for fold in FOLDS if fold != eval_fold)
        for start in range(0, len(keys_for_fold), 32):
            ...
            for index, key in enumerate(keys):
                decision = m23.policy_decision(
                    torch, outputs, index, targets[key]["valid"],
                    spec["oof_policy"])
                selected_role = decision["selected_role_id"]
                row = {
                    "record_type": "sequence_pooled_lofo_prediction",
                    "evaluation_fold": eval_fold,
                    "model_excluded_fold": eval_fold,
                    "model_training_folds": list(training_folds),
                    "sequence": key[0],
                    "event_id": key[1],
                    "trigger_frame": key[2],
    ...
    engineering_conditions = {
        ...
        "optimizer_steps_exact": global_step == 2304,
        "trace_rows_exact": len(trace) == 2304,
        ...
        "sequence_fold_overlap_zero": all(
            ...
            for index, left in enumerate(FOLDS)
            for right in FOLDS[index + 1:]),
        "oof_training_exclusion_exact": all(
            row["model_excluded_fold"] == row["evaluation_fold"] and
            row["evaluation_fold"] not in row["model_training_folds"] and
            len(row["model_training_folds"]) == 3
    ...
    decision = (
        "m25_pass_authorize_separate_plan_only_for_consumed_fold1"
        if accepted else
        "m25_fail_stop_sequence_pooled_lofo_direct_router_without_scan")
    result = {
        "schema": "sttrack-lachtt-m25-result/v1",
        "complete": True,
        "accepted": accepted,
        ...
        "data": {
            ...
            "folds": {
                str(fold): {
                    ...
                    "model_training_folds": [
                        value for value in FOLDS if value != fold],
                    "model_training_events": sum(
                        len(fold_groups[value]) for value in FOLDS
                        if value != fold),
                } for fold in FOLDS
            },
        ...
        "model": {
            ...
            "leave_one_fold_out_models": len(models),
    ...
    if (m23.file_record(Path(__file__).resolve()) != runner_record or
            ...
            git_output("status", "--porcelain")):
        raise ContractError("M25 source changed during execution")
