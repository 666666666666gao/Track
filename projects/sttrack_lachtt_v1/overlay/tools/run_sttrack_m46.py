"""Run the frozen M46 weight in two sequence shards, then audit both outputs."""
import argparse
...
def main():
    ...
    assert training['m46_spec_sha256']==sha(root/'spec.json') and training['matched_control_initialization_and_optimizer_budget']
    ...
    if args.analyze:
        ...
        saved=torch.load(checkpoint,map_location='cpu');assert saved['variant']=='geometry' and saved['m46_spec_sha256']==sha(root/'spec.json')
        assert saved['optimizer_steps']==960 and saved['base_checkpoint_sha256']==spec['checkpoint_sha256']
        result=dict(status='complete',integrity_pass=True,primary='initial_frame_coverage',primary_pass=all(gates.values()),gates=gates,
            aggregates=dict(default=base,m45=paired['aggregates']['m45'],m46=totals),per_sequence=per,
            ...
            analysis_source_sha256=sha(__file__),training_result_sha256=sha(root/'geometry_result.json'),control_result_sha256=sha(control_root/'recursive_result.json'),
            early_training_effect_mean_gain=totals['mean_iou']-paired['aggregates']['m45']['mean_iou'],
            next='freeze low22' if all(gates.values()) else 'stop this initial-frame variant; no public evaluation',
            scope='Existing Train development sequences. Historical M45 control has matched initialization, architecture, optimizer and960-update budget; only fitting-input pool adds frames2-9. Twenty sampled rounds are not20complete epochs over2015inputs. No public metric.')
