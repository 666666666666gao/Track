"""M51: train the same M45 targets with previous-object-relative geometry."""
import argparse
...
def main():
    ...
    for name,source_digest in experiment['source_sha256'].items():assert sha(repo/name)==source_digest,name
    paired_path=Path(experiment['paired_root'])/'geometry_result.json'
    assert sha(paired_path)==experiment['paired_training_result_sha256']
    paired=json.loads(paired_path.read_text())
    assert sha(root/'training_result.json')==experiment['control_training_result_sha256']
    assert sha(root/'recursive_result.json')==experiment['control_recursive_result_sha256']
    ...
    from lib.models.sttrack.lachtt_candidate_set import select_candidate,supervised_loss
    from lib.models.sttrack.lachtt_relative_geometry import RelativeCandidateSetAssociation as CandidateSetAssociation
    ...
    torch.save(dict(model=model.state_dict(),variant=variant,spec_sha256=sha(root/'spec.json'),binding_sha256=sha(root/'training_binding.json'),
        base_checkpoint_sha256=spec['checkpoint_sha256'],epochs=opt['epochs'],optimizer_steps=steps,m51_spec_sha256=experiment_sha,target_rule='default_if_iou_at_least_half',geometry_encoding='previous_selected_box_relative'),checkpoint)
    ...
    control=json.loads((root/'training_result.json').read_text())['variants']['geometry']
    assert initial==control['initial_state_sha256'] and digest.hexdigest()==control['sample_order_sha256']
    assert steps==control['optimizer_steps']==960
    assert initial==paired['initial_state_sha256'] and digest.hexdigest()==paired['sample_order_sha256']
    assert steps==paired['optimizer_steps'] and losses[0]['epoch']==1
    for name,source_digest in experiment['source_sha256'].items():assert sha(repo/name)==source_digest,name
    assert int((current_target[fit]!=max_iou_current[fit]).sum())==121
    result=dict(geometry_encoding='previous_selected_box_relative',paired_training_sha256=sha(paired_path),m51_spec_sha256=experiment_sha,trainer_sha256=sha(__file__),target_rule='default_if_iou_at_least_half',
        ...
        claim='M51 static Train diagnostics. Same M45 labels, parameters, initialization and sample order; only geometry is relative to the previous selected box. Complete recursion remains decisive.')
