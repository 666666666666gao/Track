"""Run the frozen M47 weight in two sequence shards, then audit both outputs."""
import argparse
...
def main():
    ...
    assert training['m47_spec_sha256']==sha(root/'spec.json') and training['matched_control_initialization_and_sample_order']
    ...
    if args.analyze:
        ...
        saved=torch.load(checkpoint,map_location='cpu');assert saved['variant']=='geometry' and saved['m47_spec_sha256']==sha(root/'spec.json')
        assert saved['optimizer_steps']==960 and saved['base_checkpoint_sha256']==spec['checkpoint_sha256']
        result=dict(status='complete',integrity_pass=True,primary='multipositive_correspondence',primary_pass=all(gates.values()),gates=gates,
            aggregates=dict(default=base,m45=paired['aggregates']['m45'],m47=totals),per_sequence=per,
            ...
            correspondence_effect_mean_gain=totals['mean_iou']-paired['aggregates']['m45']['mean_iou'],
            next='freeze low22' if all(gates.values()) else 'stop this correspondence variant; no public evaluation',
            scope='Existing Train development sequences. Historical M45 control has identical initialization, actions, architecture, data, sample order and960updates. Only auxiliary partial-correspondence positive destination sets change. No public metric.')
