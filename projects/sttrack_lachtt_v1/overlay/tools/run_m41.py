...
def main():
    ...
    if args.keys_file:
        requested = json.loads(args.keys_file.read_text())
        cases = [c for c in cases if c['key'] in requested]
        assert {c['key'] for c in cases} == set(requested)
    ...
    for index, case in enumerate(cases):
        ...
        maximum_error = 0.
        rounded_crop_discrepancies = []
        for step in range(1, case['progress'] + 1):
            expected_prior = case['init_bbox'] if step == 1 else case['expected_boxes'][step - 2]
            if geometry(tracker.state, 4) != geometry(expected_prior, 4):
                rounded_crop_discrepancies.append(dict(step=step, actual_prior=list(tracker.state),
                    serialized_prior=expected_prior, actual_crop=geometry(tracker.state, 4),
                    serialized_crop=geometry(expected_prior, 4)))
            ...
            serialized = np.array([float(format(float(np.float32(v)), '.4f')) for v in result['target_bbox']], dtype=np.float32)
            expected_serialized = np.array(case['expected_boxes'][step - 1], dtype=np.float32)
            assert np.array_equal(serialized, expected_serialized), (case['key'], step, result['target_bbox'], case['expected_boxes'][step - 1])
        ...
        event = dict(key=case['key'], sequence=case['sequence'], onset_frame=case['onset_frame'],
                     ...
                     serialization_exact=True, serialized_crop_rounding_discrepancies=rounded_crop_discrepancies,
