"""M59: train-only content diagnosis of M58, whose promotion gate remains failed."""
import argparse
...
def parent_ready():
    ...
    assert sha(PARENT / 'recursive_result.json') == spec['negative_parent_result_sha256']
...
def seal_bundle():
    ...
    write(ROOT / 'bundle_receipt.json', dict(status='diagnostic_bundle_sealed_with_parent_promotion_failed',
...
def analyze():
    ...
    result = dict(status='completed_train_only_same_head_content_diagnosis', observed_utc=datetime.now(timezone.utc).isoformat(),
        content_spec_sha256=sha(ROOT / 'spec.json'), bundle_sha256=sha(ROOT / 'bundle.json'),
        ...
        descriptive_content_thresholds=gates, parent_promotion_pass=False, public_evaluation_allowed=False, attributes_descriptive_only=True,
        ...
        next='Interpret all fixed-head content effects; M58 remains unpromoted; any model revision needs new Train training and recursive validation')
