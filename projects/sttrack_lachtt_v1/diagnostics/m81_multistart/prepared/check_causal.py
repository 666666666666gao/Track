...
ROOT = Path('/root/autodl-tmp/sttrack_m81_multistart_20260908')
...
episode=json.loads((ROOT/'smoke_episode.json').read_text())
...
for arm in trackers:
    ...
    bank = torch.load(b['path'], map_location='cpu'); j = bank['ids'].index(episode['id'])
    infos[arm] = dict(init_bbox=episode['init_bbox'], text_tokens=bank['tokens'][j], text_mask=bank['mask'][j], empty_text=bank['empty'])
...
assert writes['category'] == writes['empty'] == native_writes
del native
assert sha(folder / 'groundtruth.txt') == row['groundtruth_sha256']
gt = np.loadtxt(folder / 'groundtruth.txt', delimiter=',').reshape(-1, 4)[offset:]
...
result = dict(status='completed_M81_native_parity_and_causal_smoke', observed_utc=datetime.now(timezone.utc).isoformat(),
    ...
    synthetic_competition_contract=synthetic_check, formal_optimizer_steps=0, smoke_weights_saved=False, later_initialization_frame=offset, smoke_episode_sha256=sha(ROOT/'smoke_episode.json'), independent_model_review_pass=False, elapsed_seconds=time.time()-started)
