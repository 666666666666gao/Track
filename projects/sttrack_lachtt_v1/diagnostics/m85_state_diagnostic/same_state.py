"""Read-only M84 content and dense-candidate diagnostics. No GT loading and no counterfactual commits."""
import argparse
import hashlib
import json
import math
...
ROOT = Path('/root/autodl-tmp/sttrack_m85_state_diagnostic_20260921')
PARENT = Path('/root/autodl-tmp/sttrack_m84_centered_20260920')
...
def main():
    ...
    assert sha(PARENT / 'integration.json') == spec['integration_sha256']
    ...
    with torch.no_grad():
        for case in cases:
            ...
            rows = []
            dense = {arm: [] for arm in ['category', 'swapped', 'native']}
            for i in range(1, limit):
                ...
                for key in ['score_map', 'size_map', 'offset_map']:
                    assert torch.equal(outputs['empty'][key], outputs['native'][key]), (seq, i, key, 'empty/native maps')
                for arm in dense:
                    maps = torch.cat([outputs[arm][key] for key in ['score_map', 'size_map', 'offset_map']], dim=1)[0]
                    assert maps.shape == (5, 16, 16) and torch.isfinite(maps).all()
                    dense[arm].append(maps.cpu().numpy())
                ...
                rows.append(dict(frame=i, previous_bbox=previous, search_side=side,
                    crop_origin=[round(previous[0]+.5*previous[2]-.5*side), round(previous[1]+.5*previous[3]-.5*side)],
                    image_hw=[h,w], actual_template_write=bool(i % 50 == 0 and float(actual['best_score']) > .75),
                    variants=variants))
            path = outdir / (seq + '.json')
            path.write_text(json.dumps(dict(sequence=seq, rows=rows), separators=(',', ':'), allow_nan=False)+'\n')
            dense_path = outdir / (seq + '.npz')
            np.savez_compressed(str(dense_path), window=tracker.output_window.cpu().numpy(),
                **{arm: np.stack(values) for arm, values in dense.items()})
            receipt = dict(sequence=seq, positions=len(rows), sha256=sha(path),
                dense_sha256=sha(dense_path), elapsed_seconds=time.time()-started)
            receipts.append(receipt)
            print(json.dumps(receipt), flush=True)
    report = dict(status='complete', spec_sha256=sha(ROOT/'spec.json'), source_sha256=sha(Path(__file__)),
        head_sha256=sha(checkpoint), mode='preflight' if args.preflight else 'selected3',
