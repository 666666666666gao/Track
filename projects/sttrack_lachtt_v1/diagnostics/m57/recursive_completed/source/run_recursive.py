"""M57 four-arm full causal development trajectories, with independent native comparison."""
import argparse
...
def bound(root):
    ...
    for path, digest in frozen['inference_input_sha256'].items():
...
def run(root, arm):
    ...
    assert set(bank['sequences']) == set(initial_bank['sequences']) == {case['sequence'] for case in cases}
    ...
    for case in cases:
        ...
        reference_index = initial_bank['sequences'].index(case['sequence'])
        assert torch.equal(tracker.reference_bank.initial.detach().cpu().half(), initial_bank['references'][reference_index])
    ...
    write(root / (arm + '_recursive_receipt.json'), dict(status='complete', variant=arm, sequences=receipts,
        ...
        t0_reference_exactly_verified_for_all_22_sequences=True, same_slot_mask=True))
...
def analyze(root):
    ...
    primary = aggregates['initial_text']
    rule = spec['recursive_gate']
    broken = [name for name in sorted(names) if per['native'][name]['failure_episodes'] == 0
              and per['initial_text'][name]['failure_episodes'] > 0]
    gates = { 'mean_vs_' + arm: primary['mean_iou'] >= aggregates[arm]['mean_iou'] + rule['mean_gain_vs_' + arm]
              for arm in ['native', 'initial_visual', 'candidate_text'] }
    semantic_did = ((primary['mean_iou'] - aggregates['initial_visual']['mean_iou'])
                    - (aggregates['candidate_text']['mean_iou'] - aggregates['candidate_visual']['mean_iou']))
    gates['semantic_difference_of_differences'] = semantic_did > 0
    gates['low_frames'] = primary['low_iou_frames'] <= aggregates['native']['low_iou_frames']
    gates['H10'] = primary['failure_episodes'] <= aggregates['native']['failure_episodes']
    ...
    result = dict(status='complete_recursive_development', recursive_spec_sha256=sha(root / 'recursive_spec.json'),
        ...
        gates=gates, primary_variant='initial_text', primary_pass=all(gates.values()), new_failure_sequences=broken,
        semantic_difference_of_differences=semantic_did,
        ...
        claim='Same-base equal-budget same-slot initial binding and lexical-content comparison; no VOT or three-dataset gain yet',
        next='Eligible for fixed-weight same-mask recursive text counterfactual checks before frozen low22' if all(gates.values()) else 'Stop this frozen revision; diagnose all harms')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arm', choices=['candidate_text', 'candidate_visual', 'initial_text', 'initial_visual'])
