...
class CausalTrainingTracker(STTrackSemantic):
    ...
    def step(self, image):
        ...
        # Decode the already-computed native head BEFORE changing the public state.
        # This teacher is only a training readout; it never commits a trajectory.
        with torch.no_grad():
            native_response = self.output_window * native['score_map']
            native_boxes = self.network.box_head.cal_bbox(native_response,
                native['size_map'], native['offset_map']).view(-1, 4)
            native_prediction = (native_boxes.mean(dim=0) * self.params.search_size / resize).tolist()
            image_h, image_w, _ = image.shape
            native_bbox = clip_box(self.map_box_back(native_prediction, resize), image_h, image_w, margin=10)
        features = native['candidate_features']
        context = self.semantic_context
        enhanced, semantic_aux = self.network.semantic_adapter(features['search_rgb_tokens'],
        ...
        out['native_score_map'] = native['score_map'].detach()
        out['native_bbox'] = native_bbox
...
def base_supervision(network, out, groundtruth, previous, resize, search_size):
    ...
    if inside:
        ...
        cell = (centre.detach().cpu()[None] * 16).round().clamp(0, 15)
...
def supervision(network, out, groundtruth, previous, resize, search_size, support_weight=0.):
    ...
    if loss is None or support_weight == 0.:
