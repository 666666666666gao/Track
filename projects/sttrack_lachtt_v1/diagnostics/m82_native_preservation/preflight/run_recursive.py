...
def run(arm):
    ...
    training_arm = 'empty' if arm == 'empty' else 'category'
    content = 'empty' if arm == 'category_empty' else ('swapped' if arm == 'category_swapped' else arm)
    checkpoint = ROOT/'training'/training_arm/'final.pth'
    ...
    receipt = dict(status='complete', arm=arm, recursive_spec_sha256=sha(ROOT/'recursive_spec.json'),
        training_result_sha256=sha(ROOT/'training'/training_arm/'result.json'), head_sha256=sha(checkpoint),
...
def analyze():
    ...
    for arm in predicted:
        ...
        assert receipt['head_sha256']==results['empty' if arm == 'empty' else 'category']['final_checkpoint_sha256']
    ...
    per = {'native':native['per_sequence']['native'], **{arm:{} for arm in FAMILIES}}
    for case in spec['cases']:
    ...
    gates=dict(mean_vs_native=primary['mean_iou']>=baseline['mean_iou']+rule['category_pooled_mean_vs_native_minimum'],
    ...
    parent_path=Path(training['parent_recursive_result_path'])
    assert sha(parent_path)==training['parent_recursive_result_sha256']
    parent=json.loads(parent_path.read_text())['aggregates']
    hann_path=Path(training['hann_recursive_result_path'])
    assert sha(hann_path)==training['hann_recursive_result_sha256']
    hann=json.loads(hann_path.read_text())['aggregates']
    m78_path=Path(training['matched_M78_result_path'])
    assert sha(m78_path)==training['matched_M78_result_sha256']
    m78=json.loads(m78_path.read_text())
    incremental={}
    for arm in ['category','empty']:
        a=aggregates[arm]; b=m78['aggregates'][arm]
        incremental[arm]=dict(mean=a['mean_iou']>=b['mean_iou'], macro=a['macro_sequence_mean_iou']>=b['macro_sequence_mean_iou'], low=a['low_iou_frames']<=b['low_iou_frames'], H10=a['failure_episodes']<=b['failure_episodes'])
    content_gates={}
    for arm in ['category_empty','category_swapped']:
        a=aggregates[arm]
        content_gates[arm]=dict(mean=primary['mean_iou']>a['mean_iou'], macro=primary['macro_sequence_mean_iou']>=a['macro_sequence_mean_iou'], low=primary['low_iou_frames']<=a['low_iou_frames'], H10=primary['failure_episodes']<=a['failure_episodes'])
    leave_one_out={arm:{} for arm in ['category_empty','category_swapped']}
    for arm in leave_one_out:
        for name in per['category']:
            a=per['category'][name]; b=per[arm][name]
            leave_one_out[arm][name]=(primary['iou_sum']-a['iou_sum'])/(primary['valid_frames']-a['valid_frames'])-(aggregates[arm]['iou_sum']-b['iou_sum'])/(aggregates[arm]['valid_frames']-b['valid_frames'])
    result=dict(status='complete_recursive_development', observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        matched_M78_aggregates=m78['aggregates'], matched_M78_result_sha256=sha(m78_path), preservation_incremental_gates=incremental, preservation_category_increment_pass=all(incremental['category'].values()), content_gates=content_gates, content_pass=all(all(g.values()) for g in content_gates.values()), content_leave_one_out=leave_one_out,
        ...
        independent_review_pass=False, next='All four development families complete; audit before any promotion; no automatic public evaluation')
...
if __name__=='__main__':
    ...
    action.add_argument('--arm',choices=FAMILIES)
