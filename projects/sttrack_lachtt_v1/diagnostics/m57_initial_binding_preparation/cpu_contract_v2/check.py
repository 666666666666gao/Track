...
def main():
    ...
    for reference_mode in ['candidate', 'initial']:
        for use_text in [True, False]:
            ...
            slots = model.attribute_alignment.slot_embedding.detach()
            assert torch.unique(slots, dim=0).shape[0] == 5
            slot_separation = float(torch.pdist(slots).min())
            queries = model.attribute_alignment.queries(tokens).detach()
            query_separation = float(torch.pdist(queries[0, mask[0]]).min())
            assert query_separation > 0
            with torch.no_grad():
            ...
            for step in range(3):
                ...
                qkv_norms = {}
                for attention in ['read_visual', 'guide_visual']:
                    gradient = getattr(model.attribute_alignment, attention).in_proj_weight.grad
                    assert gradient is not None and torch.isfinite(gradient).all()
                    for component, value in zip(['query', 'key', 'value'], gradient.chunk(3)):
                        qkv_norms[attention + '.' + component] = float(value.norm())
                qkv_gradients.append(qkv_norms)
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1.)
                optimizer.step()
            assert all(any(row[group] > 0 for row in gradients[1:]) for group in gradients[0])
            assert all(any(row[group] > 0 for row in qkv_gradients[1:]) for group in qkv_gradients[0])
            with torch.no_grad():
                actual = model(*inputs, tokens, mask)
                padded = tokens.clone()
                padded[~mask] = 1000.
                ...
                perturbation = torch.linspace(-.5, .5, 32).reshape(1, 1, 1, 32)
                probe_results = []
                for object_index in range(22):
                    changed = cells.clone()
                    changed[:, object_index] += perturbation
                    model.attribute_alignment(changed, tokens, mask)
                    changed_objects = [j for j in range(22)
                        if not torch.equal(observed[0][:, j], observed[-1][:, j])]
                    expected_objects = ([object_index] if reference_mode == 'candidate'
                        else (list(range(22)) if object_index == 20 else []))
                    assert changed_objects == expected_objects
                    probe_results.append(dict(perturbed_object=object_index,
                        changed_conditioned_objects=changed_objects))
                hook.remove()
            results.append(dict(reference_mode=reference_mode, text='real' if use_text else 'visual_queries',
                ...
                initial_slot_minimum_separation=slot_separation,
                initial_valid_query_minimum_separation=query_separation,
                valid_slots=int(mask[0].sum()), actual_conditioning_object_probes=probe_results,
                data_gradient_norms_before_optimizer=gradients,
                attention_qkv_data_gradient_norms_before_optimizer=qkv_gradients,
                discarded_optimizer_steps=3))
