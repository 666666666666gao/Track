...
核对四组初始张量/参数一致、零输出残差时与父头完全一致、padding不改变输出。通过实际forward的guide_visual hook观察条件化短语：改变当前/上一/dynamic格点时，initial组参照应保持，candidate组应变化；改变t0格点应影响initial组当前候选的条件化短语，而candidate组只改变对应参考对象。逐组记录optimizer执行前各交互子模块的数据梯度，避免把AdamW权重衰减导致的参数变化当作梯度证据。
