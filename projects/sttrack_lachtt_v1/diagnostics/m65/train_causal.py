"""Matched category-only full-sequence training; control versus fixed null-support normalization."""
import argparse
...
def main():
    ...
    parser.add_argument('--arm', choices=['control', 'null'], required=True)
    ...
    assert sha(root / 'text_fit.pt') == spec['text_fit_sha256']
    ...
    assert tracker.use_text and tracker.network.semantic_adapter.null_support == (args.arm == 'null')
    ...
    def checkpoint(complete):
        return dict(architecture='semantic_spatial_support_v1', null_support=args.arm == 'null', arm=args.arm, model=tracker.network.semantic_adapter.state_dict(),
            ...
            status='complete' if complete else 'in_progress', seed=spec['seed'])
    ...
    result = dict(status='one_full_causal_fit_pass_complete', arm=args.arm, null_support=args.arm == 'null', sequences=len(receipts),
