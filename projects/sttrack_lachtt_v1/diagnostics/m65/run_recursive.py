...
def trained():
    ...
    for key in ['initial_adapter_state_sha256', 'base_state_before_sha256', 'optimizer_steps', 'total_track_calls']:
        assert results['null'][key] == results['control'][key], key
...
def run(arm):
    ...
    assert saved['use_text'] and saved['null_support'] == (arm == 'null')
...
def analyze():
    ...
    predicted = {a:{} for a in ['null','control']}
    receipts = {}
    ...
    per = {'native':native['per_sequence']['native'], 'null':{}, 'control':{}}
    for case in spec['cases']:
    ...
    primary=aggregates['null']; baseline=aggregates['native']; control=aggregates['control']
    broken=[n for n in per['native'] if per['native'][n]['failure_episodes']==0 and per['null'][n]['failure_episodes']>0]
    rule=training['promotion_gates']
    control_broken=[n for n in per['control'] if per['control'][n]['failure_episodes']==0 and per['null'][n]['failure_episodes']>0]
    ...
    result=dict(status='complete_recursive_development', observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        new_failure_sequences=broken, broken_control_success_sequences=control_broken, receipts=receipts, frozen_native_result_sha256=sha(native_path),
...
if __name__=='__main__':
    ...
    action.add_argument('--arm',choices=['null','control'])
