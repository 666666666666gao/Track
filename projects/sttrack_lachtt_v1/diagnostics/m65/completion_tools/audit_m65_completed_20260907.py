...
ROOT=BASE/'sttrack_m65_category_null_support_20260907'
...
def reference():
    ...
    _,_,_,aggregate,_,_,families=recompute(PARENT,['text','visual'],read(PARENT/'recursive_result.json'))
    report=dict(status='historical_sealed_family_and_scalar_metric_reference_pass',observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        new_M65_prediction_files_opened=False,M65_completion_audited=False,new_training_steps=0,new_tracking_calls=0,
...
def training_artifacts(training):
    ...
    for arm in ['control','null']:
        ...
        assert r['status']=='one_full_causal_fit_pass_complete' and r['arm']==arm and r['null_support']==(arm=='null')
        ...
        assert all(x['null_support']==(arm=='null') and x['use_text'] for x in [initial,final,latest])
        ...
        calls=steps=0;labels=Counter();scheduled_writes=0
        ...
        report[arm]=dict(result_sha256=sha(path/'result.json'),final_checkpoint_sha256=sha(path/'final.pth'),
            ...
            initial_adapter_state_sha256=r['initial_adapter_state_sha256'])
    assert all(torch.equal(initial_states['control'][k],initial_states['null'][k]) for k in initial_states['control'])
    assert base_states['control']==base_states['null']
    assert report['control']['initial_adapter_state_sha256']==report['null']['initial_adapter_state_sha256']
    for key in ['optimizer_steps','track_calls','supervised_frames']:assert report['control'][key]==report['null'][key]
    assert any(not torch.equal(final_states['control'][k],final_states['null'][k]) for k in final_states['control'])
    return report
...
def completed():
    ...
    for n in ['controller.exit','recursive_analysis.exit','training_control.exit','training_null.exit','control_recursive.exit','null_recursive.exit']:
    ...
    for n,k in [('train_causal.py','training_script_sha256'),('causal_training.py','causal_script_sha256'),
        ('run_m65.sh','run_queue_sha256'),('text_fit.pt','text_fit_sha256'),('text_development.pt','text_development_sha256')]:assert sha(ROOT/n)==training[k]
    ...
    spec,training,per,aggregate,overlap,intervals,families=recompute(ROOT,['control','null'],result)
    train=training_artifacts(training)
    primary=aggregate['null'];control=aggregate['control'];native=aggregate['native'];rule=training['promotion_gates']
    broken={a:[seq for seq in per[a] if per[a][seq]['failure_episodes']==0 and per['null'][seq]['failure_episodes']>0] for a in ['native','control']}
    gates=dict(mean_vs_native=primary['mean_iou']>=native['mean_iou']+rule['null_pooled_mean_vs_native_minimum'],
    ...
    for reference_arm in ['native','control']:
        for seq,runs in intervals['null'].items():
    ...
    report=dict(status='completed_M65_artifacts_and_development_audited',observed_utc=datetime.now(timezone.utc).isoformat(),
      ...
      training=train,families=families,recomputed_aggregates=aggregate,recomputed_frozen_gates=gates,
      broken_success_sequences=broken,sustained_null_H10_with_reference_correct_every_frame=harms,
