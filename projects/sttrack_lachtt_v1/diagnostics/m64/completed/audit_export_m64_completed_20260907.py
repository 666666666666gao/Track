...
def main():
    ...
    native_metrics={k:old['metrics_percent'][k.lower()] for k in values}
    ...
    report=dict(status='completed_negative_low22_evidence_verified',observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        metrics_percent=values,native_metrics_percent=native_metrics,delta_metrics_percent={k:values[k]-native_metrics[k] for k in values},
        ...
        initial_audit_source_sha256=sha(ROOT/'audit_export_before_native_metric_key_fix.py'),initial_audit_exit=1,
        audit_repair='Mapped the existing lowercase M39 metric fields to the current uppercase reporting keys. No model input, tracking output, official analysis or gate changed.',
    ...
    OUT.mkdir()
    names=['low22_result.json','completed_evidence_audit.json','low22_tracking.exit','low22_analysis.exit','low22_controller.exit','low22_analysis.log','toolkit_analysis.log','low22_execution.json','audit_export_before_native_metric_key_fix.py','completed_evidence_audit_initial.log','completed_evidence_audit_initial.exit']
