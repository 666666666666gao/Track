"""Conditional full VOT preparation preserving the exact M64 low22 inputs."""
import argparse
...
TRACKER='sttrack_m64_category_full127'
...
def eligible():
    p=Path('/root/autodl-tmp/m64_full_preparation_20260907.py')
...
def prepare_captions():
    ...
    old_plan=json.loads((ROOT/'low22_captions/plan.json').read_text())
    ...
    summary=dict(status='full_initializations_partitioned_without_regenerating_low22',observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        old_caption_plan_sha256=sha(ROOT/'low22_captions/plan.json'),old_caption_records_sha256=sha(ROOT/'low22_captions/records.jsonl'),
...
def bind_and_prepare_run():
    ...
    for p,k in [(ROOT/'low22_category.pt','old_category_bank_sha256'),(ROOT/'low22_captions/records.jsonl','old_caption_records_sha256'),(FULL/'all_initializations/plan.json','all_plan_sha256'),(FULL/'remaining_captions/plan.json','remaining_plan_sha256')]:assert sha(p)==partition[k]
    ...
    candidate=module('cat64',Path('/root/autodl-tmp/m64_category_candidate_20260907.py'))
    new=candidate.retain_category(raw,spec['text_protocol_sha256'])
    ...
    wrapper=FULL/'m64_full_semantic_vot.py'
    wrapper.write_text("import sys\nsys.path.insert(0,'"+str(INTERFACE)+"')\nfrom run_semantic_vot import run\nrun('"+str(FULL/'plan.json')+"')\n")
    ini=(f'[{TRACKER}]\nlabel = M64 category-retention full127\nprotocol = traxpython\ncommand = m64_full_semantic_vot\npaths = {FULL}\n'
    ...
    manifest=dict(frozen,schema='m64_same_bundle_full127_v1',tracker=TRACKER,gpu_count=1,shards=shards,
        native_source='Only native dataset/shard metadata reused; all predictions belong to the M64 bundle.')
    ...
    files=[Path(__file__),wrapper,runner,FULL/'category.pt',FULL/'plan.json',run/'shard_manifest.json',FULL/'preseed_receipt.json',ROOT/'bundle.json',ROOT/'text_protocol.json']
...
def analyze():
    ...
    name='m64_category_full127_analysis'
