"""Wait on the existing controller, then collect sealed M88 evidence on CPU."""
import argparse
...
def collect():
    ...
    assert sha(R/'frozen.json')=='473c0e5e83d5a093cfb5dd541ca5de467fe58355c20e5f917ad4391f3b545cbc'
    ...
    assert result['status']=='complete_recursive_development' and result['gate_count']==14
    ...
    def add(path,name):
        ...
        files[name]=path
    root_names=['EXPERIMENT_PLAN.md','RUNTIME.md','queue_line_endings.json','numerical_clarification.json','model_check.json',
        'training_spec.json','recursive_spec.json','frozen.json','integration.json',
        'preflight_result.json','preparation_receipt.json','CODE_REVIEW.md','code_review_receipt.json','launch.json',
        'prepare.py','preflight_m88.py','freeze_m88.py','launch_m88.py',
        'run_m88.sh','run_recursive.py','recursive_metric.py','train_causal.py','causal_training.py','support_loss.py',
    ...
    assert sha(R/'native_parity/category_zero.pth')==training['initial_checkpoint_sha256']['category']
    add(R/'native_parity/category_zero.pth','native_parity/category_zero.pth')
    for name in ['preflight_result.json','preflight.log','preflight.exit','preflight_launch.json','training_spec.json','recursive_spec.json']:
        add(R/'preflight_attempt1'/name,'preflight_attempt1/'+name)
    for name in ['preflight.log','preflight.exit','preflight_launch.json','check_model.py','semantic_spatial_adapter.py']:
        add(R/name,name)
    ...
    control=read(M84/'category_recursive_receipt.json')
    assert sha(M84/'category_recursive_receipt.json')==parent['receipts']['category']
    assert control['status']=='complete' and control['total_frames']==33130 and len(control['sequences'])==22
    add(M84/'category_recursive_receipt.json','references/M84_category_receipt.json')
    for item in control['sequences']:
        name=item['sequence'];p=M84/'recursive/category'/(name+'.json')
        assert sha(p)==item['sha256']
        add(p,'references/M84_category/'+name+'.json')
    ...
    for name in ['COLLECTION_PLAN.md','collect_completed.py','run_collection.sh','launch_collection.py','CODE_REVIEW.md','code_review_receipt.json','launch.json']:add(OUT/name,'collection/'+name)
    ...
    summary=dict(status='sealed_completed_evidence_collected',observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        gates_passed=sum(sum(g.values()) for g in result['gates'].values()),gates_total=14,
...
def wait_and_collect(pid):
    while True:
        ...
        assert str(R/'run_m88.sh') in state,state
