"""Three fixed-content full recursions for one centered Category final checkpoint."""
...
FAMILIES=['category','category_empty','category_swapped']
...
def plans():
    ...
    for a in ['category','empty','swapped']:
...
def run(arm):
    ...
    assert saved['architecture']=='semantic_spatial_local_reference_v1' and saved['status']=='complete'
    assert saved['training_spec_sha256']==spec['training_spec_sha256']
    tracker=STTrackSemantic(params,str(checkpoint))
    content={'category':'category','category_empty':'empty','category_swapped':'swapped'}[arm]
...
def analyze():
    ...
    per={'native':native['per_sequence']['native'],'M84_structure_control':parent['per_sequence']['category'],**{a:{} for a in FAMILIES}}
    ...
    gates=dict(M84_increment=compare(aggregates['M84_structure_control']),native=native_gates,
        swapped_content=compare(aggregates['category_swapped']),empty_native_parity={'all_sequences':all(parity.values())})
    loo={}
    ...
    result=dict(status='complete_recursive_development',observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Repeated DepthTrack Train development22; one trained Category head and three content recursions, not three trained models.',
