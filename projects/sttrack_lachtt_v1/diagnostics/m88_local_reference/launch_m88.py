...
with (R/'controller.log').open('xb') as log:
    process=subprocess.Popen(['bash',str(R/'run_m88.sh')],cwd=str(R),stdin=subprocess.DEVNULL,
...
receipt=dict(status='training_started',observed_utc=datetime.now(timezone.utc).isoformat(),
    ...
    source_sha256=sha(Path(__file__)),queue_sha256=sha(R/'run_m88.sh'),seed=2027,
