"""Check actual M45 tensors against the frozen paired training record on CPU."""
import hashlib
import json
import math
...
root = Path('/root/autodl-tmp/sttrack_m45_default_priority_v1_20260905')
...
assert sha(parent / 'training_result.json') == plan['control_training_result_sha256']
assert sha(parent / 'terminal_audit.json') == plan['control_terminal_audit_sha256']
for name, digest in plan['source_sha256'].items():
    assert sha(Path(spec['repository']) / name) == digest, name
training = json.loads((root / 'geometry_result.json').read_text())
control = json.loads((parent / 'training_result.json').read_text())['variants']['geometry']
checkpoint = root / 'geometry_final.pth'
assert sha(checkpoint) == training['checkpoint_sha256']
saved = torch.load(checkpoint, map_location='cpu')
assert saved['m45_spec_sha256'] == training['m45_spec_sha256'] == sha(root / 'spec.json')
