...
import hashlib,importlib.util,json,sys,time,math
import numpy as np
import torch
...
parity=dict(max_box_difference=max(abs(a-b) for u,v in zip(x,y) for a,b in zip(u['bbox'],v['bbox'])),
    ...
    limits=dict(box=1e-3,score=2e-5,loss=2e-5,parameter=2e-6))
assert parity['max_box_difference']<1e-3 and parity['max_score_difference']<2e-5 and parity['max_loss_difference']<2e-5 and parity['max_parameter_difference']<2e-6,parity
assert parity['search_crops_identical'] and parity['template_writes_identical'],parity
...
result=dict(status='fit_smoke_pass',observed_utc=datetime.now(timezone.utc).isoformat(),checker_sha256=sha(__file__),
    ...
    zero_weight_control_numerical_parity=parity,arms=results,
