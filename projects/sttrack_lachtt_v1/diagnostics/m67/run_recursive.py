...
def trained():
    ...
    for key in ['initial_adapter_state_sha256', 'base_state_before_sha256', 'optimizer_steps', 'total_track_calls']:
        assert results['support'][key] == results['control'][key], key
...
def analyze():
    ...
    predicted = {a:{} for a in ['support','control']}
    receipts = {}
    ...
    per = {'native':native['per_sequence']['native'], 'support':{}, 'control':{}}
    for case in spec['cases']:
    ...
    primary=aggregates['support']; baseline=aggregates['native']; control=aggregates['control']
    broken=[n for n in per['native'] if per['native'][n]['failure_episodes']==0 and per['support'][n]['failure_episodes']>0]
    rule=training['promotion_gates']
    control_broken=[n for n in per['control'] if per['control'][n]['failure_episodes']==0 and per['support'][n]['failure_episodes']>0]
    legacy_broken=[n for n in training['protected_prior_control_sequences'] if per['support'][n]['failure_episodes']>0]
    gates=dict(prior_control_success_protection=not legacy_broken, mean_vs_native=primary['mean_iou']>=baseline['mean_iou']+rule['support_pooled_mean_vs_native_minimum'],
...
if __name__=='__main__':
    ...
    action.add_argument('--arm',choices=['support','control'])
