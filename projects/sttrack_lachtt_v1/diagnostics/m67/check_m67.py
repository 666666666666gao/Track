...
import hashlib,importlib.util,json,sys,time,math,inspect
import numpy as np
import torch
...
assert inspect.getsource(base_supervision).replace('def base_supervision(', 'def supervision(')==inspect.getsource(old.supervision)
...
for a,t in trackers.items():t.support_loss_weight=spec['support_loss_weights'][a]
...
assert all(x==[100] for x in writes.values())
del native
assert sha(folder/'groundtruth.txt')==row['groundtruth_sha256'];gt=np.loadtxt(folder/'groundtruth.txt',delimiter=',').reshape(-1,4)
results={};trajectories={};losses_by_arm={};final={};same_output_checks=0
for a in ['reference','control','support']:
    ...
    for i in range(1,97):
        ...
        if a=='reference':loss,d=old.supervision(t.network,out,gt[i],state['previous_bbox'],state['resize_factor'],256)
        ...
        if a=='control':
            expected,_=old.supervision(t.network,out,gt[i],state['previous_bbox'],state['resize_factor'],256)
            assert (loss is None and expected is None) or (loss is not None and expected is not None and torch.equal(loss,expected))
            same_output_checks+=1
            del expected
...
def crop_key(state):
    x,y,w,h=state['previous_bbox'];side=math.ceil(math.sqrt(w*h)*4.)
    return round(x+.5*w-.5*side),round(y+.5*h-.5*side),side
x=trajectories['reference'];y=trajectories['control']
parity=dict(max_box_difference=max(abs(a-b) for u,v in zip(x,y) for a,b in zip(u['bbox'],v['bbox'])),
    ...
    max_parameter_difference=max(float((u-v).abs().max()) for u,v in zip(trackers['reference'].network.semantic_adapter.state_dict().values(),trackers['control'].network.semantic_adapter.state_dict().values())),
    search_crops_identical=all(crop_key(u)==crop_key(v) for u,v in zip(x,y)),template_writes_identical=all(u['template_write']==v['template_write'] for u,v in zip(x,y)),
    interpretation='Diagnostic independent-optimizer variability; not a bitwise-repeatability guarantee or a development promotion condition')
assert same_output_checks==96
assert final['support']!=final['control']

result=dict(status='fit_smoke_pass',observed_utc=datetime.now(timezone.utc).isoformat(),checker_sha256=sha(__file__),
    prepared_spec_sha256=sha(ROOT/'prepared_training_spec.json'),preparation_sha256=sha(ROOT/'preparation.json'),
    initial_adapter_states=initial,final_smoke_adapter_states=final,zero_residual_native_parity_frames_each=101,
    boxes_scores_queries_templates_exact=True,default_template_writes=writes,
    zero_weight_original_loss_on_same_outputs_exact=True,same_output_loss_checks=same_output_checks,original_localization_loss_source_identical=True,independent_optimizer_variability=parity,arms=results,
    formal_optimizer_steps=0,smoke_weights_saved=False,independent_model_review_pass=False,elapsed_seconds=time.time()-started)
(ROOT/'causal_smoke_result.json').write_text(json.dumps(result,indent=2,allow_nan=False)+'\n');print(json.dumps(result,indent=2),flush=True)
