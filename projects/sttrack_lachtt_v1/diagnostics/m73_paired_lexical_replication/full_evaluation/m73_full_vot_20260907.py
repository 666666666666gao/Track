"""Conditional full VOT preparation preserving the exact M73 low22 inputs."""
import argparse
...
TRACKER='sttrack_m73_category_full127'
...
def eligible():
    p=Path('/root/autodl-tmp/m73_full_preparation_20260907.py')
...
def bind_and_prepare_run():
    ...
    wrapper=FULL/'m73_full_semantic_vot.py'
    wrapper.write_text("import sys\nsys.path.insert(0,'"+str(INTERFACE)+"')\nfrom run_semantic_vot import run\nrun('"+str(FULL/'plan.json')+"')\n")
    ini=(f'[{TRACKER}]\nlabel = M73 category-retention full127\nprotocol = traxpython\ncommand = m73_full_semantic_vot\npaths = {FULL}\n'
    ...
    manifest=dict(frozen,schema='m73_same_bundle_full127_v1',tracker=TRACKER,gpu_count=1,shards=shards,
        native_source='Only native dataset/shard metadata reused; all predictions belong to the M73 bundle.')
...
def analyze():
    ...
    name='m73_category_full127_analysis'
