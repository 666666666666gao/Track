"""Complete-checkpoint and scalar-metric audit for the user-amended single-seed M73 study."""
import argparse
...
def completed():
    ...
    parent = read(ROOT / 'result.json'); assert parent['status'] == 'completed_single_seed_paired_lexical_experiment'
    ...
    passed = all(x['gate_pass'] for x in outputs.values()); assert passed == parent['single_seed_development_gates_pass']
    r = dict(status='completed_M73_single_seed_checkpoint_and_scalar_audit', observed_utc=datetime.now(timezone.utc).isoformat(),
        auditor_sha256=sha(__file__), spec_sha256=SPEC_SHA, frozen_sha256=FROZEN_SHA, parent_result_sha256=sha(ROOT / 'result.json'),
        amendment_sha256=sha(ROOT / 'single_seed_amendment/plan.json'), cancelled_seeds=[2028],
        historical_reference_sha256=sha(OUT / 'reference.json'), seeds=outputs, single_seed_development_gates_pass=passed,
