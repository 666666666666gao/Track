#!/bin/bash
set -u
root=/root/autodl-tmp/sttrack_m73_paired_lexical_replication_20260907/candidate_evaluation
model=/root/autodl-tmp/envs/sttrack/bin/python
metric=/root/miniconda3/envs/mplt/bin/python
builder=/root/autodl-tmp/m73_candidate_evaluation_20260907.py
entry=/root/autodl-tmp/m73_learned_entry_parity_20260907.py
cd "$root" || exit 1
...
queue=Path('/root/autodl-tmp/sttrack_m73_paired_lexical_replication_20260907/single_seed_amendment/controller')
assert (queue/'controller.exit').read_text().strip()=='0'
assert json.loads((queue/'result.json').read_text())['status']=='completed_M73_audit_and_conditional_content'
...
print('M73 training/content queue complete; both GPUs idle; disk exceeds1GB.')
PY
code=$?
printf '%s\n' "$code" > resource_check.exit
if [ "$code" -ne 0 ]; then printf '%s\n' "$code" > controller.exit; exit "$code"; fi
CUDA_VISIBLE_DEVICES=0 "$model" -u /root/autodl-tmp/sttrack_m73_paired_lexical_replication_20260907/candidate_evaluation/interface/run_semantic_ope.py --plan "$root/entry_parity/ope_plan.json" --mode track > ope.log 2>&1 & ope=$!
