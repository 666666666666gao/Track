...
ROOT = Path('/root/autodl-tmp/sttrack_m73_paired_lexical_replication_20260907/candidate_evaluation/entry_parity')
MODEL = Path('/root/autodl-tmp/sttrack_m73_paired_lexical_replication_20260907/seed2027')
CANDIDATE = Path('/root/autodl-tmp/sttrack_m73_paired_lexical_replication_20260907/candidate_evaluation')
...
def candidate():
    path = Path('/root/autodl-tmp/m73_candidate_evaluation_20260907.py')
    spec = importlib.util.spec_from_file_location('m73_evaluation_candidate', str(path))
    module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
...
def prepare():
    frozen = candidate()
    parent = json.loads((MODEL / 'recursive_spec.json').read_text())
    training = json.loads((MODEL / 'training_spec.json').read_text())
    ...
    assert len(cases) == 3
    reference_receipt = json.loads((MODEL / 'category_recursive_receipt.json').read_text())
    ...
    for case in cases:
        ...
        assert sha(MODEL / 'recursive/category' / (case['sequence'] + '.json')) == reference_files[case['sequence']]['sha256']
    ROOT.mkdir()
    ...
    spec = dict(status='frozen_before_real_model_entry_validation', observed_utc=datetime.now(timezone.utc).isoformat(),
                ...
                reference_receipt_sha256=sha(MODEL / 'category_recursive_receipt.json'),
...
def checked():
    ...
    assert sha(MODEL / 'category_recursive_receipt.json') == spec['reference_receipt_sha256']
    ...
    for name, digest in spec['reference_prediction_sha256'].items():
        assert sha(MODEL / 'recursive/category' / (name + '.json')) == digest
...
def client():
    ...
    for case in cases:
        ...
        expected = json.loads((MODEL / 'recursive/category' / (name + '.json')).read_text())['rows']
...
def verify():
    ...
    for case, item in zip(cases, ope['sequences']):
        ...
        reference = json.loads((MODEL / 'recursive/category' / (name + '.json')).read_text())['rows'][:202]
    ...
    result = dict(status='real_learned_OPE_and_TraX_entries_verified', observed_utc=datetime.now(timezone.utc).isoformat(),
                  ...
                  scope='Predetermined M73 seed2027 Category final and category input bank on three sealed Train prefixes. OPE six-decimal files and actual toolkit/TraX wire outputs agree with direct predictions. No official metric, new caption generation, or public benchmark.')
