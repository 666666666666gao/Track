...
class STTrack(nn.Module):
    ...
    def __init__(self, transformer, box_head, cfg, aux_loss=False, head_type="CORNER"):
        ...
        self.fix_query_window = bool(
            getattr(cfg.MODEL.TSG, 'FIX_QUERY_WINDOW', False))
    ...
    def forward(self, template: torch.Tensor,
        ...
        for i in range(len(search)):
            ...
            if self.fix_query_window:
                retained_history = (
                    self.track_beforequery_len - self.track_query_len)
                if retained_history < 0:
                    raise ValueError(
                        'TRACK_QUERY exceeds the fixed query window')
                for modality in range(2):
                    if retained_history == 0:
                        track_query_before[modality] = (
                            track_query_before[modality][:, :0])
                    else:
                        track_query_before[modality] = (
                            track_query_before[modality][
                                :, -retained_history:])
            ...
            if (self.fix_query_window and
                    any(value.size(1) != self.track_beforequery_len
                        for value in track_query_before)):
                raise RuntimeError('fixed query window length drifted')
            ...
            out['backbone_feat'] = x
            if return_candidate_features:
                out['candidate_features'] = {
                    'template_rgb_tokens': temp_x[:, :num_template_token, :],
                    'template_depth_tokens': temp_r[:, :num_template_token, :],
                    'search_rgb_tokens': temp_x[:, -num_search_token:, :],
                    'search_depth_tokens': temp_r[:, -num_search_token:, :],
                    'search_fused_tokens': feat_last,
                    'track_query_rgb': track_query_before[0],
                    'track_query_depth': track_query_before[1],
                    'num_template_tokens': num_template_token,
                    'num_search_tokens': num_search_token,
                }
            out_dict.append(out)
        return out_dict
