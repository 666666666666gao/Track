...
class PairedClips(torch.utils.data.Dataset):
    ...
    def __init__(self, spec, epoch, sample_count):
        ...
        for i, (name, info) in enumerate(zip(self.dataset.sequence_list, self.annotations)):
            count = spec['training_frame_counts'][name]
            if name == 'toy07_indoor_320':
                # Existing project contract: this exact GT has 39 annotation-only tail rows.
                assert count == 1367 and len(info['bbox']) == 1406
                assert spec['fit_gt_sha256'][name] == '683e8ae7ae401b71b8d10e9bb489c3956a150163606f5bac925a911f395444e2'
                info = {key: value[:count] for key, value in info.items()}
                self.annotations[i] = info
            assert len(info['bbox']) == count, name
...
def main():
    ...
    for name, count in spec['training_frame_counts'].items():
        folder = Path(spec['dataset_root']) / name
        assert sorted(p.name for p in (folder / 'color').glob('*.jpg')) == ['%08d.jpg' % i for i in range(1, count + 1)], name
        assert sorted(p.name for p in (folder / 'depth').glob('*.png')) == ['%08d.png' % i for i in range(1, count + 1)], name
