"""Supervise the same Hann-weighted peak competition used by the public tracker.
...
def competition_loss(out, target, output_window):
    ...
    weighted = (score * output_window.reshape(16, 16)).flatten()
    # Center Head already clamps scores to [1e-4,1-1e-4]; the native centered Hann is positive.
    ...
    return loss, dict(competition_loss=float(loss.detach()), competition_negatives=hard.numel(),
        competition_positive_index=int(positive), competition_hann_top1_index=int(weighted.detach().argmax()),
