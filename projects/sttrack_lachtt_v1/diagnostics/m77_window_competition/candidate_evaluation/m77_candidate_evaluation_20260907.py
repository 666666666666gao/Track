"""CPU entry preparation; bind only the predetermined M77 final after content passes."""
import argparse
...
M77 = B / 'sttrack_m77_window_competition_20260907'
MODEL = M77
ROOT = M77 / 'candidate_evaluation'
...
CONTENT = M77 / 'content_followup'
CONTENT_SCRIPT = B / 'm77_content_followup_20260907.py'
CONTENT_SHA = '58e52b047f428857255e56ebd18a966f81f74fcb787217d5d52eebfdc6453a7e'
CONTENT_SPEC_SHA = 'ce95e06d20f14a317b142e8c4719e4593d7762572df83bdbe74e3c1b8d6188cc'
INTERFACE_SHA = '323620eab82b163a3a9e6de54ea8e17da5dbc0cf7336b3717ef443082b59e07b'
SOURCES = ['m77_candidate_evaluation_20260907.py', 'm77_learned_entry_parity_20260907.py', 'm77_vot_low22_20260907.py',
    'm77_candidate_entry_20260907.sh', 'm77_candidate_low22_20260907.sh']
...
def inputs():
    ...
    loader = importlib.util.spec_from_file_location('m77_content_entry_prerequisite', str(CONTENT_SCRIPT))
...
def prepare():
    ...
    assert not (M77 / 'recursive_result.json').exists() and not (MODEL / 'training/category/final.pth').exists()
    ...
    protocol['selection_provenance'] = ('M77 compares category and empty training with the same added Hann competition loss, only seed2027. '
        'The fixed Category final is the only candidate, conditional on all fifteen frozen development checks and all eight same-head content comparisons. '
    ...
    spec = dict(status='M77_candidate_entry_CPU_prepared_before_final_results', observed_utc=now(),
        source_sha256={n:sha(B/n) for n in SOURCES}, training_spec_sha256=sha(MODEL / 'training_spec.json'),
        M77_spec_sha256=sha(M77 / 'spec.json'), M77_frozen_sha256=sha(M77 / 'frozen.json'),
        ...
        gate_provenance='Carry forward the M64/M67/M73 low22 gate unchanged before M77 final outcomes; require all fifteen M77 development gates and all eight existing content comparisons before binding.',
        entry_prefixes=['bag05_indoor','container01_indoor','mobilephone02_indoor'],frames_per_entry_prefix=202,
        required_order=['All fifteen M77 seed2027 development gates and completed audit','All eight fixed Category head content comparisons',
    ...
    result = dict(status='M77_CPU_initialization_routing_and_inherited_interface_verified',observed_utc=now(),
...
def gates():
    ...
    assert audit['status']=='completed_M77_checkpoint_training_coverage_and_independent_scalar_audit'
    assert audit['followup_spec_sha256']==CONTENT_SPEC_SHA
    assert audit['training_spec_sha256']==spec['training_spec_sha256']
    assert len(audit['gates'])==15 and audit['development_gates_pass'] and all(audit['gates'].values())
    primary=read(M77/'recursive_result.json')
    ...
    for name in ['controller','training_category','training_empty','category_recursive','empty_recursive','recursive_analysis']:
        assert (M77/(name+'.exit')).read_text().strip()=='0'
    ...
    assert result['status']=='completed_M77_fixed_Category_head_content_diagnostic'
...
def build():
    ...
    result=dict(status='M77_predetermined_Category_final_bound_after_development_and_content',observed_utc=now(),
        preparation_spec_sha256=sha(ROOT/'spec.json'),bundle_sha256=sha(ROOT/'bundle.json'),
        M77_audit_sha256=sha(CONTENT/'completed_training_audit.json'),content_result_sha256=sha(CONTENT/'result.json'),
...
def checked():
    ...
    assert bound['M77_audit_sha256']==sha(CONTENT/'completed_training_audit.json')
