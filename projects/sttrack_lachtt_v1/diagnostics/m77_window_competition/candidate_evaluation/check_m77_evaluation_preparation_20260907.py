...
B=Path('/root/autodl-tmp');R=B/'sttrack_m77_window_competition_20260907';E=R/'candidate_evaluation'
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
source=B/'m77_full_preparation_20260907.py'
loader=importlib.util.spec_from_file_location('m77_prepared_full_check',str(source))
...
assert spec['training_spec_sha256']=='7162b4c6043d1895097a3cdec79f6373364031e6b100edbd08983f0f8a858f47'
...
result=subprocess.run(['bash',str(B/'m77_full_evaluation_20260907.sh')],env=env,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
...
proof=dict(status='M77_CPU_real_category_transform_and_premature_full_pipeline_rejection_verified',observed_utc=datetime.now(timezone.utc).isoformat(),
