"""One frozen-budget full-sequence training pass per text/visual arm."""
import argparse
...
def main():
    ...
    parser.add_argument('--arm', choices=['text', 'visual'], required=True)
    ...
    assert sha(root / 'native_parity/text_zero.pth') == spec['initial_checkpoint_sha256']
    ...
    tracker.use_text = args.arm == 'text'
    ...
    def checkpoint(complete):
        return dict(architecture='semantic_spatial_v1', model=tracker.network.semantic_adapter.state_dict(),
    ...
    result = dict(status='one_full_causal_fit_pass_complete', arm=args.arm, sequences=len(receipts),
