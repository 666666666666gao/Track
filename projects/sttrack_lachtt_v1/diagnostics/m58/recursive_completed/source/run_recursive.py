...
def trained():
    ...
    for key in ['initial_adapter_state_sha256', 'base_state_before_sha256', 'optimizer_steps', 'total_track_calls']:
        assert results['text'][key] == results['visual'][key], key
...
def run(arm):
    ...
    assert saved['use_text'] == (arm == 'text')
...
def analyze():
    ...
    predicted = {a:{} for a in ['text','visual']}
    receipts = {}
    ...
    per = {'native':native['per_sequence']['native'], 'text':{}, 'visual':{}}
    for case in spec['cases']:
    ...
    primary=aggregates['text']; baseline=aggregates['native']; control=aggregates['visual']
    broken=[n for n in per['native'] if per['native'][n]['failure_episodes']==0 and per['text'][n]['failure_episodes']>0]
    rule=training['promotion_gates']
    gates=dict(mean_vs_native=primary['mean_iou']>=baseline['mean_iou']+rule['text_pooled_mean_vs_native_minimum'],
        mean_vs_visual=primary['mean_iou']>=control['mean_iou']+rule['text_pooled_mean_vs_visual_minimum'],
        low_frames=primary['low_iou_frames']<=baseline['low_iou_frames'],
        H10=primary['failure_episodes']<=baseline['failure_episodes'], native_success_protection=not broken)
    result=dict(status='complete_recursive_development', observed_utc=datetime.now(timezone.utc).isoformat(),
        ...
        new_failure_sequences=broken, receipts=receipts, frozen_native_result_sha256=sha(native_path),
        ...
        independent_review_pass=False, next='Fixed-weight content counterfactuals before low22' if all(gates.values()) else 'Stop this frozen revision; diagnose complete trajectories')
...
if __name__=='__main__':
    ...
    action.add_argument('--arm',choices=['text','visual'])
