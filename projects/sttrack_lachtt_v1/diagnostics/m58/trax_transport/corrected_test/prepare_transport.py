...
def main():
    ...
    from initialization_text import initialization_key, vot_wire_bbox as constructor_only_bbox
    from vot_rectangle_transport import rectangle_wire_bbox, rectangle_wire_text
    ...
    boxes = [[1.123456789, 2.234567891, 30.345678912, 40.456789123],
             [19.234567891, 7.123456789, 20.7654321, 15.4321], [40.1, 9.2, 12.3, 20.4]]
    wire = [rectangle_wire_bbox(box) for box in boxes]
    ...
    old_bank = dict(bank)
    old_bank['keys'] = [initialization_key(images[0]['color_sha256'], constructor_only_bbox(b)) for b in boxes[:2]]
    assert old_bank['keys'][0] != bank['keys'][0]
    torch.save(old_bank, ROOT / 'constructor_only_bank.pt')
    cases = []
    for index, name in enumerate(['first_object', 'second_object_same_image', 'unregistered_object']):
        ...
        for step, score in enumerate([.533012345, .060412345], start=1):
            ...
            predictions.append(dict(bbox=box, wire_bbox=rectangle_wire_bbox(box),
                                    wire_text=rectangle_wire_text(box), score=score))
        cases.append(dict(name=name, raw_bbox=boxes[index], wire_bbox=wire[index],
            wire_text=rectangle_wire_text(boxes[index]),
    ...
    wrong_precision = dict(cases[0])
    wrong_precision.update(name='constructor_only_index', expected_key_index=None,
        bank_path=str(ROOT / 'constructor_only_bank.pt'), bank_sha256=sha(ROOT / 'constructor_only_bank.pt'))
    cases.append(wrong_precision)
    ...
    fixture = dict(scope='Synthetic transport only', images=images, cases=cases,
        ...
        source_sha256={n:sha(ROOT / n) for n in ['prepare_transport.py', 'transport_worker.py', 'check_transport.py', 'vot_rectangle_transport.py']},
        precision_example=dict(raw_bbox=boxes[0], constructor_only_bbox=constructor_only_bbox(boxes[0]),
            actual_wire_text=rectangle_wire_text(boxes[0]), actual_received_bbox=wire[0],
            constructor_only_key=old_bank['keys'][0], corrected_key=bank['keys'][0]),
