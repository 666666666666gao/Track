"""Verify M78 final checkpoints, training coverage and independently recomputed scalars."""
...
def gates(per,agg,parent,t):
    ...
    return dict(mean_vs_native=c['mean_iou']>=n['mean_iou']+rules['category_pooled_mean_vs_native_minimum'],
...
def helpers():
    return (load('m78_checkpoint_verifier','audit_m73_completed_20260907.py','6ea04715e50733a0dd41ca7b458f80c6afbd6af826b01e8865ff4674aa427747'),
        load('m78_independent_scalar','audit_m67_completed_20260907.py','1773887deb3be27aa11a4409aa77bd7581f32b8d891ecab2a03454d277e769bf'))
...
def completed():
    ...
    for arm in ['category','empty']:
        ...
        for v in [json.loads(x) for x in (folder/'sampled_state_trace.jsonl').read_text().splitlines()]:
            if v['label']=='centre_inside':
                ...
                assert 0<=v['competition_raw_top1_index']<256 and 0<=v['competition_positive_probability']<=1
    ...
    assert len(g)==10 and g==result['gates'] and all(g.values())==result['primary_pass']
    harms=[]
    ...
    output=dict(status='completed_M78_checkpoint_training_coverage_and_independent_scalar_audit',observed_utc=datetime.now(timezone.utc).isoformat(),auditor_sha256=sha(__file__),
