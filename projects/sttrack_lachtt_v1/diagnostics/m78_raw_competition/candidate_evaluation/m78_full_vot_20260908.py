"""Conditional full VOT preparation preserving the exact M78 low22 inputs."""
import argparse
...
TRACKER='sttrack_m78_category_full127'
...
def eligible():
    p=Path('/root/autodl-tmp/m78_full_preparation_20260908.py')
...
def bind_and_prepare_run():
    ...
    wrapper=FULL/'m78_full_semantic_vot.py'
    wrapper.write_text("import sys\nsys.path.insert(0,'"+str(INTERFACE)+"')\nfrom run_semantic_vot import run\nrun('"+str(FULL/'plan.json')+"')\n")
    ini=(f'[{TRACKER}]\nlabel = M78 category-retention full127\nprotocol = traxpython\ncommand = m78_full_semantic_vot\npaths = {FULL}\n'
    ...
    manifest=dict(frozen,schema='m78_same_bundle_full127_v1',tracker=TRACKER,gpu_count=1,shards=shards,
        native_source='Only native dataset/shard metadata reused; all predictions belong to the M78 bundle.')
...
def analyze():
    ...
    name='m78_category_full127_analysis'
