...
ROOT = Path('/root/autodl-tmp/sttrack_m78_raw_competition_20260908/candidate_evaluation/entry_parity')
MODEL = Path('/root/autodl-tmp/sttrack_m78_raw_competition_20260908')
CANDIDATE = Path('/root/autodl-tmp/sttrack_m78_raw_competition_20260908/candidate_evaluation')
...
def candidate():
    path = Path('/root/autodl-tmp/m78_candidate_evaluation_20260908.py')
    spec = importlib.util.spec_from_file_location('m78_evaluation_candidate', str(path))
    module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
...
def verify():
    ...
    result = dict(status='real_learned_OPE_and_TraX_entries_verified', observed_utc=datetime.now(timezone.utc).isoformat(),
                  ...
                  scope='Predetermined M78 seed2027 Category final and category input bank on three sealed Train prefixes. OPE six-decimal files and actual toolkit/TraX wire outputs agree with direct predictions. No official metric, new caption generation, or public benchmark.')
