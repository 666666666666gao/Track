"""CPU entry preparation; bind only the predetermined M78 final after content passes."""
import argparse
...
M78 = B / 'sttrack_m78_raw_competition_20260908'
MODEL = M78
ROOT = M78 / 'candidate_evaluation'
...
CONTENT = M78 / 'content_followup'
CONTENT_SCRIPT = B / 'm78_content_followup_20260908.py'
CONTENT_SHA = 'af967f2978ed92058f2801842435058cb8dbef1d70a65186436457dfaae8bb20'
CONTENT_SPEC_SHA = 'e6216f7dd2a1d8ec8f24ee7a4c77a4a07e5c86b0cc03742581990d2c4df5da5d'
INTERFACE_SHA = '323620eab82b163a3a9e6de54ea8e17da5dbc0cf7336b3717ef443082b59e07b'
SOURCES = ['m78_candidate_evaluation_20260908.py', 'm78_learned_entry_parity_20260908.py', 'm78_vot_low22_20260908.py',
    'm78_candidate_entry_20260908.sh', 'm78_candidate_low22_20260908.sh']
...
def inputs():
    ...
    loader = importlib.util.spec_from_file_location('m78_content_entry_prerequisite', str(CONTENT_SCRIPT))
    module = importlib.util.module_from_spec(loader); loader.loader.exec_module(module)
    _, training, _ = module.plans()
    ...
    assert sha(MODEL / 'integration.json') == training['integration_sha256'] == '5881ef1594929f0e71b0f7e8da6564e1c4669cbef86b1986b2948d7e0f231a65'
...
def prepare():
    ...
    assert not (M78 / 'recursive_result.json').exists() and not (MODEL / 'training/category/final.pth').exists()
    ...
    protocol['selection_provenance'] = ('M78 compares category and empty training with the same added raw-score competition loss, only seed2027. '
        'The fixed Category final is the only candidate, conditional on all ten frozen development checks and all eight same-head content comparisons. '
        'Category retention is used during both training and deployment. No additional seed or checkpoint selection.')
    ...
    spec = dict(status='M78_candidate_entry_CPU_prepared_before_final_results', observed_utc=now(),
        source_sha256={n:sha(B/n) for n in SOURCES}, training_spec_sha256=sha(MODEL / 'training_spec.json'),
        M78_spec_sha256=sha(M78 / 'spec.json'), M78_frozen_sha256=sha(M78 / 'frozen.json'),
        ...
        final_head=str(MODEL/'training/category/final.pth'), candidate_seed=2027, additional_seeds=[],
        ...
        gate_provenance='Retain native low22 primary thresholds prospectively for M78; historical model dominance is not required; require all ten M78 development gates and all eight existing content comparisons before binding.',
        entry_prefixes=['bag05_indoor','container01_indoor','mobilephone02_indoor'],frames_per_entry_prefix=202,
        required_order=['All ten M78 seed2027 development gates and completed audit','All eight fixed Category head content comparisons',
    ...
    result = dict(status='M78_CPU_initialization_routing_and_inherited_interface_verified',observed_utc=now(),
...
def gates():
    ...
    _,_,_,activation,audit=module.checked()
    assert audit['status']=='completed_M78_checkpoint_training_coverage_and_independent_scalar_audit'
    assert audit['followup_spec_sha256']==CONTENT_SPEC_SHA
    assert audit['training_spec_sha256']==spec['training_spec_sha256']
    assert len(audit['gates'])==10 and audit['development_gates_pass'] and all(audit['gates'].values())
    primary=read(M78/'recursive_result.json')
    assert primary['gates']==audit['gates'] and primary['primary_pass']
    for name in ['controller','training_category','training_empty','category_recursive','empty_recursive','recursive_analysis']:
        assert (M78/(name+'.exit')).read_text().strip()=='0'
    for name in ['training_audit','prefix','empty','swapped','content_analysis','controller']:
        assert (CONTENT/(name+'.exit')).read_text().strip()=='0'
    stage=read(CONTENT/'stage.json'); result=read(CONTENT/'result.json')
    assert stage['stage']=='complete_content_diagnostic' and stage['result_sha256']==sha(CONTENT/'result.json')
    assert result['status']=='completed_M78_fixed_Category_head_content_diagnostic'
    assert result['source_sha256']==CONTENT_SHA and result['spec_sha256']==CONTENT_SPEC_SHA
    assert result['activation_sha256']==sha(CONTENT/'activation.json')
    assert result['seed']==2027 and result['additional_seeds']==[] and result['new_optimizer_steps']==0
    assert result['independent_scalar_recomputation'] and result['development_gate_pass']
    criteria=result['descriptive_criteria']
    assert set(criteria)=={'empty','swapped'} and all(set(v)=={'pooled','macro','low','H10'} for v in criteria.values())
    assert result['descriptive_criteria_pass'] and all(v for g in criteria.values() for v in g.values())
    assert sha(spec['final_head'])==result['head_sha256']==activation['head_sha256']==audit['training']['category']['head_sha256']
    assert result['receipts']['category']==sha(MODEL/'category_recursive_receipt.json')
    for name in ['empty','swapped']:
        assert result['receipts'][name]==sha(CONTENT/name/'receipt.json')
...
def build():
    ...
    result=dict(status='M78_predetermined_Category_final_bound_after_development_and_content',observed_utc=now(),
        preparation_spec_sha256=sha(ROOT/'spec.json'),bundle_sha256=sha(ROOT/'bundle.json'),
        M78_audit_sha256=sha(CONTENT/'completed_training_audit.json'),content_result_sha256=sha(CONTENT/'result.json'),
...
def checked():
    ...
    assert bound['M78_audit_sha256']==sha(CONTENT/'completed_training_audit.json')
