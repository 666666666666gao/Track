...
ROOT = Path('/root/autodl-tmp/sttrack_m62_learned_entry_parity_20260907')
...
def prepare():
    assert (M60 / 'job.exit').read_text().strip() == '0'
    assert sha(M60 / 'result.json') == 'febddd93eda9f25f3530377948751c1999030db9a08656f9bb24ae151ad64287'
    result = json.loads((M60 / 'result.json').read_text())
    assert result['descriptive_category_margin_pass']
    sys.path.insert(0, str(M59))
    from run_controls import parent_ready
    parent, training, _, _ = parent_ready()
    ...
    assert len(cases) == 3
    reference_receipt = json.loads((M59 / 'category_receipt.json').read_text())
    ...
    for case in cases:
        ...
        assert sha(M59 / 'predictions/category' / (case['sequence'] + '.json')) == reference_files[case['sequence']]['sha256']
    ROOT.mkdir()
    ...
    spec = dict(status='frozen_before_real_model_entry_validation', observed_utc=datetime.now(timezone.utc).isoformat(),
                ...
                reference_receipt_sha256=sha(M59 / 'category_receipt.json'),
...
def checked():
    ...
    for name, digest in spec['reference_prediction_sha256'].items():
        assert sha(M59 / 'predictions/category' / (name + '.json')) == digest
...
def client():
    ...
    for case in cases:
        ...
        expected = json.loads((M59 / 'predictions/category' / (name + '.json')).read_text())['rows']
...
def verify():
    ...
    for case, item in zip(cases, ope['sequences']):
        ...
        reference = json.loads((M59 / 'predictions/category' / (name + '.json')).read_text())['rows'][:202]
    ...
    result = dict(status='real_learned_OPE_and_TraX_entries_verified', observed_utc=datetime.now(timezone.utc).isoformat(),
                  ...
                  scope='Fixed category-retention weight/input bank on three sealed Train prefixes. OPE six-decimal files and actual toolkit/TraX wire outputs agree with direct predictions. No official metric, new caption generation, or public benchmark.')
