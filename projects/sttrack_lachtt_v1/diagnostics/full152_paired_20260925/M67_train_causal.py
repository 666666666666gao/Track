"""Matched Null-architecture training; original loss versus explicit box-level support supervision."""
import argparse
...
def main():
    ...
    parser.add_argument('--arm', choices=['control', 'support'], required=True)
    args = parser.parse_args()
    root = Path('/root/autodl-tmp/sttrack_full152_paired_20260925/M67')
        ...
        for seq_index, row in enumerate(spec['sequence_order']):
            ...
            for frame_index in range(1, n):
                ...
                loss, diagnostic = supervision(tracker.network, out, gt[frame_index],
                    state['previous_bbox'], state['resize_factor'], 256, support_weight=spec['support_loss_weights'][args.arm])
