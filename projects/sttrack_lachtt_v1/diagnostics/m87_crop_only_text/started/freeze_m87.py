...
assert (R/'preflight.exit').read_text().strip()=='0' and pre['status']=='M87_native_parity_and_causal_smoke_pass'
assert pre['training_spec_sha256']==sha(R/'training_spec.json') and pre['source_sha256']==sha(R/'preflight_m87.py')
...
assert spec['run_queue_sha256']==rec['queue_sha256']==sha(R/'run_m87.sh')
assert sha(R/'EXPERIMENT_PLAN.md')==spec['plan_sha256']
subprocess.run(['bash','-n',str(R/'run_m87.sh')],check=True)
