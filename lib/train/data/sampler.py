...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, hard_classes=None, hard_class_prob=0.0):
        """
        args:
        ...
        self.hard_classes = {str(c).strip().lower() for c in (hard_classes or []) if str(c).strip()}
        self.hard_class_prob = min(max(float(hard_class_prob), 0.0), 1.0)
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
        ...
        if len(valid_ids) == 0:
            return None
        if num_ids > 1:
            sampled_ids = random.choices(valid_ids, k=1)
            for i in range(num_ids-1):
                sampled_ids.append(sampled_ids[0]-i-1)
        else:
            sampled_ids = random.choices(valid_ids, k=num_ids)
        return sampled_ids
        ...
        while not valid:
            ...
            try:
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'language_category': meta_obj_test.get('language_category', meta_obj_test.get('object_class_name', '')),
                                   'language_description': meta_obj_test.get('language_description', ''),
                                   'language_appearance': meta_obj_test.get('language_appearance', ''),
                                   'language_depth_relation': meta_obj_test.get('language_depth_relation', ''),
                                   'language_depth_quality': meta_obj_test.get('language_depth_quality', ''),
                                   'language_occlusion_state': meta_obj_test.get('language_occlusion_state', ''),
                                   'language_distractor_relation': meta_obj_test.get('language_distractor_relation', '')})
                ...
                data_vi['visible'] = TensorDict({'template_images': data['template_images'][:,:3,:,:],
                                                 ...
                                                 'language_category': data['language_category'],
                                                 'language_description': data['language_description'],
                                                 'language_appearance': data['language_appearance'],
                                                 'language_depth_relation': data['language_depth_relation'],
                                                 'language_depth_quality': data['language_depth_quality'],
                                                 'language_occlusion_state': data['language_occlusion_state'],
                                                 'language_distractor_relation': data['language_distractor_relation'],
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        hard_seq_ids = None
        if self.hard_classes and is_video_dataset and getattr(dataset, 'has_class_info', lambda: False)():
            hard_seq_ids = []
            for class_name in getattr(dataset, 'get_class_list', lambda: [])():
                if str(class_name).strip().lower() in self.hard_classes:
                    hard_seq_ids.extend(dataset.get_sequences_in_class(class_name))
            if not hard_seq_ids:
                hard_seq_ids = None
        while not enough_visible_frames:
            # Sample a sequence
            if hard_seq_ids is not None and random.random() < self.hard_class_prob:
                seq_id = random.choice(hard_seq_ids)
            else:
