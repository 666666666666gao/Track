...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        if loader.training:
            self._keep_frozen_batchnorm_eval()
        ...
        for i, data in enumerate(loader, 1):
            ...
            if self.move_data_to_gpu:
                if data['visible']:
                    data['visible'] = data['visible'].to(self.device)
                    data['infrared'] = data['infrared'].to(self.device)
                else:
            ...
            save_interval_steps = int(getattr(self.settings, "save_interval_steps", 0))
            if loader.training and save_interval_steps > 0 and i % save_interval_steps == 0:
                if self._checkpoint_dir and self.settings.local_rank in [-1, 0]:
                    self.save_checkpoint(tag="ep{:04d}_it{:05d}".format(self.epoch, i))
    ...
    def _keep_frozen_batchnorm_eval(self):
        """Prevent frozen BatchNorm layers from updating running statistics during partial fine-tuning."""
        net = getattr(self.actor, "net", self.actor)
        module = net.module if hasattr(net, "module") else net
        freeze_all_bn = bool(getattr(self.settings, "freeze_bn_stats", False))
        for layer in module.modules():
            if isinstance(layer, nn.modules.batchnorm._BatchNorm):
                has_trainable_param = any(p.requires_grad for p in layer.parameters())
                if freeze_all_bn or not has_trainable_param:
                    layer.eval()
