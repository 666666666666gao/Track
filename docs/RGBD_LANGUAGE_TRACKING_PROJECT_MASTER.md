...
### 5.32 M41完整终态与模板/在线位置文本pilot结果（2026-09-05）

#### 5.32.1 M41回答了“有框选错还是无框可选”

全部124个起点、31,834帧已完成，9个factor7独立shadow也全部完成。模型/配置/裁剪/解码源文件及checkpoint后验哈希全部未变，124个事件的导出/shadow均未改写主状态。全体轨迹对M39的最大框误差为0.00009156px；33事件保留R1检查，91事件通过R3真实TraX序列化逐帧精确相等检查。R3共记录6处保存坐标重建crop的舍入边界差异。

“存在正确候选”固定定义为解码框与GT的连续xywh IoU>=0.5；这是候选几何诊断，与正式VOT raster overlap指标分开记账。GT在候选封存后才用于此统计。表中的top-K是oracle容量，绝不能写成部署已经救回K条轨迹。

| 失败起点集合 | 事件数 | Hann top1正确 | Hann top5含正确框 | Hann top10含正确框 | raw top10含正确框 | 全部256格含正确框 |
|---|---:|---:|---:|---:|---:|---:|
| 全部factor4 | 124 | 0 | 89 | 91 | 92 | 97 |
| 目标在factor4内 | 115 | 0 | 89 | 91 | 92 | 97 |
| 目标在factor4外，仍用factor4 | 9 | 0 | 0 | 0 | 0 | 0 |
| 九个框外事件的factor7 shadow | 9 | 1 | 1 | 1 | 5 | 7 |

115个框内失败中，91/115=79.13%的Hann top10已经有正确框，89/115=77.39%在top5里就有，97/115=84.35%在全部256格中有。首次正确候选排名分布为rank2:78、rank3:7、rank4:2、rank5:2、rank6:1、rank7:1。因此正确候选大多只排在第二名，主线应转到实例关联和局部时序对应，不能再声称主要是整体搜索范围小或完全没有正确候选。

#### 5.32.2 逐序列容量与剩余生成问题

| 序列 | 失败起点 | Hann top5可恢复 | Hann top10可恢复 | raw top10可恢复 | dense256可恢复 |
|---|---:|---:|---:|---:|---:|
| `ball06_indoor_2` | 7 | 7 | 7 | 7 | 7 |
| `bandlight_indoor_1` | 5 | 1 | 1 | 2 | 3 |
| `cube02_indoor_1` | 7 | 1 | 1 | 1 | 1 |
| `cube05_indoor_1` | 2 | 2 | 2 | 2 | 2 |
| `cube05_indoor_4` | 4 | 4 | 4 | 4 | 4 |
| `cube05_indoor_5` | 9 | 6 | 6 | 7 | 9 |
| `cube05_indoor_6` | 1 | 1 | 1 | 1 | 1 |
| `cup02_indoor_1` | 36 | 30 | 32 | 31 | 33 |
| `earphone01_indoor_1` | 2 | 1 | 1 | 1 | 1 |
| `humans_shirts_room_occ_1_A_2` | 6 | 6 | 6 | 6 | 6 |
| `shoes02_indoor_1` | 12 | 10 | 10 | 10 | 10 |
| `shoes02_indoor_2` | 2 | 2 | 2 | 2 | 2 |
| `toy09_indoor_1` | 26 | 16 | 16 | 16 | 16 |
| `two_tennis_balls_3` | 3 | 0 | 0 | 0 | 0 |
| `yogurt_indoor_1` | 2 | 2 | 2 | 2 | 2 |

核心四条共83个失败，Hann top10覆盖64个：cup02 32/36、toy09 16/26、shoes02_1 10/12、cube05_5 6/9。不能把四条合为一个失效机制：toy09剩余10个连dense256都达不到0.5IoU，确实有候选/回归容量不足；cube05_5则dense9/9而top10仅6/9，主要还存在响应排序/抑制损失；cup02仅3个dense仍无高IoU框。

保持top1中心、把宽高后验换成GT宽高，全部124个依然0个达到0.5IoU；这再次不支持统一框尺度修正。115框内事件仅3个是raw top1正确、Hann top1错误（ball06两个、cube05_6一个），所以关闭Hann也不是主要问题的普遍解法。

九个框外事件中，factor4完全没有可恢复框；factor7确实提供新容量，但Hann top10只1/9，raw top10为5/9，dense为7/9。四个事件的raw top1正确却被Hann改成错误峰，表明扩大crop后仍沿用强中心先验会压制远处目标。此结果只支持未来独立宽搜索shadow内研究响应排序，不授权全局取消Hann或直接提交宽搜索结果。

单帧存在正确候选不证明该分支未来H10安全，也不证明可部署selector能选对。必须回到DepthTrack Train训练与真实递归验证，而不能按这124个GT起点写规则。

#### 5.32.3 模板与在线相对位置文字的实际结果

V2全部24个固定窗口×3臂=72次本地生成已完成，约323秒。输入含首帧目标、可选默认动态模板、带STTrack预测框的紧邻前一帧和未标注的当前帧；当前及上一帧GT均未进入模型。首版归一化/时间角色语义失败结果保留在v1目录，V2没有读取v1结果来调定位门。

| V2条件 | 窗口 | 有效接受 | 正确IoU>=.5 | 错误IoU<=.1 | 非法坐标输出 | 原坏状态被救回 | 原好状态被破坏 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 首帧参考+稳定身份 | 24 | 14 | 4 | 4 | 1 | 0 | 1 |
| 首帧及动态模板+稳定身份 | 24 | 11 | 1 | 6 | 0 | 0 | 2 |
| 同两模板+在线相对位置 | 24 | 0 | 0 | 0 | 0 | 0 | 0 |

原STTrack在这些窗口中有10/24个IoU>=.5。首帧VLM定位的4个正确框并未救回原IoU<=.1状态；加入动态模板反而更差；相对位置臂24/24弃权，正确定位净增益/至少两序列改善门未通过。只在源代码末端分析中记录了一条真实非法输出：shoes02@350B:onset给出y2=362，而图像高360。它被计为非法建议，不截断修成“正确框”，也未重跑VLM。

这批24个参考中19个已有动态写入；全部动态框在写入时对GT的IoU>0.5，最低0.5174，其余主要0.82—0.93。因此该pilot中的VLM退化不能归因于“给它的动态模板已经完全换成错误目标”。当前Qwen2.5-VL-3B能产生类别或部分同类计数，却不能可靠保证实例身份及在线顺序对应。位置文字仍可作为将来训练模块的辅助假设，但本轮没有支持其直接充当定位/提交裁判。

这里是GT时点抽样的小型因果信息试验，非在线触发性能、非正式low22/全127跟踪结果。没有产生新checkpoint、没有改STTrack轨迹、没有训练主模型、没有任何正式指标更新，也不据此把所有语言方案判为无效。

#### 5.32.4 下一项最小结构与训练要求

M41现在为候选关联提供了实际容量证据。下一项应只新增一种信息：保留候选自身4×4 RGB/depth局部token，并与初始化/最近可信模板及紧邻上一帧预测目标的局部token进行空间对应，而不是立即mean成单向量。保持当前top10、backbone、回归、默认模板更新和protected路径，先与同数据/同参数规模的全局池化对照，不能同时改搜索范围、文本、阈值和模板时机。

必须在DepthTrack Train生成新的预测裁剪训练观测，使用预测的前一帧身份作为输入、GT只作监督。按序列隔离并明确哪些开发折已被历史实验使用，不能把旧折说成全新未见。候选关联需要none/无目标监督及原路径正确的保护窗口，再验证H4及更长真实递归的收益和新增失败。若改变神经模块，依照用户要求重新训练并输出明确绑定的新权重；通过low22相对M39 default工作门后，用同一权重验证DepthTrack、CDTB、full127。

在线位置文字这次没有通过信息增量门，暂不全量重注释或接入当前STTrack；下一次语言试验应在经过训练的候选局部关联模块中比较真实/空/同类错属性/打乱文本，而不是重复让3B模型凭全景描述直接重写框。宽搜索只保留为九个crop-out对应的次级shadow问题。

训练准备还发现：原官方`lib/train/admin/local.py`仍指向作者的`/nasdata/...`及`/home/hxt/...`目录，不能直接调用官方train.sh假称训练已启动；已有H4工程脚本可显式使用本机`/root/autodl-tmp/depthtrack`数据路径。当前数据盘剩余约7.7GB，后续只保留必要最好/最后权重，训练数据尽量复用或在线读取。本轮未删除已有权重。

M41与文本pilot已全部终止，GPU任务清空。项目目标仍未达到，当前没有同一新权重的三数据集完成证据。

证据索引：

- M41 `/root/autodl-tmp/sttrack_m41_candidate_capacity_v1_20260905/result.json` — SHA256 `a2ddf3e4226b7639a5ced657584c35b4eedae5325fcc86093486520b4a8c7a81`
- M41 `/root/autodl-tmp/sttrack_m41_candidate_capacity_v1_20260905/terminal_audit.json` — SHA256 `2b4b1a36d1031a1b12a4510b8f5755ddae17be1aa7d69b8f117cdc01d14779fe`
- M41 `/root/autodl-tmp/sttrack_m41_candidate_capacity_v1_20260905/factor4_diagnosis.csv` — SHA256 `c7d89a35b6d72e5db96de8a33b51e310c052170df8918fca2a5e56223f2c81a4`
- M41 `/root/autodl-tmp/sttrack_m41_candidate_capacity_v1_20260905/factor7_diagnosis.csv` — SHA256 `32c62de53816811f7923935591c3e1cc849173f014abcfcb1bc14e6f9a87d498`
- VLM-v2 `/root/autodl-tmp/sttrack_position_text_pilot_v2_20260905/spec.json` — SHA256 `8a00ea1a3638c59dc74235fe4cca755bd90d540762f2f45c550528df146e97ed`
- VLM-v2 `/root/autodl-tmp/sttrack_position_text_pilot_v2_20260905/responses.jsonl` — SHA256 `263176ab271ef628a6ba34600f52b9f50cf1b5964248e62675e23e63ccfaff10`
- VLM-v2 `/root/autodl-tmp/sttrack_position_text_pilot_v2_20260905/result.json` — SHA256 `4f30998bdcd0d3241e8ef893ce517e042ec1c6f7e7bbf15555739600cd232751`
- VLM-v2 `/root/autodl-tmp/sttrack_position_text_pilot_v2_20260905/paired_diagnosis.json` — SHA256 `6811657c4803ffbdd0f3fa43572b36d019959595dc5eb3283bf6cb7fce4c55e0`


### 5.33 M42局部空间关联的DepthTrack训练启动（2026-09-05）

承接5.32的候选容量和位置文本负结果，本轮只检验候选自身4×4 RGB/depth局部信息是否比立即平均池化提供更多实例区分能力。保留STTrack default主干、回归、query窗口、factor4、Hann、NMS top10及默认50帧/0.75模板适应。当前语言关闭；不叠加宽搜索、回归校正或新文本，以免无法判断新增信息的贡献。

**架构**：每个候选输入2模态×16格×768维；参考为首帧模板、最近一次默认动态模板、紧邻前一帧预测ROI。模板特征在其第一次因果forward使用时封存，动态图像实际变化后再更新对应参考；前帧参考跟随预测框，不读取GT。最新默认模板不是额外经过oracle验证的“可信模板”。共享768→16投影；每个参考/模态使用4×4配对特征，经1024→32→8网络得到关系向量；六个关系向量与8个响应/几何scalar共同形成56→32→1候选残差，再加原log Hann响应。集合均值/最大值形成112→32→1的NONE输出；NONE保持原默认框。空间臂与平均池化后重复16格的对照均为50,858个可训练参数。仅训练新增关联模块，已有backbone/回归权重冻结。

**数据**：现有M18 ledger中的85条DepthTrack Train序列，fold2/3/4共63条拟合、fold5共22条开发验证。这些折已有历史实验使用，明确不能宣称全新未见。fold0/1未打开；本轮不读VOT/CDTB/DepthTrack Test。每序列最多均匀选8个原IoU>=.5健康帧、8个原IoU<=.1困难帧及2个不可见/无有效GT帧，从第10帧至倒数第5帧。它们是分层训练窗口，不全是首次失锁起点，比例也不代表部署分布。GT只用于训练抽样和封存后的标签，推理清单只有初始化框及原预测路径。

冻结清单共1,375个窗口；两片分别63,065/63,402帧（合计126,467，包含85个初始化帧），分片40/45条序列。新token缓存FP16，训练转FP32，预计小于1GB。准备阶段发现toy07_indoor_320只有1,367对图像及轨迹，却有1,406行标注，因此初次严格等长检查终止；核对全部图像/轨迹编号后，按实际1,367帧采样，未使用39行尾部标注并记录在spec中。这不是重跑失败训练或用缺失图像填补观测。

| 阶段 | 状态 | 本轮新增证据 |
|---|---|---|
| ROI/模型工程检查 | PASS | patch中心对齐、默认输出保持、候选置换、空间与池化信息区分、有限梯度及一步优化 |
| 原路径冒烟重放 | PASS | 两条拟合序列共120帧、6个窗口；bbox及置信度差均为0；两次真实动态模板写入 |
| 双GPU特征采集 | 已启动 | screen sttrack_m42_train_20260905；GPU0/1 PID8097/8098；逐帧bbox<=1e-4px及置信度<=1e-6检查 |
| 关联头训练 | 排在采集完成后自动执行 | 空间/池化各20轮；未在启动记录中声称已经完成 |
| 递归/low22/三数据集 | 尚未启动 | 当前无新正式指标，无新三数据集权重通过证据 |

**优化与停止条件**：两臂同seed2026、同batch32及样本顺序、AdamW lr0.001、weight_decay0.0001、梯度裁剪5、固定20轮，只存最后权重。存在IoU>=.5候选时监督最高IoU者，否则监督NONE；不做开发集选epoch、类别重加权或阈值扫描。信息门要求空间臂选框mean IoU同时超过池化和default、救回原坏窗口多于破坏原好窗口、至少3条开发序列有正平均增益。三项全部满足才允许下一步预测裁剪递归验证，单帧门不直接授权公开评测。失败则保留负结果，不能用调低门槛或追加epoch制造提升。

**后续顺序**：两片采集成功并封存→DepthTrack Train拟合新增模块→同折独立状态的真实递归对照（保留健康窗口、因果模板/query/参考银行）→通过后冻结low22相对M39 default评测→明显改善后用同一个新权重分别评估DepthTrack Test、CDTB、full127。旧正式指标仅引用5.31/5.32和前文表，不重复登记或混合模型。

运行根目录为`/root/autodl-tmp/sttrack_m42_local_spatial_v1_20260905`，程序位于STTrack运行仓库tools与两项新增lib模块。按启动吞吐估计采集约一小时，之后才是两组小头的实际拟合。控制器等待两个采集进程退出并确认成功，不会把半成品缓存拿去训练；没有附加自动公开测试。一次SSH源文件传输在启动前被连接重置，核实无控制器/采集进程后重新传输两文件，仅启动上述一个screen，未重复发起GPU任务。

完整实验架构/协议、执行计划、状态表见运行目录及GitHub diagnostics/m42中的EXPERIMENT_SPEC.md、EXPERIMENT_PLAN.md、EXPERIMENT_TRACKER.md。本节为启动事实；终态以training_result.json及后续追加为准，项目总目标仍未完成。

本轮启动证据哈希：

- `spec.json` — SHA256 `1b7daa3672fd688021018fadfc6e702c7f5bf9551a97594fea5fdf6351c8d1c9`
- `launch.json` — SHA256 `8898c74d221600ae3c83e85fe3ed25ae119c13e99c56d704205de3493d57bf14`
- `smoke_receipt.json` — SHA256 `5dad0b7658b2a5d5abccc87b345987c152cdc6ff6e611d4c7230567544d73868`


### 5.34 M42递归入口保真与预先冻结的后续比较（2026-09-05）

M42正式采集/训练协议不变。在尚无训练结果时，已实现并冻结真实递归入口和后验分析器。新入口在两条拟合序列上与普通STTrack同时重放120帧，逐帧bbox、置信度、模板tensor及RGB/depth query全部精确相等。另有两次强制选择rank2的软件探针，确认改选时读取对应峰自身size/offset和自身置信度。这是零关联训练更新的工程验证，不是救回结果。

接入训练头后，每个独立分支自己推进搜索框、query、模板和参考银行。模板规则保持50帧/0.75；若关联头改选，使用被选峰自己的置信度决定写入，避免把原top1的分数用于另一个框。选择0及NONE时完全保留原默认路径。输入特征按原缓存的FP16→FP32方式量化，两个臂使用相同处理。

后续screen `sttrack_m42_recursive_gate`只每240秒检查一次原控制器终态；两个采集片和两臂训练全部成功、静态信息三门全部通过后，才在GPU0/1分别运行空间/池化臂的22条fold5完整可用图像序列。每条只使用正常首帧初始化，之后不读GT。普通default比较来自绑定哈希的既有full152完整轨迹，不用新模型的结果替换历史基线。

GT仅由两个完整轨迹封存后的分析器读取。固定代理为连续xywh IoU、IoU<=.1严重帧数、连续10个有效低IoU帧的失败段；初始化和无有效GT帧不计入均值，无效GT间隔截断连续段。分母及失败段长度已用确定性样例检查通过。它们是Train开发代理，不能写成VOT ROB或DepthTrack Test官方分数。

递归门在训练结果产生之前固定：空间臂mean IoU同时超过default和池化；严重低IoU帧数减少；持续失败段数不增加；至少3条序列mean IoU正增益；原default零失败段的序列不得新增持续失败。五项全部满足才允许冻结后续low22对照。本控制器不附加任何自动公开测试。若静态门失败，写入零GPU递归启动的终止记录。若递归门失败，保留证据并停止该冻结方案，不调阈值或继续加epoch。

正式权重身份须同时绑定官方base checkpoint、新训练的association-final权重、运行源文件及配置。当前仍没有新的low22或三数据集正式指标。本节补充的是可执行的后续训练验证路径，不更新旧指标表。

- `recursive_spec.json` — SHA256 `90904f96187ec8507de73f2ab91fca341067c74fc5b4857113f6542c73a0867c`

- `recursive_smoke_receipt.json` — SHA256 `e5d3191708736550ea611caa25659800a576629d16fa6b190cb087694c9815e2`


### 5.35 M42训练终态与M43池化对照的递归性能验证（2026-09-05）

**M42完成了真实训练，但“局部空间结构提供独立增益”的主假设未通过。** 原门保持失败，没有把空间臂与对照打平改写成成功。另一方面，两个已训练的头相对default均出现相同的单帧收益，值得作为另一个性能问题验证；下文把M42结果和M43新的探索性递归比较分开记录。

#### 5.35.1 训练及完整性

全部85条序列的1,375个窗口已封存，实际重放126,382帧，另有85个正常初始化；与旧默认轨迹的bbox及置信度最大差均为0。缓存879,632,094 bytes，实际默认动态图像更新计数1,420；该计数未做M34的“合法写入/坏写入”标签分类，不与其直接相减。模型、配置及原始checkpoint哈希始终不变。

63条序列共1,000个窗口拟合，22条既有开发序列共375个窗口评估；每臂20轮、640次优化器更新，共1,280次真实优化。两臂均50,858参数，全部13个参数tensor相对固定种子初始化发生变化。空间训练损失2.001483→0.555147，池化1.995757→0.535672。最终checkpoint严格加载后logits精确一致；最终数据、源文件及权重审计PASS。这里只训练新增关联头，STTrack backbone/框头保持冻结；未声称全量解冻训练。

#### 5.35.2 本轮新增开发窗口结果

表中是连续xywh IoU及窗口数量，**不是DepthTrack Test正式PR/F-score，也不是VOT的EAO/ACC/ROB**。“救回”指原IoU<=.1而新IoU>=.5；“新增严重错误”指原IoU>=.5而新IoU<=.1。

| 路径 | 拟合窗口mean IoU | 开发窗口mean IoU | 开发IoU>=.5 | 改选窗口 | 救回 | 新增严重错误 | NONE | mean IoU改善序列 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| STTrack default | 0.443016 | 0.414883 | 176/375 | 0 | — | — | — | — |
| 保留4×4空间头 | 0.456185 | 0.435760 | 186/375 | 12 | 10 | 0 | 184 | 6 |
| 平均池化对照头 | 0.455952 | 0.435760 | 186/375 | 12 | 10 | 0 | 191 | 6 |

两臂在全部375个窗口的最终候选选择完全相同，mean IoU均比default增加0.0208764（约2.08764个百分点）。它们的NONE数量不同，但NONE与选择0都会保持default，因此最终选择一致。空间臂未超过池化臂，主信息门失败；救回多于严重破坏、至少3序列改善这两项通过。原M42递归控制器据此写入零GPU任务的停止记录，没有偷偷执行失败分支。

12次改选不能描述成全部正确：其中10次救回，分布于cup08、cup10、egg、flower02、flowerbasket、ghostmask共6条开发序列；cup13@839候选与原框的IoU均为0；notebook02@1930的IoU从0.688793略降到0.685323，仍然正确。零新增严重错误不等于零伤害，更不等于递归安全。

候选容量分组（来自封存后的开发窗口）：

| 分组 | 窗口数 | 池化头最终IoU>=.5 | 池化头改选 |
|---|---:|---:|---:|
| 原框已正确 | 176 | 176 | 1 |
| 原框未达.5，候选中也无正确框 | 141 | 0 | 1 |
| GT无有效可见框 | 42 | 0 | 0 |
| 原框未达.5，但存在正确候选 | 16 | 10 | 10 |

静态样本为健康/困难/不可见分层均匀抽样，并不全在首次失锁边界；负结果只约束这套固定数据、表征、头及监督方案，不能证明所有局部对应都无效。反过来，375个窗口中救回10个，也不能推导完整递归或正式ROB已经改善。

“fold0/1未打开”的旧简写在此明确其范围：准备器解析了包含full152原预测的两个混合JSON，再筛选85条使用；fold0/1未贡献本轮GT监督、native特征、拟合样本或评估结果。不能把这一事实写成从未解析过包含它们的文件。所有本轮开发折均有历史使用，不能称为全新未见。

#### 5.35.3 已训练权重与新的M43问题

M42权重留在同一运行目录：

- `spatial_final.pth` — SHA256 `88e37f12f4645a68a09cef135785113bbb85d9b07655728a2a1357192eb10c9a`；需与既有官方STTrack base共同加载。

- `pooled_final.pth` — SHA256 `70908682c8049c1b444fe55949fbe5bb66ddbf9c9895375a017f205c87a73cca`；需与既有官方STTrack base共同加载。


M43不修补M42的失败门，也不改变权重、epoch或推理阈值。它单独问：**已训练的平均池化对照能否在完整递归中改善default？** 在看到任何新的递归结果之前，固定池化为主候选，空间臂仅作辅助诊断，不允许事后挑较好的臂替换主候选。这是使用已见开发序列的探索性后续，不是新的盲测或空间信息创新证明。

M43覆盖同22条Train开发序列的全部可用图像，每臂33,130帧，两臂66,260帧；各自维护完整bbox/query/模板/参考银行。GT只在两份轨迹封存后的分析中读取。保持既定五个性能条件：主候选mean IoU超过default、严重帧减少、持续失败段不增加、至少3条序列均值正收益、default零失败段序列不新增持续失败。这里没有新的优化器步骤、模块变更或阈值搜索。主候选通过后才能冻结low22相对M39 default的评测，未附加自动公开测试。

M43位于`/root/autodl-tmp/sttrack_m43_pooled_recursive_v1_20260905`，screen `sttrack_m43_recursive_20260905`已启动。旧正式三数据集指标没有更新，仍需通过递归、low22，再用同一新权重验证DepthTrack Test/CDTB/full127。

#### 5.35.4 评测入口及参考实现核对

新STTrack不能直接使用绑定SRTrack类的旧DepthTrack/CDTB启动参数。已核对独立PR评测器、六位小数输出、初始化confidence=1的现有规则；DepthTrack完整覆盖50条/76,373帧，CDTB为80条/101,956帧。VOT低22清单303 anchors/220,483估计帧；全127清单1,765 anchors/1,327,004估计帧，完整任务必须按实测吞吐安排长等待。详见diagnostics/m42/EVALUATION_ENTRY_AUDIT.md，本次只是源代码和清单核对，没有增加公开推理或分数。

另外核对了KeepTrack官方候选关联实现，作为后续“维持实例身份后描述当前顺序”的参考；其跨帧候选集合关联并非M42已实现的三参考单帧打分。未引入其代码或权重，也没有在看到M42结果前后更改本轮超参数。来源及本项目推断见KEEPTRACK_SOURCE_NOTE.md；不能把其他RGB跟踪基准的收益冒充RGB-D结果。

终态与新启动证据：

- `/root/autodl-tmp/sttrack_m42_local_spatial_v1_20260905/training_result.json` — SHA256 `86ed72e3159848c0606f3b02cc1a997b9df21dddb861968fbdb5b309d9c125ae`

- `/root/autodl-tmp/sttrack_m42_local_spatial_v1_20260905/terminal_audit.json` — SHA256 `dfc1ebe3a64c9bfeb9665e62b33fb901adfd78f817e187e5e73f1bfc23e6b0ab`

- `/root/autodl-tmp/sttrack_m42_local_spatial_v1_20260905/recursive_gate_receipt.json` — SHA256 `1f9134dfbc8fd787fee8d89b422ff53ad97d223a52ce07bbfcf0a7376758b29a`

- `/root/autodl-tmp/sttrack_m43_pooled_recursive_v1_20260905/spec.json` — SHA256 `d31840ac56d869e30ce49332cf76594df1df1b8352dff9ffff409e308be09c14`

- `/root/autodl-tmp/sttrack_m43_pooled_recursive_v1_20260905/launch.json` — SHA256 `1932f2c829fcd0ef692427f5a75f6337443f11ea2c8d33950164184f3ef42f45`


### 5.36 M43完整递归终态与本地相对位置文本补充核对（2026-09-05）

**M43主候选性能门：FAIL。** 两个已训练的头分别完成22条Train开发序列、33,130帧（含初始化），共66,260帧。模型、配置和权重哈希均保持冻结；这轮没有新的训练，也没有更换主候选。池化是预先指定的主候选，空间结果保持辅助诊断性质。M42局部信息优越性失败的原结论不变。

以下仍是DepthTrack Train开发代理指标，非公开DepthTrack Test/CDTB PR-F，也非VOT三指标。初始化和无有效GT框的帧排除；持续失败段需至少10个连续有效帧IoU<=.1，无效GT会断开该段。
...
池化相对default的mean IoU变化为-0.035162（-3.5162个百分点）；严重帧变化+1156，持续失败段变化+2。
...
| 持续失败段不增加 | 未通过 | 通过 |
| 至少3序列均值改善 | 通过 | 通过 |
| 默认零失败段序列不新增持续失败 | 通过 | 通过 |

池化有7条序列均值改善；默认零失败段序列中新增持续失败：无。

空间有9条序列均值改善；默认零失败段序列中新增持续失败：无。

全部44份轨迹的文件哈希、帧索引、初始化、数值合法性已核对；独立逐帧IoU和失败段重算与原分析一致。每条序列第一次非默认改选之前，bbox与default缓存的最大差严格为0，因此第一次改选可按相同递归状态比较；后续动作已在不同状态上发生，不能继续假定候选和default共享同一裁剪。

首个改选的描述性统计（GT仅作封存后的诊断，不用作触发）：

| 路径 | 出现改选的序列 | 首次改选救回（<=.1到>=.5） | 首次改选严重破坏（>=.5到<=.1） |
|---|---:|---:|---:|
| 池化 | 18 | 0 | 2 |
| 空间 | 17 | 1 | 1 |

完整22序列三路径代理指标及改选数见`diagnostics/m43/per_sequence.csv`；第一次改选帧与随后10个观测帧收益见`terminal_audit.json`。

主候选未通过上述Train递归门，这个冻结版本停止晋升，不启动低22或全量评测。单帧窗口收益没有充分转化为满足既定条件的递归收益；不能通过降低门槛、临时换成空间臂或只报告改善序列来晋升。后续新假设应以真实递归轨迹中的首次错误及候选容量为依据。

#### 5.36.1 相对位置文本：弃权与原始框不是一回事

对已封存的24次Qwen相对位置回答补做字段和原始框核对，没有再次调用VLM。24次都返回target_visible=false、identity_uncertain=true、稳定身份为空；其中16次却同时给出有效范围内的非空框，14次给出相似物体数量，0次给出左右名次。按照原协议，它们仍全部弃权。

单独诊断16个未被接受的原始框：3个IoU>=.5、9个IoU<=.1、4个介于二者之间。说明不能把全弃权直接写成没有任何定位信息，但忽略这些不确定性字段也没有可靠依据；它会同时接受更多严重错误，而且本轮没有产生可验证的“第几个”描述。原信息门、零轨迹提交和未扩正式评测的结论保持不变。原始字段互相矛盾的具体机制未证明，不能自动归因于模板污染或代码过滤过严。

终态证据：

- `diagnostics/m43/result.json` — SHA256 `6c1dc000be8c78ad9a0e4bc7b3c777decf3fcecc154867778084176cf6374a29`

- `diagnostics/m43/terminal_audit.json` — SHA256 `5520b19d8b14cadff3be5ca71e533f63e0160ee7f96b471c3bffdfc18c756f88`

- `diagnostics/m43/per_sequence.csv` — SHA256 `764daf6f840a7d50a973ec041454a6db9db69071fba0c28447cb86a5e27b978a`

- `diagnostics/position_text_v2/abstained_box_diagnosis.json` — SHA256 `c52c6db41cacee621ee03cdfeac6a549b63bfd7c039d5fb8fa36dec183b770c9`


#### 5.36.2 已核实的相邻实例误选：egg_indoor

池化路径最主要的净退化集中于egg_indoor：序列mean IoU从0.734778降到0.366590，严重帧523→1,983，持续失败段9→17。它的第一次非默认改选在0-based帧42（图像00000043.jpg）；此前完整bbox前缀与default严格相同，当帧default IoU为0.835957，而新头选了候选1，IoU为0，随后10个有效观测帧平均差为−0.382828。

空间头在同序列也退化，mean IoU为0.567484，严重帧1,152，失败段12。第一次非默认改选在0-based帧680（图像00000681.jpg），default IoU为0.822139，新框IoU为0。查看这两张原始图像及坐标可见：目标均是前排中央鸡蛋，池化新框覆盖左侧相邻鸡蛋，空间新框覆盖右侧相邻鸡蛋。因此这两个事件已有“正确候选存在却选择相邻实例”的直接证据；不能再归为无正确框可选或框尺度的统一偏差。

两次改选的原始响应置信度分别为0.201712和0.171726；没有依据称为高置信身份切换。帧42和680均不是50帧模板写入点，首次错误由候选选择直接引入，后续模板/query影响仍需另行分解，不能把整条轨迹的全部损失都归因于这一个动作。
...
#### 5.37.4 已完成拟合序列的候选对数据核验（采集仍在运行）

对当前已封存的25条拟合序列/645组候选对做了CPU检查，没有模型训练或开发集性能评测。每个文件均核对采集receipt的SHA；当前/前帧RoI、两个模板、20个坐标及response张量形状正确；previous_frame严格等于frame−1。候选0与其对应默认框的最大数值差为0.000061035px，属于原生解码的浮点差，未改动默认输出。

| 拟合数据类别 | 候选对数 |
|---|---:|
| 两帧均存在IoU>=.5目标候选 |345 |
| 仅一侧存在目标候选，需要unmatched监督 |65 |
| 两侧均无正确候选，不提供目标对应损失 |235 |
| 当前默认框IoU<=.1，但有正确替代候选 |19 |

前三行互斥并合计645；最后一行是其中的可改选子集。它证明已完成拟合数据中存在真实对应、未匹配及可改选监督，不能当作训练后救回、全数据容量或递归改善。共享Train标签JSON被解析，但只有已完成拟合序列的行参与本次计算；没有读取开发特征或开发模型评测结果。模型、采样、epoch、门槛及已绑定源码保持不变，原采集器和等待控制器继续运行。

证据：`diagnostics/m44/fit_pair_data_audit_partial.json`，SHA256 `ee24fe1572ca46a6b6335720194194ead4ffd32828b93365c60fdcbbdb41eeeb`。这里只记录本次新增数据核验，不重复历史正式指标。


#### 5.37.5 终态结果核验入口已准备，未改变活动实验

新增 `tools/audit_sttrack_m44.py`，在原控制器完成之后单独运行。它核对两臂最终权重、20个epoch/960步、拟合与开发序列隔离、采集特征SHA、训练与运行源码绑定；对全部22序列的两路递归轨迹复用M43已核验的独立标量IoU/H10统计，再核对聚合值和原晋升门。还检查每条轨迹第一次改选之前与default逐框完全相同，以定位真正的首个状态分叉。

这是终态核验工具的准备状态，尚无 `terminal_audit.json`，不能记为训练完成或检查通过。未修改原采集器、模型、训练器、递归运行器、控制器、采样及门槛；不会提前使用GT参与跟踪。该工具不属于原训练输入，新增源码SHA为 `5f6a670fd52999d3e0f84f3538331345fd8b6910400d0d3f401cc62c87d6219f`。复用独立计算实现不等于第二位评审者独立审查。


### 5.38 M44训练完成，完整递归验证正在运行（2026-09-05）

本轮两组448,739参数的候选集合网络已完成DepthTrack Train训练：每组使用1,511组拟合候选对、20个epoch、960次更新，两个最终权重分别保留。STTrack主干、框回归头及默认模板更新不变。初始化参数及样本顺序两组一致；训练器严格重载最终权重，590个开发窗口的logits逐值一致；每组46个参数张量发生变化。这里不能再把运行状态写成“仅采集、没有新权重”。

完整采集为85条序列、126,382个跟踪帧及85次正常初始化、2,101组候选对、2,273,860,306字节特征。默认bbox/score最大误差均为0，发生1,420次实际动态模板写入。120帧运行检查通过默认bbox/score和模板/query张量一致性；两个强制改选帧通过候选自身bbox/confidence以及前帧选中索引传递检查。这是已运行的接口验证，不能扩大成所有输入下的数学等价证明。

#### 5.38.1 固定最终epoch的静态结果

| DepthTrack Train静态分割 | 路径 | mean IoU | 改选窗口 | 严重默认错误救回 | 正确默认框变严重错误 |
|---|---|---:|---:|---:|---:|
| 1,511拟合窗口 |default |0.489168 |— |— |— |
| 1,511拟合窗口 |geometry |0.518949 |188 |28 |0 |
| 1,511拟合窗口 |appearance |0.517412 |228 |29 |1 |
| 590既有开发窗口 |default |0.440274 |— |— |— |
| 590既有开发窗口 |geometry（预先指定主路径） |0.441423 |43 |4 |3 |
| 590既有开发窗口 |appearance（坐标置零对照） |0.438411 |85 |4 |4 |

救回定义为default IoU<=.1、所选IoU>=.5；严重新增错误定义为default>=.5、所选<=.1。这些是窗口数量，不是VOT失败anchor或ROB。geometry开发mean IoU比default仅+0.114954个百分点，比appearance+0.301230个百分点，尚不足以称为明显改善或位置语言创新收益。两组损失分别从2.917995降至0.272082、从2.967047降至0.330653，不能仅凭拟合损失或静态均值宣称持续跟踪提升。

#### 5.38.2 已封存静态严重错误的对应检查

主路径3个新增严重窗口错误均是`egg_indoor@46/@779/@2439`，当前和前帧最佳目标候选均为0，网络最终却选择候选1，当前IoU分别从0.698304/0.811461/0.863486降为0。对这3个窗口以及appearance的同类错误做CPU检查，不改变运行中的递归轨迹。geometry从“上一帧已选候选0”出发的对应列argmax分别是unmatched/候选0/候选1，appearance三次均是候选1。因此不能笼统归因成“匹配正确，只是最后排序错误”；也没有证据支持立即用对应argmax替代整个提交路径。

三个正确当前框的中心相对默认前帧框均移动不足1px，候选1的中心更远。该局部证据指出已训练网络仍会压低正确连续实例的分数，但没有证明所有失败的统一根因，也没有改变损失、位置归一化、门槛或训练轮数。检查仅定位已封存静态反例，不是对递归最终结果的提前评判。

#### 5.38.3 正在执行与下一步

原控制器已于13:32:07启动两组完整递归：geometry PID15301、appearance PID15302，各22条既有开发序列/33,130帧，独立维护完整状态。按第一条889帧约54秒估算，每组约34分钟；最终指标与5.37冻结门在两臂封存后计算。随后执行5.37.5的终态核验。只有geometry主递归门通过，才冻结并执行low22相对M39 default的正式比较；低22明显改善后再用同一官方主干+同一新关联权重完成DepthTrack Test、CDTB、VOT full127。没有自动全量评测或新正式数据集指标。

证据与权重绑定：

- `training_result.json`：`7f6df58956ea6a9ad1cf018fb9763d26c14137bb412e11f89292ba60e78a5266`；两组完整窗口记录与20轮损失均保留。
- `geometry_final.pth`：`9ef0caf6e6a3a09d46771e9ea3d7073cfc0c0195f66979051220da97e517d978`。
- `appearance_final.pth`：`623e9a7e9f9c912e2ccb5e70238dde2805380d54519dd8adea8b930dddb2d07c`。
- `runtime_smoke.json`：`a7827e590f247c6dcd71c445ead3d956e35c57d4de4b88bcb88c0b0a37091023`。
- `egg_correspondence_diagnosis.json`：`ad4e7d21ce92d5d66c159e4c0794ddf7179f6af9db26a264807e43896b531794`。
- 原spec与training_binding保持5.37的SHA。权重与特征仅留服务器，GitHub发布源码和诊断记录。


### 5.39 M44完整递归负结果与首次错误重放（2026-09-05）

两组均完成22条既有DepthTrack Train开发序列，每组33,130帧，共66,260帧；各有28,897个纳入统计的有效帧。终态核验通过全部特征/权重/源码绑定、实际20轮960步、全部44条轨迹SHA及索引、独立标量IoU/H10重算，以及所有首次改选之前的default逐框一致性。原进程已正常结束，两组均未获准进入low22。

| 同一完整递归集合 | mean IoU | IoU<=.1帧 | 连续至少10帧失败段 | 均值改善序列 |
|---|---:|---:|---:|---:|
| STTrack default |0.652226 |7,397 |75 |— |
| M44 geometry主路径 |0.617098 |8,436 |87 |11/22 |
| M44 appearance对照 |0.640547 |7,466 |85 |9/22 |

主路径mean IoU下降3.512833个百分点，严重帧新增1,039、失败段新增12；位置版比同架构坐标置零对照低2.344925个百分点。主性能与显式位置增量均失败。主路径保护了原零失败段序列，但该局部通过不能抵消总性能退化；appearance还在原零失败段的mobilephone02新增失败。上述均为Train开发代理指标，不是VOT EAO/ACC/ROB。

| geometry主要变化 | mean IoU变化（百分点） | 严重帧变化 | 失败段变化 |
|---|---:|---:|---:|
| egg_indoor |−19.582722 |+783 |+9 |
| glass03_indoor |−58.804072 |+358 |+2 |
| mobilephone01_indoor |−13.340054 |+183 |+1 |
| beautifullight01_indoor |−14.229811 |+150 |+3 |
| book06_indoor |+11.708681 |−229 |0 |

完整22序列表保留于`diagnostics/m44/per_sequence.csv`，不能只列改善序列。首次错误检查预先选择按IoU总和损失最大的egg和glass03，从正常初始化完整重放118帧，bbox与score均逐帧吻合封存轨迹；GT在推理重放结束后才打开。

- egg第16帧（0-based）：正确候选0的IoU为0.817124，却选候选1并降为0。原始RGB/Depth对前帧目标的相似度也都更支持正确候选；学习后的身份与对应分数均支持错误候选。这是同状态下真实改选错误，早于本轮轨迹分叉。
- glass03第102帧：10个候选最大IoU仅0.152926，属于本轮正确框定义下的无有效候选；网络仍选候选9，IoU从default的0.125571降到0.089668。整个序列geometry只有这一次非默认改选，随后均返回默认候选或NONE，但整段mean IoU从0.757594降到0.169553。回到默认决策不等于恢复原轨迹。对应分支此帧偏向unmatched，身份输出却接受候选；这与egg的错误对应是不同情形。

两次首次改选都不在模板写入帧，不能直接归因为当帧模板污染。3个既有静态egg窗口的原始局部特征检查、首条bag05出现49次非默认前帧选择的输入覆盖检查，以及KeepTrack官方候选对应/增强监督/身份集合实现对照，见`ASSOCIATION_DIAGNOSIS.md`。这些证据限定于各自范围，不能据此直接替换全局选择规则或认定唯一根因。

M44结果SHA：`721553e2d8e2f37b084ca946ea0d0285ccf8436d203a9f0251851c2669a8d200`；终态核验SHA：`c01a9579be0df14270f336da75c4ce2a0a3e0423890fea3fc646fe25afce7c85`。其训练权重保持5.38的SHA并封存。本轮没有新的正式数据集指标，没有low22/full127启动。

### 5.40 M45单项监督目标对照已训练，完整递归验证中（2026-09-05）

训练标签审计发现，M44的1,511拟合窗口中有762个default IoU>=.5；其中121个、覆盖52条拟合序列，仍以更高即时IoU的其他候选为训练目标。这121次的oracle即时IoU增量中位数为0.029482，85次增量低于0.05。它说明原目标同时鼓励正确轨迹中的框贴合改选，不能证明这是全部失败的唯一原因。

M45只改变当前与前帧的身份监督目标：default IoU>=.5时使用候选0；否则仍使用IoU最高且>=.5的候选；全部不达标则为NONE。0.5沿用原正确框定义，只用于DepthTrack Train监督，不是新增推理门槛。网络仍为M44 geometry，默认模板、query、搜索、11路argmax推理均保持原实现。当前拟合标签改变121个；前帧改变174个。候选的原始IoU及评测方法不变。

重新从seed2026初始化拟合63条Train序列/1,511组观测，20轮960步；没有在M44失败权重上追加epoch。实际初始化参数、样本顺序哈希和优化步数与封存M44 geometry对照一致，直接复用该已核验对照结果。原网络和数据绑定保持不变，新训练器与运行器在首次优化前冻结。新权重严格重载后590个开发窗口logits一致。

| 同一590静态开发窗口 | mean IoU | 改选窗口 | 严重错误救回 | 严重新增错误 |
|---|---:|---:|---:|---:|
| STTrack default |0.440274 |— |— |— |
| M44 geometry原目标 |0.441423 |43 |4 |3 |
| M45保留正确default的目标 |0.446766 |15 |5 |1 |

该静态结果仍不足以晋升；匹配正确率的监督定义也改变，不能直接把匹配准确率差值解释为表征提升。整体性能继续与STTrack default比较，而不是只要求超过较差的M44。

两张GPU上的完整递归已于14:27左右启动：PID17379/17380，每片16,567/16,563帧，总计22序列33,130帧。管线PID17291，screen为`sttrack_m45_default_priority_20260905`。预计约17分钟，终态先重算完整轨迹和原default基线，再判断既定mean IoU至少+1pp、严重帧减少、失败段不增加、至少3条改善且保护原零失败段序列的门。只有通过才进入M39 default低22对照；低22明显改善后才用同一权重完成三数据集验证。

实验目录：`/root/autodl-tmp/sttrack_m45_default_priority_v1_20260905`。M45 spec SHA：`0cf2fbd6ef521312b5bade3a52f7dbfb5c1c713571ff3e8a3b6d2e11cdd6c93e`；最终关联权重SHA：`853a25fbc3c9ef12ab54442c30b27bab75f0b1fadcc9bcc82cbec6e8700ed59c`；训练结果SHA：`e2647ea8d9738632d93f99d0108e638025b1c38894f8560cc9a532eb4f13f39d`。权重要求同时使用原官方STTrack主干，不能与其他历史模型的正式指标混用。自身策略预测裁剪与更完整候选对应监督仍是后续可独立检验的方向，本轮的实际干预只有上述训练目标。


### 5.41 M45 完整递归终态：平均重叠有收益，但失败段和成功序列保护未通过（2026-09-05）

M45 的真实训练与全部 22 条 Train 开发序列递归现已完成。两份轨迹各 16,567/16,563 帧，退出码均为 0；固定权重、63 条拟合序列、现有 22 条开发序列、训练标签规则和晋升门均保持 §5.40 的预注册设置。这里是训练集开发诊断，**不是 VOT low22，也不是三个数据集的完整指标**。

| 新结果 | 有效帧数 | 平均 IoU | 相对 STTrack default | 低 IoU 帧数 | 持续失败段 | 序列均值改善数 | 原成功序列新增失败 |
|---|---:|---:|---:|---:|---:|---:|---|
| M45 default-priority，完整递归 | 28,897 | 0.6841023742783606 | +0.0318761111021168 | 6,188（−1,209） | 80（+5） | 11/22 | mobilephone02_indoor |

M45 相对同初始化、同架构、同数据与优化设置的 M44 geometry，平均 IoU 提高 0.0670044370436743；其单项干预是监督保留已经正确的默认候选。相对更强的 STTrack default，平均 IoU 提高 3.187611 个百分点，但持续失败段增加且成功序列受到破坏。因此原有晋升门仍判定 **FAIL**，没有启动正式 low22 或全量评测。这不是已经测得 VOT ROB 下降：此处 H10 失败段与 VOT multi-start ROB 并非同一指标。

收益分布也不均匀。colacan01 的平均 IoU 提高约 55.72 个百分点，减少 1,870 个低 IoU 帧，对净收益贡献很大。egg 的平均 IoU 下降 18.53 个百分点，低 IoU 帧 523→1,262、失败段 9→21；mobilephone02 的平均 IoU 下降 28.64 个百分点，低 IoU 帧 1→205、失败段 0→1。全部 22 条结果和首次改选记录完整保留在 diagnostics/m45/per_sequence.csv 与 recursive_result.json，不能只挑正例。

**完整性与汇总修正。**独立 CPU 权重检查重建了 seed2026 的初始参数，严格加载最终文件，确认 448,739 个参数、46 组张量更新、20 epoch/960 次优化以及与 M44 对照相同的初始化和样本顺序；源码、官方基础权重及配置摘要一致。首次汇总的原 controller.exit=1 保留：错误发生在 JSON 序列化，标量 IoU 例程返回的低重叠帧计数为 NumPy 整数，比较后产生 NumPy 布尔值。独立副本 analyze_m45_sealed.py 仅将当前及默认的两个 low_iou_frames 计数显式转成 int，保留原始冻结脚本及 spec，未加通用 JSON fallback。修正后的独立汇总退出码为 0，全部轨迹 SHA、帧号、框有效性、初始框、首次改选前默认轨迹、标量 IoU/H10 及固定晋升门核对通过。未重跑训练或完整跟踪，也未更改任何输出框。

| 封存产物 | SHA-256 |
|---|---|
| M45 recursive_result.json | a3dd821916ce08a8964bb2df85b9da277f9262d41a1e42c32f0fdde85eed3317 |
| M45 weight_audit.json | 52b8e0ef01b878d9e31af54b19876c0916bfc7a2533db897cf944153fdcb2ffe |
| 修正计数类型的分析源码 | 909b2c805963eaf85bd4973a3790767dd836ee9ac0d33f20c572f3af9e9c0d00 |
| shard0 轨迹 receipt | ac39e09dc1b9279dac3f9fd8019b7f3ab46d5eb8a6540952182a36ab6c87dbe2 |
| shard1 轨迹 receipt | 8dbfdd69624dcd6e32e470d8f6bcaf9668412060481ce87f5728d1833c0c181e |

**失败起点重放。**按完整结果中 IoU 总和损失最大的两条序列预先选定 egg 与 mobilephone02，共精确重放 68 帧，逐帧框和置信度与封存轨迹完全一致；GT 仅在重放完成后读取。以下帧号均从 0 起算。

| 序列/首次改选帧 | 默认→所选 IoU | 默认→所选响应 | 原生 RGB 对上一候选余弦：默认/所选 | 当帧模板写入 |
|---|---:|---:|---:|---|
| egg / 6 | 0.905881→0 | 0.642181→0.117178 | 0.982655 / 0.957366 | 否 |
| mobilephone02 / 62 | 0.566209→0.473642 | 0.643763→0.039902 | 0.958333 / 0.950529 | 否 |

egg 存在正确候选却选了邻近鸡蛋；mobilephone02 的首次改选是定位变差，不能直接称为换了另一个手机实例。两例的 learned identity 与 matching 都偏向所选错误候选，原生 RGB/Depth 与上一默认候选的相似度则略偏向正确默认。这些是具体输入的诊断，不能据此直接上线余弦阈值或断言所有失败均由投影层造成。

**下一项最小实验。**当前关联头从帧号 2 开始决策，但 M44/M45 的全部拟合记录最早为帧号 10，帧号 2–9 完全没有训练样本；每条拟合序列虽有帧号 10 样本，仍不能覆盖此前的递归状态。拟议 M46 仅补充 63 条 DepthTrack Train 拟合序列的帧号 2–9，使用真实 STTrack default 预测裁剪与当时的完整 query/模板状态，保持网络、推理、默认更新和总 960 次优化预算。其假设仅为“覆盖推理已经使用但拟合遗漏的初始化阶段”；不能保证修复帧号 62 或后续身份问题。M45 失败权重不晋升，不通过屏蔽前几帧或修改门槛制造通过。M46 仍需先冻结数据与训练规则，再训练并完整递归验证。


### 5.42 M46 补充初始化阶段输入：真实训练完成，完整递归验证运行中（2026-09-05）

针对 §5.41 的实际覆盖缺口，M46 已在拟合前冻结，只增加原 63 条 DepthTrack Train 拟合序列帧号 2–9 的 504 个候选对。新增部分使用 STTrack default 的真实预测裁剪、原生 RGB/Depth 候选 RoI，以及当时的 query/模板状态；没有加入开发序列或测试序列拟合样本。原 1,511 个拟合对与 590 个开发对复用现有封存缓存，不重复采集。

新增采集现已完成：63 条序列、567 个跟踪帧（另有 63 次初始化）、504 对、545,606,694 字节；全部框和置信度与既有默认轨迹误差为 0。帧号 2–9 不包含默认模板写入点，因此这部分模板写入次数为 0，不能据此声称禁用了默认更新。原收集器未读取独立的训练标签文件。两份 receipt SHA 为 56c4a1d8273694d5f616b50fc046844fce9bbad2273a0b24518ac4bbfd176e8d / 12a32cc0d0e631cf3a8b0715661ca82fc48e88c4f8132691292c381e70ec0171。

网络仍为 448,739 参数的候选集合关联头，官方 STTrack 主干保持冻结并作为模型组合的一部分。M45 的 default-priority 监督、loss、推理 argmax/NONE、search、query 和默认模板更新不变。训练使用相同 seed2026 新初始化、AdamW 常数学习率 0.0003、weight decay 0.01、batch32、梯度裁剪 1；每一轮从扩充后的 2,015 对拟合池做无放回随机排列，取前 1,511 对，20 轮共 960 次优化。**这里是 20 个采样优化轮，不是对 2,015 对做完整 20 epoch。**实际覆盖全部 2,015 个输入，包括全部 504 个新增对。与 M45 对照相同的是初始化、网络、优化设置和更新预算，样本顺序随数据池干预改变，不能称为完全相同。

| 新结果（静态 Train 开发诊断） | 事件数 | 平均 IoU | 正确候选数 | 改选数 | NONE | 严重救回/新增严重退化 | 正收益序列 |
|---|---:|---:|---:|---:|---:|---:|---:|
| M46 固定最终权重 | 590 | 0.445511132478714 | 278（默认 268） | 68 | 195 | 8 / 4 | 10 |

该静态结果没有明显优于 M45，不能作为晋升证据。按照拟合前冻结的计划，仍完成全部 22 条递归，原 STTrack-default 晋升门不变；不依据静态结果选择其他 epoch 或调整阈值。

**报告阶段错误与权重审计。**原训练已经完成全部 960 次优化、保存最终权重，并通过原训练内严格重载比较，随后在组织报告时失败：源码中用于样本顺序 SHA 的 digest 变量被末尾源码摘要循环覆盖成字符串。原 training.exit=1 和 controller.exit=1 保留。独立 finalize_m46_training.py 只重建数据/采样元数据、读取 20 行真实优化日志并评价已保存权重，源码中没有 optimizer.step、backward 或 torch.save；最终文件 SHA 前后完全一致，新增优化步骤为 0。补全报告退出码 0。随后独立 CPU 审计重建初始参数并核对实际文件，确认 46 组参数张量更新、448,739 参数、960 步、全部权重有限、严格加载、基础模型和冻结源文件一致。

| 封存产物 | SHA-256 |
|---|---|
| M46 spec.json | 1e0fd942361b20c16eae377398a11f5e88c68ef3069fec3bdd72dcf497a01987 |
| M46 新增采集 spec | 87bb8997675d73c2c79e9b36bd7183969bfa8804650ba6591fdd8f969b868b6f |
| M46 geometry_final.pth | 10977f0259d246f679351fb976c99c07763c7f1f63d3ebe49a085dc640987153 |
| M46 geometry_result.json | 65e76de916d1984309215c1336f7315dff226e1a1b13efdab6d44c322cdbe5cf |
| M46 weight_audit.json | aea402057f7b064afacd6e1f25d1121c417d91d52c435fceb7d1aaa88c68b9a1 |
| 无优化步骤的报告补全源码 | ccd54da087ed4b6a1814ec2b6e65375a1520080b112053867d51287373e5c4eb |

**当前运行状态。**补全报告和权重审计后，于 2026-09-05 15:13:25 CST 启动原定递归阶段：screen sttrack_m46_recursion_20260905，递归控制器 PID20235，两份 Python PID20236/20237，分别 16,567/16,563 帧，合计 22 条序列/33,130 帧。首次实际检查三者均存活。新的递归控制器退出记录为 recursive_controller.exit，与保留的原 controller.exit 区分；完整结果尚未产生。预计约 15:30 完成，下一次有意义检查安排在约 15:27，保留原进程句柄。

实验根目录 /root/autodl-tmp/sttrack_m46_initial_frame_v1_20260905，源码/协议已发布。该新权重仍需完整递归门与后续 low22 门；没有新增 DepthTrack Test、CDTB 或 VOT 完整指标。后续三数据集须使用相同的官方 STTrack 基础权重与此轮通过门槛的关联头组合，不能与历史 SRTrack/SUTrack 数字拼接。


### 5.43 M46 完整递归终态：低重叠总量下降，持续失败增加，停止晋升（2026-09-05）

M46 两份递归轨迹和独立汇总均已完成，recursive_s0.exit、recursive_s1.exit、recursive_controller.exit 全部为 0，原进程 PID20235/20236/20237 均已结束。原报告阶段的 training.exit=1/controller.exit=1 仍按 §5.42 保留；它们不能再被误认为当前递归仍在运行或需要重启。

| 新结果：全部 22 条 Train 开发序列 | 有效帧数 | 平均 IoU | 相对 STTrack default | 低 IoU 帧 | H10 持续失败段 | 正收益序列 | 默认成功序列被破坏 |
|---|---:|---:|---:|---:|---:|---:|---|
| M46 初始化阶段覆盖，固定最终权重 | 28,897 | 0.686428344316008 | +3.420208 个百分点 | 5,934（−1,463） | 89（+14） | 10/22 | mobilephone02_indoor |

相对 M45，M46 的平均 IoU 仅增加 0.00232597003764734，低 IoU 帧再减少 254，但 H10 失败段增加 9。主晋升条件仍为 **FAIL**：均值、低帧数和正收益覆盖条件通过，失败段与成功序列保护条件不通过。没有低22或全量正式评测。此处失败段是 Train 单起点代理，不能称作已测得 VOT ROB 下降。

逐序列收益与退化均保留：ball07 相对默认均值增加 29.13 个百分点，colacan01 增加 63.83 个百分点；egg 则下降 28.51 个百分点，低 IoU 帧 523→1,562、失败段 9→23。beautifullight01 下降 19.14 个百分点，失败段 4→8；mobilephone02 下降 24.98 个百分点，低 IoU 帧 1→179、失败段 0→1。不能用净均值掩盖这些稳定性损失。

首次改选记录还给出两个不同现象，帧号均从 0 起算：egg 在关联头首次参与决策的帧号 2 就将默认 IoU 0.874292 改成 0.033289；补齐训练序列早期帧没有在这条开发序列上解决初始化实例判断。mobilephone02 的首次改选在帧号 21，IoU 从 0.735234 提高到 0.765840，但完整轨迹仍新增失败。这说明单帧改选改善不能替代递归验证；尚未证明第 21 帧这一次改选单独导致后续失败，后面还有其他决策。

全部 22 条轨迹 SHA、帧号、初始化框、预测有限性、首次改选前默认轨迹、默认标量 IoU/H10 重算、权重/源码摘要及固定晋升门均通过检查。聚合指标也由逐序列结果独立加总一致。最终模型与 §5.42 已审计权重相同，未增加训练步骤。

| 终态产物 | SHA-256 |
|---|---|
| recursive_result.json | c8e8625ebb6d519e1ec78e1b65be089429030feb48ccaafff9dee2be21567851 |
| per_sequence.csv | d61255b3b0535949af84931c368a600197aa0b8346e865ad3c9fc8da59d25a78 |
| shard0_receipt.json | 32c4439014f2ec77a1cd9bf6443d25388022bad5ed99a5be572853d5d964ac07 |
| shard1_receipt.json | c64a7de94a4da7d882fdd23f00a8599bb28ee8e2169cce660b243b0f7d8d7e8a |

M46 作为一次固定优化预算下的输入覆盖对照封存，停止继续扫描早期帧数量或设置推理 warmup 屏蔽。下一个具体诊断应回到候选关联监督：统计拟合序列中同一 GT 同时对应多个有效候选框的频率，检查当前单一候选标签的排序监督和 partial matching 是否需要区分“多个框属于同一实例”与“只能提交一个动作”。这只是待查假设，不能先把单一动作标签称为错误 GT，也不能以此跳过主门或直接改成最高余弦选择。新神经改动仍需 DepthTrack Train 拟合与同栈递归对照。

同权重三数据集接口的只读检查另见 diagnostics/m46/FORMAL_EVALUATION_CONTRACT.md：旧 SRTrack 数据集类自动附带历史文本，当前语言关闭的 STTrack 不应直接使用这些入口；VOT 使用既有 M39 TraX 路径，DepthTrack/CDTB 保留已验证的 bounded-overlap PR/F-score 计算和输出精度。尚未创建或启动新的正式评测运行，也没有新的完整指标。


### 5.44 M47 拟合输入普查：候选框序号不能直接代表物体序号（2026-09-05）

M47 当前只完成一次 CPU 训练输入审计，**尚无 M47 新权重、新网络干预或跟踪评测结果**。检查范围是 M44/M45 已有的全部 63 条拟合序列、1,511 对输入；逐一核对缓存 SHA 并按既有 IoU≥0.5 定义重算候选有效性，没有计算新的开发序列或测试序列性能。
...
| 前一帧候选集合同时含多个有效框 | 906 |
| 两帧同时含有效框 | 836 |
| 活跃正向匹配中还存在其他有效前帧候选位置 | 822 |
| 活跃反向匹配中还存在其他有效当前候选位置 | 798 |
| 当前有效框超出每输入一个框的总数 | 3,582 |
| 出现多个有效候选的拟合序列 | 63/63 |

有正确候选的当前输入中，812/872=93.12% 同时有多个框达到阈值。观测代码采用 3×3 格点抑制提取十个框，没有证明它们对应十个独立物体。因此，“候选列表第几个”不能直接用作用户希望描述的“相似物体中第几个”；语言相对位置必须对应到真实实例及其关联，不能给重复框赋予不同物体身份。

此结果也不能把唯一动作标签称为错误 GT：跟踪器最终仍需提交一个框。值得冻结的下一项单独对照是：保留 M45 的 default-priority 动作监督、原始训练数据、初始化、960 次优化与全部推理逻辑，仅检验辅助 partial matching 是否应接受多个达到现有阈值的目标候选位置，而非强制唯一格点对应；其他干扰物真实身份仍未知，不能编造其匹配标签。这项训练尚未实现或运行，必须先验证 loss 语义并冻结方案，不能用此次统计宣称它一定能改善递归或 VOT。

完整 1,511 条计数、分布和逐序列统计见 diagnostics/m47/valid_candidate_multiplicity.json，454,171 字节，SHA-256 969846496607a1681a75e0c1acae2000801b55846bb973f5d60f5a17ba65ff0e；审计源码 SHA-256 d027aa5ef274f985418b7194ad0bf318759290e650e33d5f2d018cc979552d79。实验根目录为 /root/autodl-tmp/sttrack_m47_correspondence_audit_v1_20260905。M45/M46 负结果保持封存，M39 STTrack default 仍是正式低22直接基线，完整三数据集目标仍未由同一新模型验证达成。


### 5.45 M47 多正例辅助匹配：loss 语义验证与真实训练完成，完整递归运行中（2026-09-05）

基于 §5.44 的拟合数据普查，M47 现已实现并冻结一个单独的辅助匹配对照。动作监督仍为 M45 的 default-priority 唯一动作标签及原 cross-entropy；选取的当前/前一帧目标查询也不变。唯一变化是辅助 partial matching 的目标位置：同一查询允许将概率质量分配给另一帧所有达到既有 IoU≥0.5 标准的有效目标框，目标集合为空时使用 unmatched，两帧都没有有效目标时匹配项贡献仍为 0。

每个有效查询的匹配项为 `-log(sum(probability[valid_target_destinations]))`，正反方向按原有效查询数量求平均，总 loss 仍为动作 CE 加 0.25 倍匹配项。其他干扰物的真实身份仍未知，没有为它们编造跨帧 ID。这里没有把候选序号当作物体序号，也没有给网络增加语言输入。

**训练前的 CPU float64 loss/梯度检查已执行并通过：**只有一个正例时，新旧总 loss、分项 loss 和梯度最大误差都为 0；两帧空目标时匹配项及其梯度为 0；未标注干扰物之间没有新增直接匹配监督；动作 loss 完全保留。将一个额外有效匹配位置的分数提高后，旧单位置 CE 从 2.397895 升到 2.626868，新集合匹配项从 2.051322 降到 1.563404，符合此次定义。测试时优化步骤为 0，这些是语义检查，不是性能收益。

网络结构与 M45 相同：448,739 参数候选集合头，原生 RGB/Depth 候选 RoI、相同两个时刻与模板参考、同一 geometry 路径，官方 STTrack 主干保持冻结。复用原 63 条拟合序列/1,511 对与已使用的 22 条开发序列/590 对缓存，不加入 M46 的早期新样本。20 epoch、batch32、seed2026、AdamW 学习率 0.0003、weight decay 0.01、梯度裁剪 1、固定最终 epoch，共 960 次优化。原生置信度、argmax/NONE 行为、query、模板更新、搜索裁剪及全部推理源码不变。

真实训练已完成，training.exit=0。拟合前后都核对了 M45 当前/前帧动作标签，初始权重和样本排列摘要完全相同；最终权重严格重载后 590 个开发输入的 logits 完全一致。独立 CPU 审计重新构造初始参数、加载实际权重文件，并重新比较两套标签映射，确认 46 组张量发生更新、参数总数/960 步正确、权重全部有限、基础模型与冻结源文件一致。

| 新结果：静态 Train 开发诊断 | 输入数 | 平均 IoU | 正确候选数 | 改选数 | NONE | 严重救回/新增严重退化 | 正收益序列 |
|---|---:|---:|---:|---:|---:|---:|---:|
| M47 固定最终权重 | 590 | 0.44191789627075195 | 271（默认 268） | 41 | 204 | 7 / 5 | 8 |

静态结果仍为混合收益，没有超过 M45 的静态均值。训练 loss 下降或“匹配成员正确率”不能单独证明改进：新匹配统计按多个有效位置计算，不能与旧唯一位置准确率直接比较。拟合前冻结的计划要求完整运行 22 条递归，不依据这些静态数值改选 epoch 或修改门槛。

| 封存产物 | SHA-256 |
|---|---|
| M47 spec.json | 47f0b9903e845777c406fde350010ac71ecb9689fb3d26b1405af26194b7267b |
| 多正例辅助 loss 源码 | fd1ead4a48451b585db8be7c43645b4dc0e527af250c2c2b5eb9aeea940d6b36 |
| loss_contract.json | fee80ba5b1ef933e21ef915e9c5fb17b15fc5d66944dadb5e3cbf79c140f4fcc |
| geometry_final.pth | 016a3898d43ac870761c18424c7627ab35fe4a495bf61579c14629e1607066ab |
| geometry_result.json | c2df1099344ea29dfea3555647b04aa6f1b1d3ac245df316c76a2b659061065c |
| weight_audit.json | 255446a3234a3648aec8d140da86dfdb0072c8ec7675c2a5e71edbf66035ce2d |

当前训练实验根目录为 /root/autodl-tmp/sttrack_m47_multipositive_v1_20260905；先前的 CPU 普查目录单独保留。screen sttrack_m47_multipositive_20260905，原控制器 PID22403、完整递归 Python PID22493/22494，分别 16,567/16,563 帧。2026-09-05 16:10:18 CST 实际检查三者均存活，递归已运行约 3 分17秒，预计约16:23–16:24完成；接近结束再检查相同句柄。

完整递归结果尚未产生。晋升仍要求相对 STTrack default 均值至少增加 0.01、低帧数下降、H10 失败段不增加、至少3条序列均值改善且保护原零失败序列；随后才是冻结 low22 与同权重三数据集全量验证。没有新增正式 VOT、DepthTrack Test 或 CDTB 指标，M45/M46 负结果不追溯晋升。


### 5.46 M47 完整递归负结果封存：多有效匹配目标未改善持续跟踪（2026-09-05）

本节接续 §5.45 的训练与权重审计记录，只补充已经完成的完整递归结果。M47 仍是同一个 448,739 参数候选关联头，唯一改变辅助匹配的有效目标集合；M45 的动作标签、初始化、样本顺序、20 epoch / 960 次优化、推理模块以及 STTrack 默认模板更新保持不变。模型架构和训练详情沿用 §5.45，不把本次损失改动称为新的语言贡献。

**执行已结束，性能门失败。** 两个固定分片完成 16,567 / 16,563 帧，共 22 条既定 DepthTrack Train 开发序列、33,130 帧，其中 28,897 帧参与该代理指标。16:24:46 CST 核查训练、两个递归分片及总控制器退出码全部为 0；原 PID 22403 / 22493 / 22494 均已结束。没有未完成的 M47 GPU 工作。

| 新增 M47 完整递归指标 | M47 实测 | 相对原生 STTrack default 的变化 |
|---|---:|---:|
| 平均 IoU | 0.571037773888139 | −0.081188489288105，即 −8.118849 个百分点 |
| 低重叠帧 | 9,946 | +2,549 |
| H10 持续失败段 | 99 | +24 |
| 序列平均 IoU 的宏平均 | 0.655882378166541 | 描述性指标，非晋升主指标 |
| 平均 IoU 改善序列 | 9 / 22 | 达到覆盖数量条件，但不能弥补总体退化 |
| 默认零失败序列中新增失败的序列 | 0 | 本轮保护条件通过 |

原生默认和 M45 的完整基准值已分别记于前文，不重复铺表。M47 相对 M45 的平均 IoU 下降 11.306460 个百分点。本轮三个关键条件——均值增加至少 1 个百分点、低重叠帧减少、失败段不增加——全部失败；序列覆盖与默认零失败序列保护两项通过。**性能判定为 FAIL，停止 M47 晋升，不开展该权重的低22或三数据集全量验证。**

退化集中于 notebook02_indoor 和 egg_indoor：前者平均 IoU 为 0.388048019635767，新增 1,318 个低重叠帧、17 个失败段；后者平均 IoU 为 0.427174125164203，新增 1,194 个低重叠帧、12 个失败段。两条合计新增 2,512 个低重叠帧，接近全体净增 2,549；其余序列同时存在改善和退化。这只用于定位下一轮诊断，不能据此断定某个首帧改选独自造成了所有后续失败。

完整性检查通过：实际权重、源码绑定、全部轨迹 SHA、初始化、帧数、有限值、首次改选前的原生默认路径精确一致、逐标量 IoU/H10 重算以及冻结门槛计算均验证。下载后的 22 行逐序列数据汇总也复现最终总量。因此不能用静态候选救回、匹配损失降低或完整性 PASS 替代完整递归的负结果。

新增产物位于 `/root/autodl-tmp/sttrack_m47_multipositive_v1_20260905`，公开副本位于 `projects/sttrack_lachtt_v1/diagnostics/m47/`：

- `recursive_result.json`：13,045 字节，SHA256 `a669372d8f89614cfc565207c163832f5090385f136f3e1db918219023e60409`。
- `per_sequence.csv`：2,163 字节，SHA256 `fd4a36c2718c96e9cbcddef3e7aa6d64a66f8cf19ddb8e11769e6043eb077adb`。
- `shard0_receipt.json`：2,669 字节，SHA256 `f68eef6425806e169912e5e41874ab2134335a34473dcfcefb71f9a1c3914c33`。
- `shard1_receipt.json`：2,688 字节，SHA256 `dfee97f19113793c1662620696bbd72495db9362b2b90cf84b00efe2297d0606`。

下一项工作限定为 M48 **拟合缓存中的原生特征连续性审计**。依据 M45 已重放的错误改选，比较关联头提议与默认候选各自到上一目标 RoI 的原生 RGB、Depth 特征相似度；固定两路都不下降的零边界条件，先量化它会保留和丢弃哪些拟合收益。原拟合缓存上一选择均为 0，且 M45 拟合集没有严重错误改选，因而该审计不能证明跨序列安全。两路特征来自融合后的跟踪网络，也不是统计独立证据。只有审计仍保留实际候选收益时，才考虑冻结一个单变量递归对照；不凭个别正例宣布这类筛选可靠，不放宽条件来增加动作数。

本节不产生新的正式 DepthTrack / CDTB / VOT 指标。用户要求的同一模型权重组合三数据集验证和 VOT 最终目标仍未完成。


### 5.47 M48：原生特征连续性筛选的拟合审计、完整状态检查与单次递归对照（2026-09-05）

接续 §5.46 的限定计划，M48 已完成拟合缓存审计与实际运行检查，并于 16:57 CST 启动一次完整 22 条开发序列递归对照。**目前尚无 M48 完整性能结果。** 本轮不改变神经网络架构，不增加参数或优化步，直接复用已在 DepthTrack Train 的 63 条训练序列、1,511 个样本上完成 20 epoch / 960 步训练的 M45 关联头。不存在“未在 DepthTrack 训练就把新网络直接拿去测试”的情况。

**唯一动作条件：** 对 M45 提议的非默认候选，比较它与默认候选各自到上一已选择 RoI 的原生 RGB、Depth 特征余弦相似度，两路都不下降才允许改选。边界固定为 0，不扫描或校准新阈值。使用候选自身对齐的 4×4 RoI 特征，在学习投影前比较，并保持原路径的 half→float 数值表示。两路来自融合后的跟踪网络，不能称为彼此独立的证据。

| M48 拟合缓存审计（63 条 / 1,511 样本） | M45 原提议 | 固定条件筛选后 |
|---|---:|---:|
| 改选次数 | 96 | 20 |
| 平均 IoU | 0.513510402512411 | 0.494053173051319 |
| IoU≥0.5 的正确候选数 | 853 | 780 |
| 严重失锁救回：默认≤0.1、候选≥0.5 | 26 | 5 |
| 严重破坏：默认≥0.5、候选≤0.1 | 0 | 0 |
| 有益改选 | 94 | 19 |
| 有害改选 | 1 | 1 |

该拟合集默认平均 IoU 为 0.489167940853231。76 次否决中，75 次否决了有益改选，21 次否决了原本的救回；唯一一次有害改选仍被保留。剩余 20 次改选覆盖 15 条序列，5 次救回覆盖 4 条序列。**这说明条件损失了大量拟合效用，没有证明安全性改善。** M45 在拟合集本来就没有严重破坏事件，不能把筛选后的零严重破坏写成泛化能力。保留的少量救回仅支持进行一次完整递归试验，不支持放宽边界制造动作。

审计完整读取并核验原 63 条拟合序列的缓存 SHA、M45 封存动作与标签，未计算开发集或公共测试性能。全部上一选择均为原生默认候选 0，因此拟合结果没有覆盖自身策略递归后非零上一选择的分布。CPU 与 GPU 上的 1,511 个接受/拒绝决定完全一致。审计产物 `native_continuity_audit.json` 为 883,988 字节，SHA256 `969a982bf7633b31d25569b2cdd4bcf6e957b45934705fceb808085b3fa24950`；源码及公开副本在 `projects/sttrack_lachtt_v1/diagnostics/m48/`。

推理实现采用 `STTrackCandidateContinuity` 包装原有头的输出：否决时在状态提交前转为原有 candidate0 动作，之后仍由冻结的 `STTrackCandidateSet.track` 统一更新框、置信度、模板、query、参考 RoI 和上一选择。接受时完全沿用 M45 候选路径。没有改动已封存的 M44—M47 运行文件。默认模板更新保持开启；本轮语言关闭，以单独判断候选接受条件的作用。

实际运行检查已通过，产物 `runtime_contract.json`：

- 四个数值案例覆盖两路更弱、上一选择为非零时更强、两路相互矛盾、完全相等。
- 2 条拟合序列共 120 个实际跟踪帧，强制提出并拒绝 118 个非默认候选，框、置信度、query、模板及 mask 状态与原生默认逐项精确一致，覆盖 2 次真实默认模板更新。
- 2 帧强制允许的非默认候选与 M45 逐项一致，上一选择按 0→1 传递。
- 权重仍为同一个 448,739 参数 M45 头，新增参数 0、优化步 0；原权重 SHA 与所有本轮源文件绑定检查通过。

运行根目录 `/root/autodl-tmp/sttrack_m48_native_continuity_v1_20260905`，screen `sttrack_m48_native_continuity_20260905`，控制器 PID 24424，两个跟踪进程 PID 24427 / 24428。两个固定分片分别 16,567 / 16,563 帧，共 33,130 帧；启动回执时间 16:57:04 CST，预计 17:14—17:16 完成，首个有意义的进度查询安排在约 17:11。不得因一次观察超时重启训练或跟踪进程。

协议 `spec.json`：3,688 字节，SHA256 `f398dc9b4639f15e3f4a69bae067037b5ebc6ac4bac7244ea348be4eb37128a0`。运行检查 SHA256 `1ce4b05ed965f2342354e46fe71100d0dcc7764f0b79faee2a68004a654b622f`。复用 M45 权重 SHA256 `853a25fbc3c9ef12ab54442c30b27bab75f0b1fadcc9bcc82cbec6e8700ed59c`。新增包装器 SHA256 `143a496eb249dcf16e8c2e1d0b12fe689e6b857f695d81ec1d2a9ae8f624bd9f`。本轮无新的训练权重文件。

晋升顺序不变：完整训练开发递归相对实际 STTrack default 达到原有五项条件后，才进入 M39 低22基准对照；低22有明确改善且保护原成功序列后，才验证同一 base＋head＋runtime 组合的 DepthTrack Test、CDTB 与完整 VOT。开发折和低22都已反复用于诊断，不能宣传为完全未见数据。本节没有新增正式指标，VOT 最终目标仍未完成。


### 5.48 M48 完整递归：总体大幅改善，仍有一条默认成功序列的保护条件未通过（2026-09-05）

接续 §5.47，M48 已完成原定全部 22 条 DepthTrack Train 开发序列。17:16:14 CST 核查控制器 24424 与跟踪进程 24427 / 24428 均已退出，三个退出码全部为 0。两个分片共 33,130 帧、28,897 个有效评测帧，未重新训练或改变 M45 权重，也未修改已冻结的接受条件。先前运行中状态已由本节终态替代。

| M48 新增完整递归指标 | 实测值 | 相对原生 STTrack default 的变化 |
|---|---:|---:|
| 平均 IoU | 0.714587093266861 | +0.062360830090617，即 +6.236083 个百分点 |
| 低重叠帧 | 5,331 | −2,066 |
| H10 持续失败段 | 68 | −7 |
| 序列均值的宏平均 | 0.710221088507192 | 描述性指标 |
| 平均 IoU 改善序列 | 7 / 22 | 覆盖条件通过 |
| 默认零失败序列中出现新增失败的序列 | 1 | mobilephone02_indoor，保护条件失败 |
| 非默认提议 / 否决 / 实际接受 | 914 / 858 / 56 | 实际因果运行记录 |

**五项冻结条件通过四项：均值收益、低重叠帧减少、失败段不增加、序列覆盖均通过；默认成功序列保护未通过，整体晋升门仍为 FAIL。** 不能把本轮一概写成没有收益，也不能因为总体收益较大而事后取消保护条件。相对同一个 M45 关联头，固定筛选使平均 IoU 再提高 3.048472 个百分点。当前保留本轮全部证据，不启动低22或三数据集全量验证。

收益集中但真实：ball07_indoor 平均 IoU 为 0.829398917114345，比默认增加 46.371919 个百分点，低重叠帧减少 1,106、失败段减少 3；colacan01_indoor 平均 IoU 为 0.434090208872385，比默认增加 33.293493 个百分点，低重叠帧减少 1,115。其余序列仍有改善与退化，完整 22 行指标保存在 `per_sequence.csv`，不能只汇报两条正例。

完整性检查通过：全部轨迹及源码 SHA、实际复用权重、初始化、帧数、有限值、每帧提议/否决关系、上一选择的因果传递、首次接受前原生默认路径精确一致，以及独立标量 IoU/H10 与冻结门槛计算均验证。下载后以 22 行逐序列结果重算总量也一致。

新增产物位于 `/root/autodl-tmp/sttrack_m48_native_continuity_v1_20260905`；公开副本在 `projects/sttrack_lachtt_v1/diagnostics/m48/`：

- `recursive_result.json`：13,047 字节，SHA256 `e3934bd736e9f14b6009cad4c5c05090a2527c3410aa54bb179eab2d84feeb7b`。
- `per_sequence.csv`：2,179 字节，SHA256 `5d7a26361a522952d89c6a90075ae28ec8e490dcf63c7eb4939c8528c354e70d`。
- `shard0_receipt.json`：3,120 字节，SHA256 `5b3a2b406895d9dc58b68f2a500044a1fbf2c533095a408e40d1e464e35d2c6d`。
- `shard1_receipt.json`：3,144 字节，SHA256 `bceb7580a635dd7d93d549d9a9f89bf3bc535129c3558aa2814bd7c2dee66e8c`。

补充的拟合关联诊断是在协议冻结后完成，未修改条件。被否决的 21 次拟合救回中，20 次的上一默认框不满足有效目标监督规则；保留的 5 次救回中，仅 1 次如此。相似于前一错误状态可能阻碍找回，这说明静态拟合收益与完整递归表现不能直接互相替代。`previous_target_diagnosis.json` 的 SHA256 为 `3d29fc94f55b6096ed0bbec47fdf3d80659f25c16988b1b63207fe2d5d2c145a`。

**唯一保护反例 mobilephone02 的完整重放已完成。** 700 个实际跟踪帧的框、置信度、提议、接受动作与相似度差均精确复现封存路径。整条轨迹只有一次非默认接受，发生在从 0 开始计数的第 383 帧：默认 IoU 为 0.759101033534034，新框 IoU 为 0.697772825315564，二者框 IoU 为 0.689389。原生响应分数由 0.533020735 降至 0.060397927，但到上一目标的 RGB/Depth 相似度分别提高 0.002609968 / 0.000928104，因此通过本轮固定条件。当时两框仍覆盖真实目标，不能直接称为已经证实的即时身份切换。

后续第 450 帧，关联头输出 NONE，但原生响应为 0.833474576，仍满足默认模板写入条件。该帧无有效 GT，模板 IoU 记为 null，不能填成 0；原生默认路径在同帧分数为 0.391427606，不发生模板写入。M48 在第 497—700 帧形成 204 个有效低重叠帧的失败段；500、550、600、650、700 帧的后续模板写入均为零 GT 重叠。第 383 帧的一次框改变距离确认失败起点相隔 114 帧，短窗口内有正确框并不能保证长期递归稳定。

第 450 帧写入目前是待验证的因果因素，不能直接断定它是唯一原因。下一步已限定为：完整重放该序列，只禁止第 450 帧这一次模板写入，之后立即恢复原条件，判断它是否改变后续跟踪。该时间点来自已封存开发失败分析，是诊断干预，不能变成按帧号部署的规则，也不能把救回后的单序列结果拼进完整 22 条指标。若干预显示有用，再研究训练集可观测的选择性更新条件；不会恢复全局停更方案。

在线文本的只读复查同时完成：72 条原始回答均可解析，记录的当前图像都是 640×360；相对位置条件的 24 个原始回答全部声明身份不确定，14 个给出数量，0 个给出目标序号。序号缺失不是解析器丢字段，不能忽略不确定标记强行提交。既有样本主要来自失败起点及前十帧，这一结果不能替代跟踪健康时更新位置描述的验证。没有新一轮 VLM 调用或在线文本提交。

本节全部数值仍为训练开发代理指标与诊断，不是正式 VOT EAO/ACC/ROB 或 DepthTrack/CDTB PR/F。用户要求的同一最终模型组合三数据集完整验证和 VOT 最终目标仍未完成。


### 5.49 M48 模板单次干预的负结果与重新出现时的搜索覆盖（2026-09-05）

§5.48 计划的单次模板干预已经完成。只在 mobilephone02_indoor 第 450 帧把模板写入条件暂时关闭，处理完该帧后立即恢复原生阈值；没有改变框选择、query 更新、后续模板条件或网络权重。700 个实际跟踪帧完整执行，第 450 帧及以前的框、置信度与原 M48 路径精确一致，确认该次模板写入确实被抑制。预测封存后才读取后续 GT 计算指标。

| 新增诊断干预：仅取消第450帧模板写入 | 完整单序列结果 | 相对原 M48 的变化 |
|---|---:|---:|
| 平均 IoU | 0.568362728917360 | 0 |
| 低重叠帧 | 205 | 0 |
| H10 持续失败段 | 1 | 0 |
| 非默认接受帧 | 仍只有383 | 没有新增接受 |

**该次写入的单独抑制没有救回目标。** 后续框和置信度确实发生变化，不能写成两条轨迹完全相同；本表说明的是有效 GT 上指标一致。第 450 帧本身没有有效 GT，不能给它填入虚构的模板 IoU。本轮不支持直接新增“NONE 就禁止模板更新”的部署规则，更不能把它外推为默认模板更新整体无效。

根目录 `/root/autodl-tmp/sttrack_m48_native_continuity_v1_20260905/phone_skip_write450`：协议 `spec.json` 为 700 字节，SHA256 `83fba94667e821d66a790c8a8279829a773ca3dc228d13d55088ad5d196a1e7b`；结果 `result.json` 为 1,657 字节，SHA256 `3e91001fd2752ad389948b4df6995bee8f406f86619a3bac924f3d72e4c40c3d`；完整诊断轨迹 SHA256 `8b91cb42ec87a100ba55fb9510fb10f8b3b527958ac0b7080ba02944a2122e56`。这是已知开发反例上的事后定时干预，不是可部署触发器，不产生新的完整 22 条或正式指标。

随后按运行源码 `sample_target` 的确切公式 `ceil(sqrt(w*h)*4)` 及取整裁剪原点，重建第 497 帧的搜索几何。该帧是前一帧 GT 无效之后的有效目标帧，GT 框为 `[326,136,26,31]`。三条路径都使用各自第 496 帧预测状态，未用 GT 改变搜索位置。

| 重新出现时的路径 | factor4 搜索区域 xyxy | 真目标中心覆盖 | 真目标面积覆盖 |
|---|---|---|---:|
| 原生 STTrack default | `[268,84,411,227]` | 是，完整框也在内 | 100% |
| M48 | `[411,156,706,451]` | 否 | 0% |
| M48，仅取消第450帧模板写入 | `[376,-9,812,427]` | 否 | 0% |

这说明取消单次模板写入后，错误分支的搜索状态仍未恢复。完整原生默认路径保留着能够覆盖重新出现目标的搜索位置，而 M48 的单条递归路径已经丢失了它。该证据来自 M48 **新增的 DepthTrack Train 反例**，不修改 M40 对原生 STTrack 的 VOT 124 个失败起点的统计，也不能据此恢复全局宽搜索或全局停更策略。

搜索诊断 `mobilephone_search_geometry.json` 为 1,947 字节，SHA256 `2e7648eebeb0e0473da2016ac54e5a87c32f5ddda2057b589110c1ab33a428f0`；使用的 `processing_utils.py` SHA256 为 `7aca916f5e5f62e1865fbd322ffb63dc08197c544a241938b6c822d4bfb89bf6`。完整重放报告 `mobilephone_diagnosis.json` 为 9,164 字节，SHA256 `648d3e6c7fae2c6c2acca22762e48c198ab72db03d417dcda3e5f4fa96b1e132`。

后续工作应先验证：在试探性改选期间持续保留真实 default 的完整独立状态，是否能在遮挡后提供仍然有效的候选及身份依据，再决定关联/语言/状态提交的改造。不能只恢复一个框、只恢复旧文本或追加按序列名/帧号的例外。这里尚未实现新的后继模块、训练权重或运行任务；若后续改动神经网络架构，将先在 DepthTrack Train 训练，再按冻结顺序验证同一模型组合。

当前 M48 全部运行、手机完整重放和模板干预均已结束，没有尚待完成的上述 GPU 任务。M48 的总体收益与唯一保护失败都保留，晋升门不变；没有新的正式低22、full127 或三数据集结果，最终目标仍未完成。


### 5.50 M49 相邻帧运动、尺度与模板更新验证（2026-09-05）
...
- [相邻帧实例图](https://github.com/666666666666gao/Track/blob/main/projects/sttrack_lachtt_v1/diagnostics/m49/adjacent_frame_examples.png)；[可导出PDF](https://github.com/666666666666gao/Track/blob/main/projects/sttrack_lachtt_v1/diagnostics/m49/adjacent_frame_examples.pdf)。
- [中心与尺度散点图](https://github.com/666666666666gao/Track/blob/main/projects/sttrack_lachtt_v1/diagnostics/m49/motion_scale_scatter.png)；[可导出PDF](https://github.com/666666666666gao/Track/blob/main/projects/sttrack_lachtt_v1/diagnostics/m49/motion_scale_scatter.pdf)。
- [完整性审计](https://github.com/666666666666gao/Track/blob/main/projects/sttrack_lachtt_v1/diagnostics/m49/terminal_audit.json)：628份GT/轨迹输入、124个M40起点/裁剪、124个M41连续IoU容量行复算通过，85份Train特征hash通过。
...
### 5.62 M54两模板视觉读取器已实现、契约通过，修正缓存存储后实际排队（2026-09-06 03:11 CST）

本节承接M53完成态容量，不重新试验M50的尺度阈值追加更新。M53中当前＋初始组合有16个严重事件、7条序列的单帧特权容量，因此先训练小范围的视觉读取器，检验能否将容量转化为完整递归收益。M52两组负结果保持原判，原生自身状态与修改后状态的差异仍是未闭合问题。本节尚无训练完成权重或新的正式指标。

**实际网络。**基础权重仍为STTrack_Vot22，SHA256 `cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98`。ViT/BSI、TSG固定query窗口、Mamba、Center头、factor-4/256搜索、factor-2/128模板保持原样并冻结。比较同一帧前状态下的两个真实前向：当前`[initial, dynamic]`与替代`[initial, initial]`。初始模板原本已占第一槽，替代组合是改变其占比，不是增加新视角。初始化至第一次动态写入前两槽为同一张量，复用同一输出。

新增TemplateReader有58,923参数，输入各组合自身原生top1的4×4 RGB-D RoI、初始化/上一实际目标的因果参考、raw/Hann响应图、几何及原生分数。RoI投影32维，局部跨注意力匹配初始化与上一参考，按模态组合局部证据；响应图经小卷积编码后联合评分。缓存和运行中的RoI均先float16再转float32，避免两种输入精度不一致。模块初始logits等于原生log分数；训练后直接比较两组合，不增加阈值扫描。没有文本，不宣称旋转等变、姿态角识别或局部token独立收益。

两个前向分别复制query。最终bbox、confidence、下一query与上一RoI统一来自所选前向；读取初始组合不永久覆盖动态槽，默认每50步confidence严格大于0.75的模板写入保留，并使用当前所选框。只有一条真实递归路径，没有始终并行的独立原生保护轨迹，也没有NONE/失踪状态。读回当前组合不能恢复早先已分叉的原生轨迹。

**实际完成的修正后契约：**

| 检查 | 帧与更新 | 完成结果 |
| --- | --- | --- |
| 当前读取与独立原生比较 | chair01/cube04各120步，共240窗口，3次写入 | bbox、score、模板、query、模板图像完全一致；两读取输出不同90窗口；79.31秒，exit0 |
| 固定时刻读取初始组合与原生临时替换槽比较 | 同两序列各120步，16次固定初始读取、3次写入 | 完整状态一致；cube04有7次输出框真正不同，chair01为0；79.52秒，exit0 |
| 读取头接线 | 58,923参数，CPU梯度/视图交换、GPU实际选择输入路径 | 全证据分支梯度有限且活跃，原生log分数初始化和输入精度路径一致 |

固定读取帧索引50、60—65、100只用于契约，沿用零起点编号，未作为部署触发。上述检查没有GT、优化器步骤或性能评测。修正后contract SHA256 `332552c634ba9a08a159050354949d0d48e8e99e3890eb20ee058e960e2ba04b`，runtime_contract SHA256 `42b6c7e33780382a96059c2f1800fe7e8bd6d7ab285dff1d2c9edacb5949a94c`。

**固定训练与验证。**只用DepthTrack Train的63条拟合序列，既定1,511事件与每10帧定期窗口并集，共10,615个窗口、93,362个原生跟踪步，均截至原最后固定事件。主采集路径保持当前读取，对既有封存原生框/分数逐帧核对；全部63份特征封存并验证hash后才读拟合GT。无效GT不监督；标签1仅在当前IoU<0.5且初始组合IoU≥0.5时成立，其他有效窗口标签0。正常窗口一并采集，但原事件仍含后验选择，不是可部署触发数据。

读取头固定seed2026、20完整有效窗口epochs、AdamW lr/weight_decay各0.0001、batch32、clip1，步数为20×ceil(valid_windows/32)，只保存并严格回读唯一final权重。底座不更新，不按开发结果选择epoch。随后无条件跑完既有22条DepthTrack Train开发序列的实际递归，并执行原门槛：mean IoU至少比原生0.652226263提高0.01、低重叠帧少于7397、H10不超过75、至少3条均值改善、全部原生零H10序列不新增失败。开发集已反复使用，不称全新未见。当前全模块筛选不能单独归因于局部匹配或响应编码，也未补充自身策略状态训练，不能据此宣称解决M52问题。

通过完整开发门后才冻结低22：相对M39的57.135993 EAO / 75.719622 ACC / 73.022401 ROB、124失败，EAO与ROB各至少+1pp、ACC最多下降0.10pp、失败减少且保护7条零失败序列。之后同一base＋读取器权重＋运行策略验证DepthTrack Test、CDTB及full127；长期目标仍是完整VOT的ROB>93.7、EAO>77.9、ACC>82.1。未通过则停止此版本，不放宽门槛。

**实际存储问题及最小修正。**02:50测得实验持久盘仅余2,445,430,784字节，按240窗口约48MB估算全拟合特征约2.13GB，影响仍在运行的原生结果输出余量。先停止尚未进入长采集的等待进程174609，再将正式采集与训练读取两行改到spec指定的`/dev/shm/sttrack_m54_features_v1_20260906`；该挂载约90GiB可用，系统可用内存约690GiB。模型、样本、标签、预算、运行门不改；权重、日志、轨迹、结果留在持久实验根。没有删除旧实验数据或停止原生评测。第一轮规范、两文件旧快照、79.76/80.41秒契约、队列及审计原文保存在initial_storage_binding。新规范SHA256 `1ca1387e6eb33c897e12d5e0c10b746d48257e7745906c431e8ac857f63d7267`。

共享内存重启会丢失，因此实际新增并启动本机独立归档程序：03:09:53启动PID48148，03:11核实存活，首次05:00检查采集终态，此后240秒；只在成功封存后将全部63份特征下载到本机私有工作区，逐份核对字节数/SHA256并写archive_receipt，不读取GT或删除源缓存。本机可用磁盘约13.33GB。当前只有等待进程证据，没有归档完成凭证。

**执行状态与审计边界。**第一轮独立实现复核未发现具体模型代码缺陷。存储复核指出缺少归档步骤，以及新契约未结束就排队可能安全失败；两个原意见完整保留。实际已添加上述归档程序，并在新版两契约均PASS、退出0、当前spec和7个服务器源hash一致后才重启等待队列。03:11:44核实新队列PID182350存活，尚无长采集/训练。首次03:30检查GPU0两项原生OPE tracking.exit，之后240秒；均exit0后按采集→训练→22序列递归→分析顺序运行。公开评测不自动启动。复核为同GPT家族Type-A建议性审计，不是跨家族认可，更不是学习权重效果验证。

| 当前结果范围 | 本节时点状态 | 可报告的新指标 |
| --- | --- | --- |
| M54接口与接线 | 两契约完成、exit0 | 仅上表执行一致性结果 |
| M54拟合/训练/完整Train-dev递归 | 已排队，未开始长采集或训练 | 无 |
| 原生DepthTrack Test / CDTB | §5.61已启动，等待完整终态 | 无新增完成指标 |
| 原生VOT full127 | 原GPU1任务继续 | 无新增完成指标 |
...
### 5.64 原生STTrack的CDTB完整结果完成，M54已进入长采集（2026-09-06）

原生CDTB参照已完成80条序列、101,956帧；tracking、analysis、controller退出码均0，04:11:08 CST通过SSH核实终态。主跟踪耗时8,415.20秒，动态模板实际更新1,614次。本节是原生STTrack参照的完整结果，不是新训练读取头的结果。

| 完整CDTB指标（%） | 当前原生STTrack | 项目目标 | 当前减目标（百分点） | 是否达标 |
| --- | ---: | ---: | ---: | --- |
| Precision | 69.933113 | 72.900000 | -2.966887 | 否 |
| Recall | 68.067744 | 75.600000 | -7.532256 | 否 |
| F-score | 68.987821 | 74.200000 | -5.212179 | 否 |

三指标位于同一个全数据集最大F阈值0.580139（100点网格零起点索引89）。口径与§5.63一致：bounded VOT矩形重叠、无效GT Special(0)、全局分数网格、逐序列PR宏平均后求最大F；六位小数输出、初始化confidence=1，没有另外缩放报告框或逐序列优化阈值。

**网络与完整性。**同一ViT/BSI＋TSG固定query窗口＋Mamba＋Center底座、两个RGB-D模板、factor-4/256搜索和原生50步confidence>0.75写入，语言与学习关联头关闭；权重SHA256仍为`cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98`。Python入口SHA `11748e8e005e9cf0706a1ff24aa59bf9830dcaf23160db5274fa702720a04ec4`，shell控制器SHA `9947aa8dec69267e3bbd0d51b20dcfad5a2bbb9e4e0cfe23a9b8b868d6bddbf7`，固定评测器SHA `05879f2e732aed982fbcbebd9756ce063ed0fa945c1f6b0c04092c3e487466cc`。先封存160份输出，再读后续GT分析；没有训练步骤。原始框/分数、80份GT及80张首图均下载至私有复核目录并核对hash，原始GT和图像不公开。

全部80条逐序列P/R/F、总帧、有效帧与选中帧已导出JSON/CSV，均使用全局阈值；有效GT共91,300帧、选中91,650帧。整体F由宏平均P/R计算，不等于逐序列F均值。正式metrics SHA256为`a00f5db6e853dd9ac59ef2e174ed7a06e991701e61bfd85badf29ae273c1b731`，tracking receipt为`6ea74472df6647ba6e1c3ca50b11bce5a03ed903bf883da62585611a40ed43fe`。完整结果、全80行表、退出码、日志、下载绑定与审计见`projects/sttrack_lachtt_v1/diagnostics/native_ope/completed_cdtb/`。

独立只读复核从原始框、分数、GT与实际图像边界重新计算重叠和100点宏平均PR，没有调用项目评测函数。聚合、全部80行JSON、80行CSV及CSV/JSON比较均零差异；所有预测、GT、首图hash和帧数一致。实际边界分布为31条640×360、23条768×432、23条960×540、3条1920×1080。未发现具体完整性缺陷，完整性PASS，范围与目标WARN，evaluation_type为real_gt。复核只访问本地/Git与下载数据，没有访问服务器；远端终态由主执行器SSH核实。仍为同GPT家族Type-A建议性审计。完整原文EXPERIMENT_AUDIT.md的SHA256为`fb4bcaf262c91ba511af727ee3f29f91f80d40df33c171f62f08af3fc6a2344d`，不把失败指标归为测量错误。

**M54实际启动。**两个原生GPU0跟踪任务exit0后，原等待程序于04:09:59 CST启动冻结的采集→读取头训练→完整22条Train-dev递归→分析流程。04:11:08实际核实队列182350、控制器203098、collector子进程203099均存活。04:16:31核实spec仍为`1ca1387e6eb33c897e12d5e0c10b746d48257e7745906c431e8ac857f63d7267`，7个服务器源码hash全部匹配，没有因公开结果修改运行方案。

| M54项目 | 04:16:31已完成 / 冻结总量 |
| --- | ---: |
| DepthTrack Train拟合序列 | 4 / 63 |
| 原生跟踪步 | 5,724 / 93,362 |
| 两种模板读取窗口 | 669 / 10,615 |
| 原生动态模板更新 | 31 |
| 封存特征文件 | 4份，134,378,964字节 |

最新ball05序列bbox/score回放误差均0；该序列没有动态写入，两种读取没有不同框，不是学习效果。共享内存剩余约96.5GB，持久盘剩余约2.38GB，仍沿用§5.62缓存路径与本机归档安排。早期325.44秒完成5,724步，约17.6步/秒，调度上粗估05:40 CST附近结束全采集；以实际吞吐和退出凭证为准。归档程序首次05:00检查成功封存，此后240秒；本节时点没有归档完成凭证，也没有训练启动或新权重。
...
### 5.66 M39当前STTrack低22逐序列三指标补全（2026-09-06）

本节补全的是M39已完成STTrack default的逐序列结果，取代“只有历史SUTrack逐序列三指标可看”的资料缺口。没有重跑跟踪、训练或替换权重。新增封存后导出程序在服务器先验证default/no-update各909个结果文件（各303 anchors）的原merge hash，再使用同一VOT toolkit 0.7.1、同一工作区GT和原分析配置计算。两配置完整聚合结果与M39原报告逐项相同，不更新旧聚合指标表。

**表中EAO须单独限定含义。**它是同一官方EAOScore在原low=115/high=755配置下，限制到一条序列的固定区间诊断值。全量EAO按每个时间位置的有效曲线权重聚合，不能对这些单条EAO简单平均，也不能当作各序列的损失贡献。短且完整成功的序列在长区间没有支持的位置被toolkit填0，因此低单条EAO不等于跟丢，不据此触发恢复或选择部署参数。ACC、ROB直接取原分析的逐序列依赖。
...
停止动态模板更新的全22条EAO/ACC/ROB和全部44行原始精度数据，见`diagnostics/m39/per_sequence_20260906/no_update_table.md`及`per_sequence.csv/.json`。当前表完全来自STTrack，不混用旧SUTrack数值。失败统计和原124对155结论不变。

ROB也不等于“未失败anchor的百分比”：单序列ROB是各anchor存活帧总和除以运行帧总和，失败得早或晚会改变它。全低22的ACC按存活帧权重聚合、ROB按序列长度权重聚合，不是表中各列的普通均值。比如cup02虽然36个anchors都最终失败，当前ROB仍为18.866661；而cube05_indoor_2的ROB为100、失败为0，但其146帧只支持本次实际640个EAO score切片位置中的31个，固定区间单条EAO为4.339054。两种情形不能混为一谈。
...
源M39结果SHA256为`cf953c0d3c69609bcd83c11cb24ba57f37e30b38d3b3bcad32860b3a9ba9c1b5`；导出程序SHA为`2be18741c87e1c6c18f09f555584751443bf95d77ff2ded9e40e5346cc0f73c8`；原VOT multistart源SHA为`5a09065e2315387405f4cb8f96c0b8fd32d7428996a34eedd92ac4e2a4deeb02`。完整原分析、merge manifest、逐序列EAO依赖曲线、导出日志与退出码保存在`diagnostics/m39/per_sequence_20260906/`；它是资料与测量补全，不是新模型收益或晋升。

**正在执行的工作。**M54于05:02:13 CST实际核实仍在采集，控制器203098及collector203099存活；完成34/63条、53,615/93,362步、6,090/10,615窗口，已封存特征约1.223GB，已完成序列的bbox/score最大回放误差均0。当前吞吐17.35步/秒，全采集估计仍在05:40附近完成，尚未开始训练。本机归档进程48148存活，已按计划在05:00首次检查并确认采集未完成，后续240秒；没有归档完成凭证。后续按原方案继续训练和全22条Train开发递归，低22的正式新权重验证须等开发门通过。没有改变M54或原生full127运行方案。

### 5.67 后续原生RGB-D底座训练入口的实际准备缺口（2026-09-06）

本节接续§5.65的权重来源核对，补查原生训练入口、模型构建、环境路径及trainer源码，未读取训练图像/GT、执行优化器或改动当前运行代码。这些发现针对后续底座微调准备，不能归因为当前STTrack完整指标下降，也不否定M54独立实现的读取头训练。

| 项目 | 已核实的当前行为 | 接续要求 |
| --- | --- | --- |
| 原训练环境 | local.py的数据、工作、tensorboard及预训练路径指向作者环境，05:26实际观察均不存在；本机Train实际位于/root/autodl-tmp/depthtrack/train | 使用独立运行目录，显式绑定真实数据路径 |
| 预训练初始化 | 默认SOT_Pretrained_256.pth.tar不存在；仅改名为STTrack_Vot22.pth.tar时，build_sttrack同时跳过backbone加载和SOT限定的完整模型加载，原train_script没有另行显式加载当前权重 | 微调前明确加载绑定的完整net并严格核对参数，不能只改配置文件名 |
| 验证周期 | 原配置15 epochs、验证间隔50；trainer仅在epoch % interval == 0时调用loader | 这15 epochs不会执行该验证loader；后续方案须明确实际验证周期 |
| 训练终态 | 原入口fail_safe=True，BaseTrainer的num_tries=1；捕获训练异常后可能打印Restarting及Finished training并正常返回，却没有实际重试 | 让训练异常传播，核对真实epoch/optimizer步数、最终权重及回读，不能仅凭末行日志认定完成 |
| 官方6条验证集的项目历史 | 其中bag04、bottle03、toy03属于当前M54拟合，flower03属于开发；官方146/6自身仍互斥 | 官方划分不等于本项目全新未见划分，不无说明替换当前Train划分 |

原生推理tracker在建网后显式严格加载params.checkpoint；M54由冻结spec传入基础权重、另行训练TemplateReader。因此原训练入口中的字符串加载分支不影响本次已封存原生推理及当前M54实现。这里没有证据表明上述异常路径曾在M54中实际发生，也没有启动新的底座微调实验。

后续底座微调若成为下一步，应在独立目录与冻结方案内绑定基础权重和Train划分，先完成真实batch的前向、反向、参数变化与严格回读，再按固定预算运行，并记录采样条目、搜索帧、全局batch、优化器步数和验证周期。最终仍以同一训练结果与运行策略完成三个数据集验证。当前优先完成M54既定训练和完整开发递归，不以此准备记录替代模型收益。

原始观察绑定9份源码/配置/划分，后补观察另绑定两份trainer源码，见`diagnostics/native_ope/training_entry_20260906/observation.json`及`trainer_source_addendum.json`。同族GPT-5.5 xhigh Type-A静态审查和补充审查确认以上范围；审查未自行SSH、启动训练或GPU测试，首次原文与后续补充均保留，不属于跨模型家族复核。完整证据及接续说明见同目录`RESULT_REPORT.md`。此处PASS仅表示准备问题得到证据支持，不表示原训练入口已经修复或可直接运行。

### 5.68 M54完成DepthTrack Train读取器训练，开始全22条开发递归（2026-09-06）

M54已完成原方案规定的63条拟合采集和固定20 epochs训练。结构沿用此前冻结的两种模板组合读取器，共58,923个新参数，基础STTrack权重冻结、语言关闭，默认动态模板写入条件保留。本节首次记录真实训练终态，不把拟合窗口的静态结果当作开发轨迹或公开评测收益。

| 训练与封存项 | 实际结果 |
| --- | ---: |
| 拟合序列 / 原生采集跟踪步 | 63 / 93,362 |
| 封存窗口 | 10,615 |
| 有效GT训练窗口 / 无效GT排除窗口 | 9,729 / 886 |
| 初始化重复组合为正标签的窗口 | 143 |
| epochs / 优化器步数 | 20 / 6,100 |
| 第一epoch / 最后epoch交叉熵 | 0.421167318 / 0.036099138 |
| 发生变化的state_dict张量 | 22 |
| 最终权重严格回读 | logits逐元素一致 |

6,100步为20×ceil(9729/32)，与20行训练日志及checkpoint记录一致。全部63份特征与事件身份封存后才打开拟合GT，无效GT窗口不进入优化。采集逐序列bbox/score最大回放误差均为0，共1,131次默认动态写入；两种组合在9,861个采样窗口产生不同解码框，这只表示输出不同，不表示替代组合更正确。

| 同一9,729个有效拟合窗口的静态量 | 当前组合原生top1 | 训练读取器选择 |
| --- | ---: | ---: |
...
拟合mean IoU提高0.010229850，约1.022985个百分点。98次恢复是训练窗口计数，不是98条轨迹或VOT anchors；零严重破坏也不证明开发成功序列得到保护。本轮仍由原生状态缓存拟合，没有因此解决读取器自身分叉后的状态分布问题，完整递归门继续检验其后果。

最终`reader_final.pth` SHA256为`53374903f5c7314dbc08c857d9ac6d56f2adc914908e62831176fb7c2b7d0c05`；基础权重SHA仍为`cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98`。spec SHA为`1ca1387e6eb33c897e12d5e0c10b746d48257e7745906c431e8ac857f63d7267`，采集凭证SHA为`708d1b326cdca803cb7813a63ff48ab82943bdf5a8fe2bb5610136c489091e63`。权重保留在服务器M54运行目录，并已下载到本机`.codex_remote_staging/m54_training_completed_20260906/reader_final.pth`核对哈希。

本机于05:44:10 CST完成63份特征持久化，共2,132,202,561字节；之后又逐份重新核对大小及SHA，没有残留partial文件。归档未复制GT、未删除服务器共享内存中的源特征。训练日志、采集及训练凭证、归档凭证、下载绑定和本阶段一致性核对见`diagnostics/m54/trained/`。这些检查覆盖日志/来源/步数/参数变化/持久化完整性；本阶段没有另外独立复算全部拟合IoU，不能写成已完成独立开发效果审计。

**开发递归已经实际启动。**05:45:29 CST核实控制器203098及进程244873存活，collection.exit与training.exit均0，recursive/analysis尚无退出码。首条bag05_indoor完成889帧、15次初始化组合读取，用时79.09秒。全开发共22条、33,130输出帧/33,108跟踪步，剩余序列继续运行；当前没有开发GT指标。根据首条吞吐暂估全量约50分钟、06:30附近完成，下一次检查安排在06:25，后续按240秒间隔并依实际进度修正。

后续仍先封存完整22条输出，再计算全部开发指标和原生成功序列保护门。全部通过才冻结低22评测，低22明显改善后才以同一base＋读取器权重＋运行策略验证DepthTrack Test、CDTB及完整VOT。当前不改变标签、预算、阈值或最终epoch，也不因拟合结果而提前晋升。

### 5.69 M54固定读取器完整递归结束：整体改善，手机成功轨迹保护失败（2026-09-06）

本节续接5.68的训练完成态，报告同一最终读取器在全部22条DepthTrack Train开发序列上的递归结果。33,130帧输出、33,108次跟踪均已封存，之后才统一打开开发GT。controller、recursive、analysis退出码均0；本节不是公开数据集指标，也不把拟合窗口收益重复计为轨迹收益。

| 完整Train开发递归 | 原生STTrack default | M54模板组合读取器 |
| --- | ---: | ---: |
| 有效评测帧 | 28,897 | 28,897 |
...
mean IoU增加2.050801个百分点，严重低重叠帧减少705，H10段减少10；12/22条均值上升，共685次选择初始＋初始组合。五项冻结门中前四项通过，但成功轨迹保护失败：mobilephone02_indoor由0段新增为1段，均值由0.851274441降至0.568574590。primary_pass=false，停止当前冻结读取器，不启动低22或三数据集评测，不调整门槛来制造通过。

收益集中例：colacan04提高34.455666个百分点、H10由12降至6；ghostmask提高13.772078个百分点、H10由3降至0；glass03提高13.647692个百分点、H10由3降至1。损害还包括mobilephone01下降16.833437个百分点和notebook02下降7.296198个百分点。结论是模板组合读取存在真实递归效用，但当前方案不能保护所有原生成功轨迹，不能归为语言收益。

手机进一步定位（均为0-based）：mobilephone02第一次非默认读取与首次bbox分叉均在414帧，GT无效，不能报告该帧IoU=0或认定选错实例；497帧开始出现204帧有效低重叠，延续至700帧。另三次读取位于568、649、691帧，路径已失败。mobilephone01第一次分叉在746帧，其四次非默认读取都发生在无效GT帧。这提示后续需要检查不可确认状态下的读取行为，但无效GT本身不能证明具体遮挡机制，也没有通过取消某次读取的反事实重放确认唯一原因。分叉后的原生与读取器预测来自不同状态；恢复默认读取不等于恢复原生轨迹。

M54仍是固定STTrack＋58,923参数读取器，保留默认50步且置信度>0.75的动态写入，实际只比较初始＋动态与初始＋初始两种组合，没有持续独立原生保护轨迹。63条拟合、22条反复使用的开发序列不交叉；开发集不能称为全新未见数据。权重SHA仍为53374903f5c7314dbc08c857d9ac6d56f2adc914908e62831176fb7c2b7d0c05，底座仍为cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98。递归结果SHA为9ee6b81720d104ceca3b10019ba5affccbd7daa498bf4af1a745e1b28d04f855，递归凭证SHA为97763dc780a5dcc226d1110ed3df5cfb5c7ae58170cb8670d9123d333571d1aa。

证据目录：服务器/root/autodl-tmp/sttrack_m54_template_reader_v1_20260906；公开projects/sttrack_lachtt_v1/diagnostics/m54/completed。原始读取器轨迹、原生框及数据集GT共75文件、8,702,676字节已复制到本机私有m54_recursive_completed_20260906。原生框逐值提取自SHA绑定原始trace，GT按字节复制，完整预测封存后才导出。完整逐序列表、结果、日志、导出脚本与独立代理复核另附；复核为同GPT模型族Type-A advisory，不声称跨模型族验证。原始大trace未被复核代理独立下载，原生框来源由提取代码与原始SHA绑定支持。

代理独立复算44条预测的逐序列及聚合指标，所有整数计数和门槛一致，mean IoU差小于1e-15；本机读取器权重、training_result和training.exit已补做字节哈希复核。基础权重文件本身仍未由该代理独立下载复核。首次审阅的阈值符号疑问经UTF-8复查确认原文已是IoU≤0.1，无需改动，原始审阅与澄清均保留。

### 5.70 TSG方向输入共享获真实权重单帧证实；训练采样顺序另立问题（2026-09-06）

为避免继续在未知表示上堆关联或模板规则，本轮检查当前STTrack模型的TSG方向分支。源码temp_x_flip=temp_x、temp_r_flip=temp_r建立别名，随后原地翻转空间前缀，使原分支输入一起改变。候选修正仅为两处.clone()，新增参数0；尚未修改原生运行源码或训练修正权重。

证据分层：① SHA绑定的原循环在CPU顺序敏感探针算子下，两次调用共享同一输入；复制翻转分支后与函数式双向参考输出最大差0，梯度在1e-12容差内一致。此处算子不是实际Mamba。② 使用真实底座权重，分别独立初始化原生与内存clone-only模型，在DepthTrack Train chair01_indoor的第一张图初始化、第二张图跟踪，各一次前向。两层TSG两模态的原生两次调用输入与输出差均为0，存储相同；clone-only两次调用存储不同、输入输出差非0。③ 原生进入融合层的[1,384,768] RGB/Depth空间token与其序列倒序逐元素相等，最大差0；clone-only不再如此，最大绝对差分别44.444160与23.387903。两侧输出query长度均4。

这是真实权重单帧内部数值诊断，当前/后续GT未打开，optimizer steps=0，未计算IoU或任何轨迹/基准指标。不同的单帧框或更高置信度不能当作收益。空间token倒序也不是物体旋转角度。TSG源码与实际Mamba wrapper、记录时机、哈希、退出码、日志/结果一致性已经代理复核；没有独立连接服务器，结论仅覆盖下载的单帧证据。

现有候选关联与M54读取器的局部RoI来自模型返回的TSG后search_rgb_tokens、search_depth_tokens及template tokens。因此发现触及实际下游输入，但尚不能证明它解释了M42—M54或某条序列的全部失败。后续融合与预测层仍可使用其他信息，不能把形式上的重复直接换算为ROB损失。

另一个独立源码事实：causal训练采样只保证搜索帧晚于基准模板；_sample_visible_ids使用random.choices，带放回且不排序。四张搜索帧不保证不同、按时间有序或相邻，dataset、processing、actor保留所给顺序，模型按其更新query。这不是下载权重训练历史的证明；仅排序也不等于预测crop递归训练。该问题不得与TSG两行修正同时改动后统一归因。

下一步先建立独立代码目录，做真实网络反向与显存契约，再冻结DepthTrack Train的训练参数范围、两组相同采样顺序和预算；原生控制与clone-only组从相同底座初始化，保持当前query、模板、搜索、语言设置。先完成对应训练与22条开发递归，保护门通过后再低22，之后同一bundle验证DepthTrack Test、CDTB和VOT full127。当前没有已训练clone权重，也没有新公开分数。

诊断服务器目录/root/autodl-tmp/sttrack_tsg_semantic_probe_v1_20260906，公开diagnostics/tsg_semantics_20260906。真实probe SHA为cf15f6edf82680d11b0541eeb40282115f1f3ec30a7c5052f495b9e2c847929e，结果SHA为3086d4f856aad970fc50345b4684c6f56561bc363d6a65ff304980bcf94a412a；运行模型源码SHA仍d62cd0b2e6b383fd2049212f22d62334d32ea972150871522b874515e57ecb13。

原生full127保持既有进程与旧源码继续运行，06:31:53 CST观察到1,548/1,765 anchors，无完成态指标；GPU0在M54结束后仅执行上述短诊断，GPU1原生全量继续。预计约07:45—08:15附近完成，仅为按当前吞吐估计；下一次查看安排约07:30，若仍未完成则240秒间隔。DepthTrack Test/CDTB原生完整结果仍引用5.63—5.65的同一底座实测，不拿历史SRTrack数字替代，当前项目目标未达成。

### 5.71 M55冻结配对微调并实际启动：训练入口契约与toy07既有尾部例外（2026-09-06）

本节承接5.70，已从方向分支诊断推进到真实DepthTrack Train微调。原生Control与Clone各有独立代码目录，逐组封存157个源码／配置文件；模型差异只有temp_x_flip和temp_r_flip两处.clone()，新增参数0。原生full127的源码和权重不修改。语言、候选关联和M54读取器均关闭，保留原有ViT/BSI、两层TSG、Mamba Fusion和Center Head。

两项训练检查分别完成，不能混为一个实验：固定chair01／cube04两个片段的contract_v2执行3次优化更新，验证真实网络反向、参数变化及显存；正式sampler_contract使用16个随机增强片段、8个微批次和2次优化更新，分别验证前期[1]及后期[1,.85,.7]保留设置。两组严格载入相同基础权重，初始张量和实际输入流一致。正式小检查数据流SHA均为0616c78386ad3df13d7591a13417d28913d30ace77e2151d677fe3c7072d41c7。以上均无保留训练权重、无开发或公开指标。最早探针把keep_rate误传为标量而在优化前退出，失败日志同样保留。

| M55两组共同训练设置 | 冻结内容 |
| --- | --- |
| 数据划分 | 既有63条fit与22条反复使用的development，按序列不重叠 |
| 基础权重 | cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98，逐张量严格核对 |
| 模型参数范围 | 原STTrack全模型；参数元素133,065,093，无新增网络层 |
| 模板／搜索输入 | 2个128×128模板、4个256×256搜索图；因子2／4 |
| 预算 | 15轮×2,048片段，30,720片段／组、3,840次优化更新／组 |
| 微批次／梯度累积 | 2×4，等效优化批次8；BatchNorm仍按微批次2计算 |
| 优化 | AdamW；主干LR1e-5、其余1e-4；weight decay1e-4；梯度裁剪0.1 |
| 损失／调度 | 四帧逐帧2×GIoU＋5×L1＋Focal求和；第10轮后LR乘0.1 |
| 权重选择 | 固定第15轮，不按开发集挑epoch |

这是已有底座上的有界微调，采样量小于原配置15×15,000，不声称复现原论文完整训练。原有带放回、未排序的causal采样和ViPT增强保持不变；不是预测裁剪训练，也没有证明解决自身状态分布错位。每批记录实际序列、帧ID、种子及增强后输入张量散列，完成后须比较两组整个数据流。自定义CUDA内核不保证逐位复现，不能把输入一致扩大为所有浮点结果必然相同。

**首个正式启动遇到的是已知数据契约遗漏。**v1 Control于07:52:46启动，完成20次优化更新后，以退出码1停止；新采样器在第164个片段读取toy07_indoor_320的第1406张RGB时失败。全63条fit普查确认只有该序列有长度差：连续1,367对RGB-D，GT为1,406行。24.152.3和既有analyzer已记录这一固定GT的39行尾部例外，本次新入口漏接。没有通过catch-all跳过错误。82个已进入循环的微批次中，80个完成累计优化，另2个未形成下一次更新；未保存完整epoch或最终权重。尚未启动的Clone队列被取消，失败日志、部分批数据及状态封存。

v2在新目录接回原契约：仅toy07且GT SHA仍为683e8ae7ae401b71b8d10e9bb489c3956a150163606f5bac925a911f395444e2、GT行数1,406、连续RGB/Depth帧数均1,367时，在内存中使用前1,367行；其余62条必须全部对应。全部原始GT与图像不修改，无通用截断。第160～167个真实片段的读取／增强检查覆盖原失败的第164个；其合法0-based search IDs为[1318,1319,1366,1338]。两组再次通过正式采样合同，划分、种子、预算和晋升条件均不变，并重新从同一原始底座开始。v1的20步不并入v2训练步数。

v2运行目录为/root/autodl-tmp/sttrack_m55_tsg_direction_v2_20260906，spec SHA为b53991ca364cbe767641fcc3216faa41e8bd6699e9ed1e59c0224b140da56998，训练器SHA为0025030f18647eeb047f2072df53007125c9f6019d0c762e278551e182f98d9a。Control于08:10:26在GPU0重新启动；Clone队列于08:26:27确认原生full127成功结束且GPU1空闲后实际启动，队列退出码0。08:45:55两组最近刷新更新数为550／300，控制器PID309414／313370均未出现退出文件。两组首轮各2,048片段、1,024微批次、256次优化，耗时987.33／984.79秒；首轮输入日志逐字节一致，SHA均为19a40d3005d03524b9c34d897f941d71d279d0076d034d4c14176f945113c7b2，初始张量散列清单也一致。初估Control约12:20、Clone约12:40完成，下一次训练检查接近第10轮约10:40／11:00，后续按实际吞吐修正。临时优化器状态每轮写入共享内存，机器重启会丢失，当前无自动恢复入口；最终模型将写持久磁盘并本地归档，不删除旧成果。

完成两组固定预算后，先全22条Train开发因果递归。Clone有效帧pooled mean IoU须至少比原生高0.005、比Control高0.002，IoU≤0.1帧与H10段均不超过另外两条路径，且原生H10为0的序列不得新增失败。统计使用既有函数，初始化和无效GT不计入均值，无效GT中断H10连续段；完整预测封存后才打开后续GT。全部门通过才冻结低22；低22明显改善并保护成功轨迹后，再用同一个权重／运行策略验证DepthTrack Test、CDTB与VOT full127。本轮没有完成态训练、递归收益或语言收益。

完整计划、代码、两阶段小检查、v1数据失败与v2修订证据见projects/sttrack_lachtt_v1/diagnostics/m55。审阅采用同GPT模型族Type-A advisory，只读本机字节证据；首次审阅对2步／3步的疑问经补充独立sampler合同后撤回，无需改已冻结spec，原始审阅及澄清保留。不把局部审阅称为跨模型族复核或完整训练验证。

原生full127已经完成，完整数字与复核见紧接的5.72；不能把它记作M55训练结果。训练完成后先封存两组权重、核对全部输入流，再运行固定22条开发递归，未通过既定门不得提前晋升。

### 5.72 原生STTrack完整VOT正式结果及同底座三数据集参照闭合（2026-09-06）

原生STTrack default full127完成，evaluate、finalize和controller退出码均为0，官方VOT 0.7.1分析成功。覆盖127条序列、1,765个multi-start anchor和5,295个预测文件；其中303个复用M39封存轨迹，其909个文件保持字节一致，另1,462个anchor新运行。确认失败为183个，完整per-sequence及anchor outcome已封存。语言关闭，无新增权重，优化步数0。

| VOT完整原生指标 | 实测（%） | 目标：严格大于 | 差值（百分点） | 通过 |
| --- | ---: | ---: | ---: | --- |
...
ROB即使显示为一位小数93.7，仍未严格超过93.7。相对旧SUTrack identity-only full127，本次EAO＋3.301071、ACC−0.108154、ROB＋4.103458个百分点；这是基础模型与相应运行路径的比较，不能算作语言、M54或M55收益。183是确认失败数量，ROB采用官方序列长度加权，不能用简单的失败比例代替。

此前5.63—5.65的原生DepthTrack Test、CDTB结果与本次VOT的基础权重、原生模型、tracker及配置SHA一致，因此现在可以完整列出同一个原生STTrack参照，而不再拼接历史SRTrack数字：

| 数据集 | 顺序 | 同底座完整指标（%） | 本数据集全部目标通过 |
| --- | --- | --- | --- |
| DepthTrack Test，50条 | P／R／F | 62.415336／62.682004／62.548386 | 否 |
| CDTB，80条 | P／R／F | 69.933113／68.067744／68.987821 | 否 |
| VOT-RGBD2022，127条 | EAO／ACC／ROB | 77.321654／82.471190／93.669109 | 否 |

共同基础权重SHA为cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98。VOT result SHA为51213ef55085d270a56ae2952b8871c06557891d418ae572ec3fb932102f1d8b，official analysis SHA为6159ce97ab294129599435265567a2beb987788dea835d4c5974a068cc564ff4。服务器运行目录/root/autodl-tmp/sttrack_default_full127_v1_20260905；公开结果、协议代码、逐序列失败表与绑定见projects/sttrack_lachtt_v1/diagnostics/native_vot_full127。

本机私有归档41,882,265字节，SHA为92cd1421002ad0915d07bd6cd5e0dd03544ef59554ad5b9b2fbb46b555d76c90，全部7,231个文件的大小和SHA匹配。独立同GPT族Type-A advisory再次核对所有本机绑定、预测文件、复用文件及计数，没有发现具体缺陷。EAO按返回曲线的官方切片重聚合，ROB按已存outcome及序列长度重聚合，均与官方值一致。该审阅没有重新计算GT与预测的栅格重叠，没有重跑跟踪，没有检查实时服务器或远程原始M39路径；不能扩写为这些范围已复核。公开包未再分发原始图像／GT／轨迹或权重，私有归档保留这些GT和预测证据。

下一步仍是完成5.71的两组DepthTrack Train微调，先固定22条开发因果递归，再按冻结门进入低22及新权重全量验证。原生VOT接近目标并不替代DepthTrack/CDTB缺口，也不表示用户要求的新训练三数据集模型已完成。


### 5.73 M56：文本主线恢复、稳定属性局部交互三组训练完成，静态结果尚无独立语言优势（2026-09-06）

用户进一步明确：强原生STTrack是当前比较起点，主要创新应是文本与RGB-D对实例身份的持续帮助，可以接受幅度不大但可归因的提升。三个候选研究方向为：实例绑定的三模态语义交互、语义对比驱动的时序实例关联、身份—视角解耦的语义记忆。它们均为待验证假设，不能把通用attention、TSG修复、模板阈值分别包装为已成立的三项贡献。

#### 5.73.1 基线范围与M55/M56关系

当前原生同权重完整结果是DepthTrack P/R/F=62.415336/62.682004/62.548386，CDTB=69.933113/68.067744/68.987821，VOT EAO/ACC/ROB=77.321654/82.471190/93.669109。VOT接近原目标，但另外两数据集仍未达标；历史SRTrack成绩不属于当前STTrack。

完整VOT与低22的57.135993/75.719622/73.022401相差约20个EAO百分点，是评测范围不同，不能归为改造损失。真实同集合退化包括全局停更使低22失败124→155、ROB下降6.394183个百分点，以及M48/M50/M54完整递归中的伤害。

M55仍是原生Control与两处TSG独立clone输入的配对全模型训练／验证，不含语言。截至本节所记M56 CPU完成与排队时，M55没有完成态训练及递归收益。已有M55完整开发递归runner/spec准备完毕，尚未执行。最终底座需根据该配对验证决定；内部语义修复不自动晋升。

M56作为现有原生缓存上的独立小试验，始终绑定原生base SHA `cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98`，没有混入M55参数或特征。若最终采用新底座，语言模块重采集、重训练；当前M56不能跨底座迁移其消融结论。

#### 5.73.2 本轮实际实现与文本协议

新增`lachtt_attribute_candidate_set.py`，在原候选集合头的cell投影后、对象展平前插入短语读取局部RGB-D及局部格点读取条件化短语的交互块。新增35648参数，头总参数484387，冻结STTrack与CLIP。保留top10候选、原几何、当前候选0／NONE语义及默认50步且置信度严格大于0.75的模板更新。

这还不是完整的初始化实例绑定：没有先用仅初始化RGB-D建立持久实例语义参照，也没有属性相关的模态职责／可靠性、显式竞争性语义margin、持续ID传播或身份—视角记忆。当前文本字符串不在线更新，没有Qwen动态caption或真实物体ordinal。

复用既有DepthTrack Train的类别与稳定属性字段，85条（63fit/22development）编码为173个不同短语（含空串），最大5槽，每槽冻结CLIP ViT-L/14输出768维。原始rich标注SHA `56c03871b8e2a005bbd9ca12c32ed8821c5445d16af9e2631963267b89785b58`，文本库SHA `93cd72856177f338d14ae05959459e36bf3082ddda340e9b332e5a33089ae967`。7条具有历史跨帧／裁定复核来源，其中development为bag05和egg；不把该协议说成严格自动首帧注释验证。

cat04的唯一初始化bbox差异来自GT底边超出640×360图像5像素，旧文本crop恰好是图像边界裁剪；已记录，未修改GT。准备器解析既有混合标签文件后只物化1511条fit数值标签，训练器只读取fit文件。训练仍使用原生预测状态缓存、previous_choice全0，没有声称补齐M52的策略状态分布。

#### 5.73.3 实际配对训练与静态结果

三组分别为attributes（每个类别／属性独立短语）、pooled（完全相同短语均值，单有效槽）、empty（有效CLIP空串，单有效槽）。使用同参数、同seed2026初始状态、同事件顺序，20轮、batch32、AdamW lr3e-4、weight decay0.01、梯度裁剪1.0，960步／组，固定最终轮。全部三组最终权重封存后才开始开发数值分析。

训练于10:09启动，在CPU 4线程上完成。三组实测训练时间约187.4、185.7、182.8秒；train/static/controller退出码均为0。训练spec SHA `cafc4471d87c8c8de5c9cb051ae659f1d14404691d2d178cff2131fa7f30a45e`。

| 590个重复开发缓存事件 | 平均IoU | IoU≥0.5 | 严重原错→正确 | 原正确→严重错 |
| --- | ---: | ---: | ---: | ---: |
| 同状态原生候选0 | 0.440274 | 268 | — | — |
| attributes | 0.450518 | 275 | 7 | 3 |
| pooled | 0.451725 | 277 | 7 | 0 |
| empty | 0.450667 | 276 | 7 | 0 |

590事件中495个GT有效，95个按既有M45缓存评估口径记零IoU／NONE；不能与完整递归排除无效GT后的mean IoU直接横比。

固定attributes权重：空文本0.449481，整套文本错配0.450700，仅7/590个实际候选选择发生变化。9条同类颜色标注冲突子集共225事件，真实0.472446、冲突0.468270；同短语数的6条144事件，真实0.499052、冲突0.492527。局部正信号没有形成属性组超过同预算空文本／池化的总体证据，不能宣称语言优势。

三组head SHA依次为：attributes `c68818cc78aacb05d7e03d3523d732032b274ed664b841a84380638eaa28c48e`，pooled `ce38d9ab614e7c414e66eab97a9984870254db1c1db8ff3e9e1a33e236e7bceb`，empty `64c8eb2eb72d4a3d5c5bc969805d21a6519bf9d97a50c5fa03c7dfd59ba40ca5`。

必须保留两类归因限制：单槽pooled/empty没有与多槽attributes相同的有效注意力容量；打乱22条中15条换类别、10条换短语数，冲突9条中3条也换短语数。相同覆盖、同短语数、同类别等分层在训练前确定并完整报告，不能只选择最有利子集。

#### 5.73.4 完整递归排队与后续三项研究

三组各22序列、33130帧的完整因果递归已封存spec，SHA `bdc367d05dca2ea1bedbde9fa7ab3a7891ea3eb4bbddc15afd0ca16678e6cde5`。10:26排队：GPU0在12:15以后检查资源，运行attributes、empty；GPU1在12:35以后检查，运行pooled。若仍被M55占用，则每240秒检查，显存低于500MiB才启动。排队不等于GPU执行完成，未产生M56正式VOT或三数据集结果。

完整递归门相对独立原生轨迹、empty和pooled同时成立：attributes有效帧均值至少分别+0.002/+0.001/+0.001，低重叠帧与H10不增加，并保护原生H10为0的序列。静态未显示属性优势不替代这项已冻结比较，也不会触发自动公开评测。通过后仍需文本反事实与冻结低22，再以同一最终模型验证三数据集。

后续三个方向按序研究：先建立初始化实例绑定并证明内容级语义增量，再让竞争语义进入实际跨帧身份传播，最后引入语义约束的视角记忆读写。每一步有同底座、同容量的视觉／文本对照及完整递归验证。UVLTrack、QueryNLT、ATCTrack、KeepTrack、ROMTrack、MambaVLT的相关机制已作为近邻约束记录在`diagnostics/m56/LANGUAGE_RESEARCH_DIRECTION.md`；命名不能替代新颖性与实测证据。

#### 5.73.5 审阅、归档与公开范围

按用户要求，当前实验审阅skill及共享路由已改为`gpt-6-astra`、`reasoning_effort=max`；历史GPT-5.5审阅的真实元数据保留。Astra对语言方向、启动前代码、静态／递归接线分别给出有范围限制的审阅；属于同家族Type-A，不称为跨家族独立验证。

真实fit缓存工程检查通过零残差logits/affinity精确保真、padding不变、短语置换误差约3.34e-6及有限反向；检查权重丢弃。参数变化可能包含AdamW weight decay，不能据此声称各子分支都获得非零数据梯度。队列成功标记提前写入的审阅WARN已在首次启动前修正，原始审阅及处理说明均保留。

完成态CPU私有归档为36个文件，归档SHA `f5706dc87cb93388f91882a8e4f730c263363f39b092bd963c9572c5e1281c60`，下载后逐文件字节及SHA全部匹配。公开行聚合验证复核了三组初始化／顺序／预算和六种静态干预，未重新计算原始候选与GT重叠。公开目录为`projects/sttrack_lachtt_v1/diagnostics/m56/`，不分发图像、GT、原始特征缓存、CLIP bank或训练权重。


### 5.74 M55：最终权重递归入口补齐绑定，两组实际队列已启动等待（2026-09-06）

本节承接5.73。M55仍是原生Control与两处TSG clone版本的同初始化、同数据和同预算训练，不增加参数或语言。11:08 CST核实原始训练PID309416/313371仍活跃；Control为第11轮/2750步，Clone为第10轮/2500步，均未产生完成态结果。Control已按固定计划降低学习率，Clone尚在第10轮。此处没有新增VOT、DepthTrack或CDTB指标，也不将训练loss用作晋升依据。

递归入口的原v1只验证result与权重SHA，未读取权重内部已保存的variant、spec、epoch和steps。Astra/max审阅给出两项绑定WARN；现仅补充内部元数据、固定最终路径及execution_binding→trainer/model source/initial_state的一致性检查。两组结构兼容使成功加载不足以单独识别组别；本次没有发现实际混组。训练源码、训练spec、输入和既定性能门保持不变；旧runner/spec已封存，源码delta复核PASS不是最终权重或GPU递归验收。

| 实际排队任务 | 前置依赖 | 固定评测范围 | 11:08核实状态 |
| --- | --- | --- | --- |
| GPU0：M55 Control | M56 GPU0控制器终止、GPU空闲、两组M55训练正常完成 | 第15轮最终权重；22条/33130帧 | 队列PID322434存活，尚未推理 |
| GPU1：M55 Clone | M56 GPU1控制器终止、GPU空闲、两组M55训练正常完成 | 第15轮最终权重；同一22条/33130帧 | 队列PID322595存活，尚未推理 |

队列13:00 CST开始每240秒检查，显存低于500MiB才启动。前置M56是否达到性能门不作为M55条件；它只决定资源何时释放。GPU0在两组预测封存后统一分析，分析成功后才写队列成功标记。所有后续GT只在封存与来源核验后解析；本轮没有自动低22/full-127操作。计划时间不等于训练完成证明。

初次外层启动因`screen -DmS`不fork而等待GPU0，已停止该启动器、保留原GPU0队列，再用`screen -dmS`启动GPU1。训练没有重启，GPU0没有重复创建。纠正收据、14个服务器来源文件SHA、两次原始审阅及新旧入口均已归档。

服务器M55目录仍为`/root/autodl-tmp/sttrack_m55_tsg_direction_v2_20260906`。新递归spec SHA=`e05a9814371f5974387f8fa9bbe259a3b74b4c062391ae461f7f45625f6094e2`；runner SHA=`56f4604b1e1ee94c201d3285ffe6baf1924e48887a1ee2b9a884c1c11988a8bc`。公开来源在`projects/sttrack_lachtt_v1/diagnostics/m55/recursive_preparation/`。

下一步接收两组固定最终权重，验证配对训练来源与完整递归，不根据中途开发指标选epoch。最终底座由完成态证据决定；若变化，重新采集特征、训练文本模块。文本的实例绑定、语义时序关联和身份/视角记忆继续作为研究主线，同一最终权重及运行协议仍需验证三个完整数据集。


### 5.75 M57 初始化实例绑定准备：同槽2×2原型与CPU接线验证（2026-09-06）

本节新增未训练原型及工程证据，不新增跟踪分数。M55仍未完成底座配对验证，M56完整递归排队状态不变；12:10 CST实查两组原训练PID309416/313371仍在，Control第15轮3675/3840步、Clone第14轮3450/3840步，未出现最终完成记录。下一步继续按既定队列完成因果递归后决定底座，不能按中途loss选择模型。

### 架构与最小问题

当前NativeReferenceBank.initial实际在第一次regular track才冻结，是受t1搜索图交互的模板段ROI。M57另写初始化帧提取辅助函数：用同一t0 RGB-D图和合法初始框执行raw forward，传query=None，仅提取template0的2×16×768 ROI，丢弃返回query；未来接线只赋bank.initial，不调用before_decision，以免同时改变dynamic/encoded_dynamic。本次没有执行真实GPU提取，也尚未接入运行tracker。

新交互类保持文字读视觉、局部格点读条件化文字两次注意力。对照为initial/candidate视觉来源×真实文本/同槽视觉查询，四组计划获得相同t0初始参照、相同动态参照及10+10候选，只切换第一次注意力K/V来源。initial使用交互前object20；candidate使用各对象自身。第二次注意力Q始终是各对象自己的格点。四组均484547参数（448739父头+35648交互+160槽查询），不同槽随机初始化而四组完全共享起点。视觉查询组保留原mask、有效槽数和顺序，只去词义内容，不能称为完全无语言元信息。

### 工程验证表（不是跟踪指标）

| 项目 | 实测／状态 |
| --- | --- |
| CPU输入 | chair01_indoor的22个旧fit事件循环成32样本；旧reference为t1，仅作接线检查 |
| 4组初始张量 | 完全相同，摘要234b6c97a3f53bd0832679794c9e9f37b749555daa36b818e550dafb3557395a |
| 零残差父候选头保真／padding | 两者均逐元素完全一致；不是完整原生轨迹保真 |
| 不同槽／有效查询 | 初始槽最小距离0.143845，本批4有效槽；真实和空内容查询均不同 |
| 逐对象来源探针 | 4×22=88项通过；精确核对被影响的对象集合 |
| 数据梯度 | 第2/3临时优化步各交互组与两次注意力Q/K/V均有限非零 |
| 临时优化预算 | 每组3步，全部丢弃，没有保存checkpoint |
| 真实t0提取／正式特征／正式训练／开发或公开评测 | 均未执行 |

v1检查曾返回pass，但合并扰动和整层梯度不足以排除错误对象来源或Q/K未学习。收到独立审阅中途WARN后，执行者补为逐对象非均匀扰动、槽差异及Q/K/V分片检查；v2通过，模型源码没变，两版产物并存。v2 checker SHA7623effc675f47081259e345045949271714b247473efa4ca65394e257933938；模型SHA59af4516e854e686dc0425c7e57dc70df7238f7afbd12e68fb170df6eaf06746；提取器SHA2f63f49f90b08f3e205c9bb48cb22aad1d800b6bc7d1d823746c638bf9ffddde。17文件证据包SHA2e6846a9398b5580303efc96d9caf8739cdb7753af5df8c2838d821f276a3da0。

### 审阅边界与后续

gpt-6-astra/max完成方案审阅；实现审阅返回具体WARN后因服务额度限制中断，未返回最终报告，修订后的独立delta复核未完成。当前只能记为部分审阅／WARN，不能把执行者的CPU通过写成独立审阅PASS。原方案响应、中途意见、错误和两版检查均在diagnostics/m57_initial_binding_preparation归档。

后续先接收M55/M56完整递归，冻结最终底座及t0提取协议，再验证真实初始化前向对bbox、模板、query、frame_id、网络及随机状态的影响，并验证后续默认模板写入不改变初始参照。正式缓存与runtime必须同协议；换底座则重新采集。本原型尚无持续ID、属性模态可靠性、在线文本或身份/视角记忆，不代表三个语言贡献完成。新增参数在DepthTrack Train训练，完整开发递归通过后先冻结低22，再以同一base/head/runtime/text协议评测DepthTrack Test、CDTB、VOT full127。


### 5.76 M55两组固定最终权重训练完成并核验，M56已接续递归（2026-09-06）

Control于12:20:17 CST、Clone于12:35:30正常完成；两组训练进程和外层控制器退出码均0。仍是已冻结的原生Control／两处TSG clone同预算对照，没有语言或新增关联头。本节仅报告新完成态事实，不重复训练超参数，也没有新增三个数据集的跟踪指标。
...
上述loss是训练四帧定位／响应损失，不能替代完整递归或据此选择底座。完成态导出复用已冻结递归runner的binding，实际读取两份checkpoint内部variant/spec/15轮/3840步，核对权重、初始化、执行绑定、157个源码文件和日志SHA；再解析实际日志，确认连续3840步、15360个microbatch、30720个唯一(epoch,index)片段及63条fit覆盖，没有development序列。两组完整采样记录与输入张量指纹、各轮数据流均相同，模型计算差别确实只有两处clone。

Control权重SHA=`fe1d4b2faddc7f0dc523b6ec9cacbbf878f523cfc1985cd2a0d8a3fb4c4ba284`；Clone权重SHA=`d1ce44c82593fe5f508954a536762ba9ef772e9264da12500e62f80064e46c81`；每份532399447字节。共同完整数据流SHA=`7990c9ae35381ad10ac5d99baa12988f6587ec960546ddbfa2499509db96200a`。最终权重仍在M55 v2目录training/{control,clone}/model_final.pth；完成态原始结果、逐轮逐步统计和检查器归档至diagnostics/m55/training_completed。原始权重及完整采样batch记录未随代码分发。

首次导出器因Control原赋值行等号后的双空格而在字符串还原比较中误报；核对真实diff后仅修正导出器的比较文本，v2通过。原始失败、修订日志与退出码均保留；没有更改或重跑训练、权重或冻结递归入口。本次是完成态绑定及日志核验，没有新增独立性能审阅，不能将已有源码审阅扩大为新权重效果背书。

12:40 CST实查M56 attributes（GPU0 PID326519）和pooled（GPU1 PID327331）已运行，分别封存11/22和1/22条序列预测；empty仍等待attributes结束。这只是进度，不是部分性能分析。M55递归队列PID322434/322595仍等待对应M56外层控制器终止，按既定240秒间隔检查后，各跑22条/33130帧，再统一读取后续GT。全部轨迹通过冻结门后才进入低22比较，当前尚未决定底座、未启动M57正式采集或训练。


### 5.77 M57真实初始化提取验收脚本准备；M56属性组递归完成（2026-09-06）

M56属性组已完成22条/33130帧，递归退出码0，实际耗时1976.563秒；接续空文本进程PID328005已启动。12:59 CST实查池化组PID327331仍在运行。尚未计算三组配对指标，没有根据部分序列调整权重或门槛。

M57新增真实GPU验收脚本准备，尚未执行，也未选底座或生成运行spec。计划固定chair01_indoor一个fit前缀，三条路径分别为原生、开启特征观测和原bank、观测路径加t0参照提取。每条102帧（初始化+101步），比较框、置信度、query、模板及t0前后网络注册状态/随机状态；第50/100步若更新模板，须核对第51/101步实际读取新模板时初始参照仍保持。若没有实际写入，覆盖字段为false，不能把模板写入部分当作通过。该短前缀不证明语言语义或长期跟踪收益。

原M44输入清单含expected_rows等历史预测，本准备已提取仅含sequence、split、frames、init_bbox的单条初始化清单，运行脚本拒绝其他字段；新进程无需获得原清单中的后续预测。清单SHA=`2bff3239f6a3a43a718ec53d7fc0c50a06c727d77168fd99c29591c917e4f981`，检查器SHA=`272fb28aa96e86f47d4354360e58e8f3dbca3de79441a55e5808f95e680e3598`。已通过Python3.8语法解析，并确认必要观测/预处理文件存在于两份M55快照；这不是GPU状态保真验收。

准备材料归档于diagnostics/m57_initial_binding_preparation/t0_validation_preparation；正式spec仍须在底座决定后绑定实际checkpoint、完整源码、提取器和初始化清单。当前没有新的独立审阅结论，不启动正式特征采集或训练。继续等待M56完整配对分析与M55递归结论，再推进语言实例绑定。


### 5.78 M56完整递归完成：属性交互有相对收益，但未通过原生保护门（2026-09-06）

三组固定最终权重各完成22条/33130帧，推理、两路控制器和分析退出码均0；独立原生轨迹按冻结SHA复用。完整预测封存后才打开后续GT。新增结果如下，不能与590个静态事件或正式VOT三指标混用。
...
四组均有28897个有效评测帧，排除初始化与无效GT；H10为连续至少10个有效帧IoU≤0.1的段，无效GT会断开。属性组相对池化/空文本分别提高6.754/4.789个均值IoU百分点，但相对原生只+0.117个百分点，未达到冻结+0.002门槛；H10和原生成功序列保护也未通过，新增失败为mobilephone02。本版停止晋升，没有运行低22或三数据集全量。

这次完整递归确实出现了属性方案相对池化/空文本的明显差异，不再仅停留在静态比较。但pooled/empty有效槽数为1，attributes多槽；两者同时改变词义与有效注意力结构。固定属性权重尚无完整递归的空文本/错配文本干预，已有静态错配负证据仍保留，因此不能将4.789个百分点全部写成纯文本收益。

属性组11条均值上升、9条下降、2条相同。ball07 +45.320pp、book06 +14.215pp、flower02 +12.109pp、glass03 +9.459pp；egg -26.379pp、mobilephone01 -17.072pp、mobilephone02 -28.590pp抵消了大部分收益。全部22条、三组和原生的均值/H10、低重叠帧均已保留在diagnostics/m56/recursive_completed，不能只报告成功个例。

对全部66条完整轨迹作后验时间诊断：首次bbox差异（>1e-6）均与首次非默认改选同帧，未观察到未改选先偏离原生；无改选路径保持一致。egg第6帧第一次改选将原生IoU0.905881换成0；mobilephone02第266帧第一次改选时两框仍正确（0.882225→0.702984），后续第497—700帧形成204帧连续低重叠，全程10次改选。这里是时间关联而非取消首次动作的因果实验，不能把231帧后损害全部归因于首次改选；未重放模板/query来确认单一根因。帧号从0开始。

完成态绑定复用冻结runner，66预测共99390帧SHA全部核对。完整结果SHA=`36123474fec5ac4e06ec551f3417828d7d25a02e6b98e270dbc5c628eebaf3e6`；私有完整归档SHA=`16a3e07ff3ba90302115c567197f179b69fc62e9df6d58ae79ca0e49c9dad201`；后验诊断SHA=`082aab2b7c4440d5f7dfff62b6d3366659c28029151a11476b26becf49c4e25b`。逐帧IoU和H10区间与冻结统计一致。本次没有新独立结果审阅，已有Astra源码审阅不扩写为新结果背书。

13:32 CST实查M55 Control PID329564已启动，Clone PID328685仍在，两组底座比较尚未完成。M57仍只完成原型和验收脚本准备，待底座决定后进行真实t0验收及同槽2×2正式采集/训练；不混用不同底座的表示缓存。文本实例绑定、时序消歧、语义视角记忆仍是研究主线，不因本次部分正信号就宣称三个创新已成立。


### 5.79 M57原生t0提取真实GPU验收完成，覆盖一次模板写入后的读取（2026-09-06）

利用M55 Clone结束后释放的GPU1，提前完成固定原生参照的短工程验收；正式采集/训练仍等待M55底座决定。实际使用冻结Control代码快照的157个文件配原生checkpoint，并非微调后的Control权重；与M56共有的10个冻结来源文件一致。如果最终采用Clone，须对Clone重新验收。

| 已执行项目 | 实际结果 |
| --- | --- |
| 数据与运行范围 | chair01_indoor合法初始化；三条路径各102帧，共306帧 |
| 原生／观测+t1／观测+t0前缀 | 框、响应、query、模板、frame_id逐项完全一致 |
| t0提取前后状态 | bbox、模板对象、网络注册状态与随机状态均保持 |
| 实际默认模板写入／下一帧读取 | 第100步／第101步；初始参照保持 |
| 后续GT／正式特征／训练 | 均未使用或执行 |

13:49:34启动，13:50:05正常完成。第50步未写入，本次只实际覆盖一次写入。首次预检指向未安装的overlay提取器，启动前停止；随后改为现有准备目录中相同SHA的提取器，源码未改，也未添加fallback。t0参照为有限无梯度2×16×768 float32。本结果限定于一个拟合前缀，不证明语义定位或长期性能。

实际spec SHA=`fb8887ad905b268311b2cd9c1c892c8e4ca56bb0d6ec77e3d495b9257713dc03`；GPU结果SHA=`84939bf0ac600fe09fa29ac183564fc9fbb9b2d5ce9fe212d9284ff7122feb85`；13文件私有归档SHA=`308f029ed312e45dace9cb2092dabf462f9a21d528acb5d0f92c0b11776e51cb`。下载后逐字节核对，三份原始rows直接相等。公开记录位于diagnostics/m57_initial_binding_preparation/native_t0_completed，没有新增独立结果审阅。


### 5.80 M55两组完整递归完成，保留原生底座推进文本实例绑定（2026-09-06）

Control与Clone两组固定第15轮权重均完成22条/33130帧，推理、外层控制器及统一分析退出码0。全部预测封存后统一打开GT，复用独立原生轨迹。两组训练来源已经在5.76记录，本节不重复训练超参数。
...
三组均为28897个有效评测帧；初始化/无效GT排除，H10是至少10帧连续有效IoU≤0.1段，无效GT断开。Clone有效帧均值相对原生+2.487pp、相对Control+0.821pp，低重叠帧减少944/214，但H10高于原生和Control，新增flower03、mobilephone02失败，冻结晋升门为false。本轮没有低22或正式三个数据集的新分数。

Clone的序列等权均值反而低于原生和Control，收益分布不均匀。Control也破坏这两条原生成功序列，不能把全部共同退化归因于clone。Clone在ball07/car01/car02/colacan01改善明显，却在flower03下降约58.18pp、mobilephone02下降29.01pp、cup13下降29.71pp；全部22条原生/Control/Clone的均值、H10和低重叠帧已公开保留。只有单seed配对，不宣称统计显著性；这是视觉实现修正，不是语言贡献。

完成态导出重新调用冻结runner的binding，核对真实权重内部元数据、训练执行绑定、初始状态、两组157文件源码及完整日志；44预测共66260帧SHA一致。结果SHA=`a2ee9b85208010ca97003052cd1fbaab0520d3a542a1ebf328862aa2d6db0e7b`；私有归档SHA=`ae3d02956f79007f1ca031769f6ff0bf288b01bf7893c0f599a0553af0c637f6`。公开完成态位于diagnostics/m55/recursive_completed，未新增独立效果审阅。

M57正式保留原生checkpoint `cacbd799115be1aaeb049cee0db89270851e3b6dd68997553b4c2c31c1104f98`，不采用M55或M56新权重。机器决定位于服务器sttrack_m57_initial_binding_v1_20260906/base_decision.json及公开M57准备目录，SHA=`bdeaa840a47dca0588bca214829150f1dee79b870c2258bcb942cb0133f79c1b`。该决定保留了已知原生TSG别名语义，不能把两方向当独立证据；没有推断其他训练下修正一定无效。原生t0提取已完成5.79验收，下一步冻结同槽2×2正式训练与数据协议。当前正式采集/训练尚未开始，同一最终模型三个数据集目标仍未完成。


### 5.81 M57正式完成因果t0采集、同槽2×2训练与真实输入验收（2026-09-06）

按5.80保留原生STTrack，固定172文件代码快照与原始checkpoint。85条Train序列各读一个合法初始化RGB-D帧，额外前向query丢弃；t0 RoI为2×16×768，依运行half().float()协议分别保存63 fit／22 development。新增两份参照约4.2MB，采集14:30:25—14:30:39正常完成，没有读取后续图像或GT。只有RAM内references[:,0]替换为t0，旧原生缓存文件不修改。

正式运行类STTrackInitialInstanceCandidateSet复用原候选跟踪路径，只安装t0及同槽文字。四组在chair01的1002帧强制沿原生候选0运行，22事件的真实输入与训练装载逐项完全相等，包括候选、初始/动态参照、几何、分数、上一选择、文本与mask；公开框也精确一致。4组共4008帧，耗时258.928秒。此为接线契约，不是学习后完整轨迹保真；此前t0模板写入/读取验收见5.79。

四组candidate_text、candidate_visual、initial_text、initial_visual各484547参数，只干预短语视觉源与真实词义内容。四组都获得相同t0和5槽，视觉查询组保留槽数/顺序/mask等语言结构元信息。STTrack与CLIP冻结，新增头GPU0 float32/noAMP，seed2026、20轮、960步、batch32、AdamW lr3e-4/wd.01/clip1；63 fit／1511事件，同初始化和顺序，固定第20轮。每组实际优化约40—42秒，训练退出0。四权重内部variant/mode/spec/base/textFit/t0Fit/20轮/960步已核对，开发数字打开前封存。
...
训练spec SHA=`e84d477c7a0cb55498809a84e4937cb735f4bc9b702e39b683b8d5088d76635f`；真实输入契约SHA=`902b5a5089252bc090d557674d2e96708d918da41cf8cbfe5d0d3ba25be5f41d`；四组事件顺序SHA=`27439ed4334bb955089ca1580578f4831171742b46bc9ad832dcb945b29fd079`。训练只读取fit text/t0和数值标签；仍是previous_choice全0的原生轨迹缓存，未解决M52分布问题。

这是候选RoI上的初始化实例条件化，不是语言已进入主干生成，也没有完整ID传播、属性可见性或视角记忆。沿用M56文字及七条历史复核来源限制；没有在线Qwen/序数/逐帧caption。原生TSG别名限制保留。独立Astra/max仍因额度限制没有新的完成态审阅PASS。

### 5.82 M57静态文字证据混合，四组完整递归已启动（2026-09-06）

全部四个权重封存后完成590个既定开发缓存事件的静态诊断，程序退出0。495个有效GT，95个沿历史口径赋0；下表不是完整递归或正式VOT/DepthTrack指标。
...
initial_text比默认静态候选略好，但低于initial_visual约0.2244个百分点；静态“初始化词义增量减候选词义增量”的差分之差为负。因此不能把这轮当前结果写成初始化绑定已经带来独立语言收益，也不以静态结果改门槛或更换主实验。

固定权重/原始mask的内容对照如下；同mask组内循环错配覆盖21条561事件，cup13无配对，只从错配中排除，真实组在同一覆盖重算。
...
initial_text清空词义改变9/590选择，错配改变13/561；错配中有3个真实词义正确而错配严重错的事件，反向0。说明部分选择依赖文字，但不足以证明文本模型优于同结构视觉头或完整轨迹改善。错配可改变类别，不称为已标注同类错误属性。所有四组、八种模式及全部22条静态均值均公开，不能只报这几个局部正例。

14:53:56两张GPU开始完整递归，各顺序两组，共132520帧。GPU0 initial_text→initial_visual，GPU1 candidate_text→candidate_visual。新运行类每条序列因果重算t0并和已采集参照核对；自身预测生成后续crop，默认模板/query策略保留。原生参照是独立冻结轨迹。静态只在权重封存后读取缓存标签，不传入推理；四组完整预测封存后才读取逐帧GT汇总递归指标。后台依赖检查每240秒，全部退出0后自动调用冻结分析。

主实验initial_text须mean相对原生≥+.002、相对initial_visual与candidate_text各≥+.001，词义增量差分之差>0，低重叠帧/H10不高于原生并保护原生零H10序列。通过后固定权重完整递归文本反事实，再冻结低22；有改善才用同一base/head/runtime/text协议跑三个完整数据集。当前完整递归未结束，没有新的低22或全量指标。

递归spec SHA=`c00665e1d8b12b7fd332d3c5c94514932b6183817a29ca69cf27a957e34eeb30`；静态spec SHA=`fc390a231f9a36790676ad00228edf62b6ba3a3dc026c72fdc00fe6be6103f5f`；静态结果SHA=`1987764e5db554232289366600cbc06c979767f105ca886d246b121a45057556`。57文件完成态归档SHA=`75abb049c198f8c53191fa71dcfb0f3d0ab0666e3d0fbcf4843bbcd08996e53e`，下载后逐文件校验并从事件行复核聚合。公开位于diagnostics/m57，权重、特征/text银行、图片和原始预测未发布。项目目标仍未完成。


### 5.83 M57后验审计收窄文字证据：同类可纠错覆盖少，明显正例混有类别与历史来源因素（2026-09-06）

在5.82已经封存的静态输出上追加后验子组分析，没有重新推理、改文字/权重/阈值，未打开本轮完整递归预测或逐帧GT。同类别子组是结果出来后划分，不称为预注册确认性实验。
...
四条为ball07、cup08、car01、flower02，分别从ball19、cup10、car02、flower03取得同mask的另一描述。initial_text选框不变不等于logits或属性表征不变。114中仅3个事件满足默认IoU<.5且oracle≥.5，只有cup08与flower02；两种文本头在这3个事件均无真实/错配选择变化。覆盖不足，不能据此说属性没用，也不能说同类实例消歧已充分验证。
...
三个明显正例实际为egg两帧与ghostmask一帧，错配均同时改变类别；不是三条独立序列或完整轨迹救回。egg原文字还带已披露的跨帧复核来源，这两次不能作为严格首帧自动注释协议的独立证据。这里只核对旧来源，未作新的属性真假标注；公开审计保留全部来源描述和子组覆盖。

当前1511个拟合标签中默认0为762、NONE为639、其他候选为110，其中候选1为51。此为标签分布，不是已证实致因；重复训练轮次不会增加独立事件。若M57完整主门未过，下一步应检验是否增加了同类属性区分监督及可纠错事件，不能只重复旧缓存、放宽门槛或把通用注意力称为已成立创新。M52扩充失败的限制继续保留。

两个文本版完整递归已正常完成，各22条33130帧，两个视觉对照已自动接续；四组全部结束前仍不打开逐帧GT汇总主指标。结果范围审计SHA=`60b16f801e75acfd80262ac000b482b60c69c31089ee8b08a76be01cb7dee8a6`，公开位于diagnostics/m57/static_content_scope。15:22:48在完整结果生成前冻结后续分叉诊断/完成态导出工具，当前仅完成Python3.8语法检查与来源绑定，未假报完整轨迹核验。项目正式三数据集目标仍未完成。


### 5.84 保留两个Qwen，清理已淘汰权重并核对运行任务（2026-09-06）

用户要求清理无用权重，随后明确两个Qwen都保留。15:48:34 CST逐项删除8个文件，共1,987,808,818字节：M55 Control/Clone终态权重2份1,064,798,894字节、旧SRTrack派生初始化权重826,888,229字节、M32 DINOv2权重88,283,115字节、M57未训练接线验收临时权重4份7,838,580字节。M55已完成配对训练和完整递归，但H10和成功序列保护未通过；当前M57使用原生STTrack权重，并未使用M55微调权重。M55源代码（包括M57复用的Control代码）、日志、预测和指标仍保留。已删除的M55权重若要重跑需重新训练，不再宣称可直接加载旧文件。

实验盘可用空间1,028,653,056→3,016,482,816字节，约1.03→3.02 GB。Qwen3-8B、Qwen2.5-VL-3B、原生STTrack、SUTrack、CLIP、DropTrack原始权重及M57四个正式训练权重均保留，删除前后大小与修改时间一致。旧SR派生权重的原始输入、映射清单、seed2026及构建工具保留。数据集、源码、日志、完整预测和评测记录未删除；M57两个视觉对照进程清理后仍正常运行。

删除前清单SHA=`ededc99ef7db2b94dae2807771d263e54bb0629af44aa916186785eb64ed6c43`。公开记录位于diagnostics/storage_cleanup_20260906，服务器原始清单位于/root/autodl-tmp/sttrack_disk_cleanup_20260906_1550。先验证文件与实验状态再逐个unlink，没有增加后台自动清理策略。

同时修正5.83尚未发布草稿的计数笔误：1511个拟合标签中，默认0为762、NONE为639、其他候选应为110（此前误写97），候选1为51。直接复核四组封存训练记录；实际训练数据、权重、输出和指标均未变。


### 5.85 M57四组完整递归完成：初始化文字绑定主实验七项条件均未通过（2026-09-06）

四组固定第20轮/960步权重各完成22条33130帧，共132520帧；四组推理、两路队列和统一分析全部退出0。所有预测封存后再读取GT。结果如下，五组各28897个有效评测帧，属于反复使用的DepthTrack Train开发集，不是正式三个数据集指标。
...
initial_text相对原生mean下降5.6891pp，相对initial_visual下降6.0512pp，相对candidate_text下降1.0121pp；低重叠帧增加1740，H10 75→80，并在原生零H10的mobilephone02新增失败。七项预注册门全部未过，当前版本停止晋升，不执行固定权重完整反事实、低22或三个数据集全量。其7条均值上升、12条下降、3条相同；全部22条和四组均公开，没有只保留正例。

candidate_text比candidate_visual高1.7639pp，但两者都低于原生；initial_visual相对原生+0.3621pp且低重叠帧少344，H10却升至94。词义增量差分之差=-0.078150906571，未支持这版初始化绑定。四组参数、预算、槽结构、t0均相同，但只有一个seed，视觉查询仍保留文本槽元信息；完整轨迹的同权重空文本/错配干预未做。该结论限定当前RoI头和1511个原生状态事件，不能说所有语言或实例绑定无效。

88条实际轨迹的首个bbox差异均发生在首次非默认改选，未观察到改选前的接线漂移；所有H10区间与冻结指标一致。mobilephone02第7帧改选时两框都正确（0.774563→0.566184），全程11次改选，后续第497—700帧形成204个连续低重叠帧，距首次分叉490帧，不能全部因果归于第一次改选。egg首次改选时两路IoU均0；flowerbasket首次改选帧GT无效，不能断言当时IoU。flowerbasket低重叠帧123→555而H10仍3，说明失败持续长度也必须报告。ghostmask均值+10.4715pp、H10 3→0，但局部收益不足以抵消损害。

服务器完成172个源文件、四个头元数据、88份预测及132520帧的绑定核验；逐帧IoU/H10诊断与冻结分析一致。私有归档42份记录+88预测，6,837,204字节，SHA=`4a3785958dc260e994a52781a976c707442ad36f58605ea377d41b854430c2d4`；结果SHA=`5ed4a20beae88a8e8b85513000a8f020311e934b9206678f7d670b50b0737064`；诊断SHA=`147cc6b5a2dec3c307ae14ba4c13346b66347e3fc95b5e495ba4744509275c35`。下载后全部记录/预测SHA匹配并复算聚合，未在本地重算逐帧GT。没有新的独立审阅PASS，Astra/max代理仍为额度受限状态；执行者核验不冒充独立审阅。

下一优化方向保持语言主线：检验文字能否参与密集搜索特征与响应学习，在DepthTrack Train原始RGB-D片段训练对应模块，而不是继续仅在1511个候选快照上调整分类头。先明确新增监督、同预算视觉对照及真实递归接线，再冻结训练；尚未实现、训练或证明有效。保留原生STTrack底座，模板默认更新和两个Qwen均保留；同一最终base/head/runtime/text协议仍须通过低22后验证三个完整数据集。项目目标尚未完成。


### 5.86 M58密集语义原型接入完成，三序列零残差路径与原生完全一致（2026-09-06）

在保留原生STTrack权重、原TSG方向语义、固定4个历史query、默认模板更新的前提下，新增289,154参数的SemanticSpatialAdapter。它读取初始化实例RGB/Depth的4×4局部token及最多5个CLIP类别/属性槽，通过短语与初始化局部证据绑定，在全部256个搜索位置形成密集交互残差，作用于TSG/Mamba之后、原Center Head之前。能影响center/size/offset，不是原候选top10重排。本阶段未接在线Qwen、运动或新模板策略，也未实现另外两个拟议身份/记忆创新。

零末层初始化保证初始输出为原融合特征。CPU合成两步检查三路投影均有非零有限梯度；这不是数据集训练。真实GPU同时独立运行OFF、零TEXT、零VISUAL三条路径：chair01、bag05、cup08各102帧，共306图像帧、918路径状态、909次track调用，47.35秒，退出0。三者bbox/响应/query/全部模板及裁剪图逐元素一致，OFF框还与既有独立原生前缀一致。chair01在track步100更新，cup08在50/100更新，均覆盖更新后的读取；bag05没有更新，不能计作写入覆盖。初始化RoI额外前向的query不写回。160个源文件验收前后核验一致。

VISUAL使用空CLIP词向量但保留相同槽/mask，仍含文本槽结构元信息。模态权重和属性注意力尚无可靠性/可见性真值验证。零残差接线通过不保证训练后安全，也不是新三数据集性能。actual_dataset_optimizer_steps=0。结果SHA=`a0ba39436647d5cdb603bc61f48acdbc6ad51aedfa24df4037958ebddb8b0bca`；集成快照SHA=`7d6bc4512ac6cdf6003aede134bcffa08fc56cab70fd965ec778b6993889f52d`；私有下载包33,637字节、SHA=`d8428af509331ae5d3be6e4d18e2c38814a8f5e884b5b723c0a6d187f18232d5`。

原始DepthTrack Train清单152条，拟用130条训练、沿用已反复分析的22条开发；比原63条多67条。全部首框有效、RGB/Depth帧名一致，toy07_indoor_320的39行GT尾部无图像沿用已知明确契约。仅初始化图像文字准备与因果预测裁剪训练仍待完成，正式预算/门槛应先冻结。M55是整网微调且采用GT扰动裁剪，并非TSG-only或预测裁剪训练；当前不混用其描述。M52一次策略缓存扩充失败也保留在结论中。

公开源码与验收位于projects/sttrack_lachtt_v1/diagnostics/m58/preparation；旧inventory/integration状态按时间保留，由随后parity完成态补充。无新的独立审阅PASS。原生base、两个Qwen和正式M57头继续保留，磁盘清理不影响本阶段依赖。当前目标尚未完成。


### 5.87 M58首帧自动文字完成，两组全序列因果预测裁剪训练启动（2026-09-06）

本地Qwen2.5-VL-3B对152条DepthTrack Train首帧生成类别/属性，输入仅同帧标框RGB全图与目标crop，无序列名/类别提示或后续帧。首版因Markdown包装JSON退出1、0条有效记录；v2仅补上已观察到的固定包装解析，prompt/生成设置不变，152条退出0、186.32秒。原始文字SHA=`ae92f492bcc1e9ca08bed3a734f934210f0c1a9bdd212f0c701c3686d1e6286e`，生成spec SHA=`50d83fe2811acbec045fd35dee255f634ada284aa39fc43e62142d2b36a2addb`。CLIP冻结CPU编码59.72秒，130fit/22dev银行分开，五槽mask同构。

自动文字并非语义真值。执行者查看随机seed2026的24条fit及全部22条dev首帧crop，发现bag05→hand、包装物→phone、花簇→insect等明显误认以及背景/瞬时属性，未改写任何文字，也未读取后续图像来纠正。本轮固定自动协议及噪声，对其现实输入做文本/空文本配对；不能宣称152条描述准确，亦不能把后续失败仅归于网络。公开记录范围与示例，不分发原图/GT。

真实训练推进器在chair01 fit上通过102帧生产路径对齐（含模板写入后读取）；96帧/3步工程优化后RGB/Depth/text投影有有限非零梯度，冻结底座全部参数和缓冲区SHA不变，工程权重未保存。正式两组仍从同一个零残差初始化开始。这补上了训练用局部反向和部署用完整前向的接线核对，不是效果证明。

2026-09-06 17:20:56起，GPU0 text、GPU1 visual各完整遍历130条/186824图像帧、186694次track，参数289154；STTrack底座及BN保持冻结eval。仅首帧GT初始化，其余帧先按预测提交bbox/query/默认模板，再用GT算损失；不GT重置、不加宽crop。全负热图用于中心出搜索域，GT无效则无损失但继续递归。每32已观察帧按有效监督均值更新，AdamW lr1e-4/wd1e-4/clip1，固定一遍，最多5896步。focal中心+2GIoU+5XYXY L1监督GT中心单元，但GT单元不决定公开预测。状态detach，无跨时间/离散裁剪反向；训练参数在线变化而部署权重固定的区别保留。

正式训练spec SHA=`42f618b8d3af47b9e5d5cff83e19b113a0bae74417a7046d95ea96626e7dbb53`；按试运行估计4.30小时/组，初步预计21:40附近完成。首个封存状态两组各已3条/6218次track/196步且继续运行，未据训练损失声称收益。每组仅保留latest和固定final适配器/优化器，不复制底座；两个Qwen均继续保留。

递归spec SHA=`edfd55664e6d584b6dcc5e9cc6572208164eaf1a4bb8b2cbe3bd3ce7b1ee08eb`在训练完成前冻结。独立队列每240秒检查两组训练终态，固定final及预算核验后各运行同一开发22/33130帧，两组预测全部封存后读取后续GT并分析。主实验需同时比原生mean+0.002、比空文本对照+0.001，低重叠/H10不增且保护原生零H10；通过后才做同权重词义干预、低22及最终三个数据集。任何非零退出保留证据停止后续，不自动进入正式公开数据集评测。

源码、冻结参数/顺序摘要、原始自动文字、工程验收及启动记录已整理至projects/sttrack_lachtt_v1/diagnostics/m58/training_preparation。私有下载包87,402字节、SHA=`2bf41fdc5b1dafcc03ee2c3d47da67a2ac116ff5f142fb9ad800fff3fe3b409c`。无新的独立Astra/max审阅PASS，代理额度限制仍在，执行者检查不冒充独立意见。当前无M58开发结果、VOT低22或新全量分数，最终目标尚未完成。


### 5.88 M58监督对齐问题确认：v1停止，v2保持同一初始化/预算重启（2026-09-06）

M58新训练器把中心标签floor到单元，而预训练STTrack原生generate_heatmap使用round，坐标也未直接复用原生transform_image_to_crop。冻结底座时，该差异会惩罚已有正确峰。这是本次M58训练器问题，不能用来解释M55/M57或所有历史负结果。

对cube04、chair01、bag04三个fit序列各102次track的同一零残差输出，仅改变训练标签，未优化参数、未选择开发结果。共306预测帧、298有效GT；170帧IoU≥0.5且两种标签均在搜索内。188个可表示中心单元与原生heatmap峰位置核验一致，20.82秒。首个cube04预测IoU=0.898150、Hann响应0.928095，旧标签(7,7)、原生单元(8,8)原始响应0.944102。

| 同状态诊断，不是新跟踪指标 | v1旧坐标/floor | 原生坐标/round |
| --- | ---: | ---: |
| cube04首样本中心focal | 8.024560928 | 0.000299875 |
| 同样本框回归损失（2GIoU+5L1） | 0.163962871 | 0.292907059 |
| 170个正确可比帧平均focal | 3.443467691 | 0.094345461 |

因此停止错误监督下的两组训练和等待队列：17:40:32发送TERM，三者退出143且进程消失。两组各完整封存9条/16157次track/507步，未完成的第10条不计为完整完成。没有运行开发22/低22/全量，不以部分训练损失作为语言结论。旧spec及源码不回写；两份latest核对SHA和内部元数据后删除，共6,984,934字节，日志/配置/终止证据保留。两个Qwen及原生STTrack不变。

v2复用160个完全相同的推理文件与289154参数结构，只修正监督坐标及中心取整。采用原生transform_image_to_crop与round，并把单元约束到有限0..15网格；本次旧训练采样未观察到round到16的边界，不能声称已测得边界收益。重新通过chair01的102帧生产路径对齐（含模板更新后读取）和96帧/3步梯度检查，底座参数/缓冲区SHA不变；工程权重未保存。原始自动文字、同槽控制、顺序、预算、递归状态和晋升条件全部不变，文字噪声限制保留。
...
### 5.90 M58固定同权重文字对照已冻结，条件队列等待主开发门（2026-09-06）

为区分新增参数/训练与文字内容本身的作用，在原text/visual两组完整开发比较之后，预设同一个text final的输入干预。初始化RGB-D实例参照、槽/mask、配置、默认模板更新及运动规则不变；由文字造成的递归框/query/模板分叉保留。没有新增训练、caption或正式测试集输入。

| 计划中的同权重内容对照，尚未实跑 | 序列 | 图像帧数 | 词向量干预 |
| --- | ---: | ---: | --- |
| empty | 22 | 33130 | 全部有效槽换为空字符串向量 |
| category | 22 | 33130 | 保留类别，属性换为空字符串向量 |
| mismatch | 22 | 33130 | 同槽数、自动类别不同的另一序列描述 |
| attributes | 14 | 20392 | 类别保留，换为同自动类别同槽数的其他属性 |

四组119782图像帧；原文字全22预测复用父阶段封存结果，属性组只与同14条原文字比较。供体由全部152个既有Train初始化描述按固定SHA规则选择，允许重复；不是一个排列置换。其余8条无合格属性供体，名单全部保留，不用不同类别补位。自动类别未人工确认真实物理类别，供体属性也未证明对目标为假；跨类别错配、空文本的分布变化不能独自证明属性消歧。原文字噪声限制保持，次级属性/类别对照不预先宣称独立贡献。

原M58开发主门全部通过后，内容主门再要求原文字pooled mean IoU相对同权重empty、mismatch分别至少+0.001。所有组统一报告低重叠/H10、逐序列和首个框分叉；不把固定权重敏感性替代同预算视觉训练对照。22条仍为复用的Train开发集。102次初始化读取、mask/框保持和预设向量干预CPU检查通过，160个父推理文件不变；track/优化/GPU初始化均0。五份文字银行共6376643字节，不复制底座/训练权重。

首次准备后发现父调度脚本还写post_training_queue.exit，在任何内容运行前将这一真实终态加入等待检查。仅队列一行变化；初版spec/source保留，科学方案、银行、映射与门槛逐字段一致。初版spec=60deb0882064ff9fc54c549ba89a261b1ec4303c7d7ee535afafdfe51c63f310，现行spec=be4a25212f9231963dcdb2acfcf446715910074eefa1265848850c1059b93096。

19:07:29启动条件screen，每240秒检查；19:09:54确认PID344799等待，两个GPU仍仅运行原训练，两组各48/130条、69715次track、2174步。父阶段异常或主门未过即记录不执行；通过后才绑定实际final SHA，先用bag05/cup08/mobilephone02各102帧核对共享运行类与原文字封存预测，再分两GPU完成四组。所有控制与原文字完整预测核验后才进行本阶段GT分析。前缀验收不冒充全轨迹复现/真实TraX，内容门通过后才准备低22技术验收及当次anchor文字，本队列不自动启动公开全量。

公开位置projects/sttrack_lachtt_v1/diagnostics/m58/content_controls。私有下载包18088字节，SHA=4869fa8b1c839b94cdd9fc8d143ef1486d01af89702ec98b76b5b218c8e8422d；当前没有内容对照结果或学习后提升结论。两个Qwen完整保留，磁盘可用2941288448字节。无新独立Astra/max审阅PASS，项目目标仍未完成。


### 5.91 M58真实TraX通信验收与初始化文字索引精度修正（2026-09-06）

本轮用真实VOT toolkit 0.7.1 TrackerProcess与TraX 4.0.2服务端完成CPU合成通信验收。发现的是新文字入口离线准备中的精度假设，不是模型训练或历史VOT分数下降的因果结论；尚无新的学习后指标或正式测试集运行。

§5.89的153次检查仅覆盖TraX Rectangle构造器的float32存储，没有覆盖协议收发。实际坐标链为toolkit xywh→float32→四位小数协议文本→接收端float32。例如x=1.123456789，构造器值1.1234568357467651，实际文本1.1235，接收值1.1234999895095825。旧辅助函数仅做第一步，按它准备的精确文字键可能与运行时实际框不同。原先一位小数例子没有暴露此差异，本节明确修正此前对完整协议转换的解释。

原initialization_text.py等六个接口被条件内容队列绑定，保持逐字不变。另加vot_rectangle_transport.py供未来VOT输入准备使用，包含实测完整转换；输入仍需先按toolkit的实际region→xywh处理。运行时严格按收到的框取文字，没有改变框或加入模糊回退，OPE坐标/六位小数导出不改。M58尚未生成正式VOT文字银行，不能把该问题用于解释旧SUTrack或原生STTrack的指标差异。

初次CPU测试在第一个报告框比较处因只预期float32而失败，原source/fixture/log/exit=1保留。修正后的四个真实通信会话：同一图像两个不同高精度初始化框各取得对应文字并完成两次报告；未登记框在模型初始化前KeyError；同一个高精度框配旧构造器精度索引也在初始化前KeyError。两次正确会话、两次预期拒绝，整个测试exit=0，完成4次预测报告，耗时8.36秒。RGB-D通道/帧顺序、真实wire框及原始置信度均核对。

生产VOT循环、M39 bridge、RGB-D解码和文字银行查找实际执行；模型工厂、bundle验证及银行工厂边界使用确定性CPU替代。图片为3张程序生成RGB与3张uint16 Depth，向量为合成值。GPU初始化/权重加载/数据集图片读取/真实模型track/优化均0，不冒充真实权重GPU入口验收、语义正确率或模型收益，也未声称覆盖其他TraX版本和所有region边界。
...
### 5.94 M58 v2配对训练与开发22完整递归完成：均值提高，H10门未过（2026-09-06）

§5.88的v2两组已于21:22:39 CST完成全部130条fit序列，每组186824图像/186694 track调用、5798步、289154新参数，约12756.3秒。文本与纯视觉同初始化/同mask/同种子2026/同顺序/同预算，原生底座参数及buffer保持不变。v2使用原生crop坐标与round中心监督；保持真实预测裁剪、默认模板和query递归，不做首帧以外GT重置，也不通过时间或离散裁剪反传。架构延续已记录的后融合密集语义残差，未添加候选头、在线文字、Clone或新模板规则。

21:26队列发起完整开发22两组验证，各33130帧、约1767.5秒；两组先封存后于21:58:01进行GT分析。当前原生及候选三组全部轨迹已重读并按冻结指标重算一致，有效帧均28897。
...
文本相对原生按帧IoU+6.0416个百分点，相对纯视觉+0.4510个百分点；低帧分别少1968、319；H10分别多3、11。按序列等权文本比纯视觉低1.4912个百分点。原生零H10实际为container01、flower03、mobilephone02三条，文本均保住；此门不代表其他轨迹没有损害。五个冻结门四个通过，仅H10≤75未过，primary_pass=false；不放宽门、不晋升M58 v2。

原条件内容队列已退出0并记录not_run_parent_gate_failed，实际control track调用0、candidate bundle未创建。没有新的VOT低22/全量、DepthTrack Test或CDTB指标。上述是重复使用的DepthTrack Train开发22、一次种子下的连续IoU/H10代理，不能冒充正式P/R/F或VOT ROB，也不能将6.04pp全归给文字。

相对原生文本IoU提高10条、下降12条，但删除任一单条后按帧整体增益仍正。相对纯视觉提高12条、下降10条；分别移除beautifullight01、cup13或mobilephone02任一条，剩余集合的文本增量转负，说明额外文字收益不稳。完整22条表和全部220个H10区间在公开报告，无按效果删样本。

H10总区间帧原生7160、文本5173、纯视觉5440。文本存在6段共217帧H10区间，而独立原生在这些对应帧全部IoU≥0.5；相对纯视觉有6段246帧。故不能把所有失败段增加都归为碎片化。相对原生的6段都紧接无效GT窗口；这不等于已测明遮挡/旋转原因。flower02文本0.218057/346低帧/6H10，相对原生0.650547/100/2严重退化；其零基[414,520)106帧文本持续低重叠而原生全正确。cup13、beautifullight等又有收益，必须做同一文本权重下的内容干预来分清文字作用和不同训练状态。

下一项计划另立训练集内内容诊断：固定M58文本final和策略，复用已冻结empty/category/mismatch/attributes观测；前三项全22，属性项预定义14，完整递归后解释文字是否影响正负轨迹。该诊断不改变M58晋升失败结论，不让原条件队列自动跳过门，也不直接导向公开集。得到证据后才决定如何改语言模块，重新训练/递归通过后按既定低22→同bundle三数据集顺序验证。

两个final均3492373字节：text SHA=0f2303ae1f9c12b4a25e6e6531b1fe39fa79578938aa9bc8c517e10bec06b629，visual SHA=b609d4fde11eb58e3ef930d5bb271c1fd1ff5c7768e7754d67482df699228649。完成态日志、全部23张量有限且更新、optimizer有限、源160文件和当前原生文件SHA已复核。CPU审计前两次仅因检查器误读sequence_order记录类型和稀疏计数而失败，修正检查器后v3 exit0；历史失败源/log保留，训练/推理/结果未动。

22:14:51两GPU均空闲1MiB，可用磁盘2897113088字节；两个Qwen按5/2分片、16381516776/7509337976字节确认保留，本轮删除权重0。执行者审计不是独立审阅，无新Astra/max独立PASS。公开projects/sttrack_lachtt_v1/diagnostics/m58/recursive_completed；证据包SHA=5713a83cd46da63b884a7f08746d9284086bf593eeba134f47a050938ce7fb8f，完成态审计SHA=8e6392a1e0e0f11acfebae80faf635ccfcdbafcec85903e5d5d59db66ed6aa05，父递归结果SHA=54fd8445297e1eef761fa36e8a17def01afa027f71371984998f08229f6f5ee9。项目目标未完成。


### 5.95 M59固定同一文本权重的Train内容诊断已启动；M58仍未晋升（2026-09-06）

为区分§5.94中额外文本收益与不同训练权重/状态的作用，另立M59训练集内诊断。固定M58文本final及原生底座、mask、初始化RGB-D、query和默认模板，复用§5.90看结果前冻结的银行/捐赠映射及全22/属性14子集，仅替换词向量内容，不再训练或生成caption。M58 H10门仍失败，原条件队列仍not_run；M59显式绑定父负结果，禁止公开评测，不以新诊断放宽晋升要求。

| 固定文本head条件 | 序列 | 图像帧 | 执行方式 |
| --- | ---: | ---: | --- |
| 原自动文字 | 22 | 33130 | 复用封存原轨迹 |
| Empty | 22 | 33130 | GPU0第一组 |
| Mismatch异自动类别同槽数 | 22 | 33130 | GPU0第二组 |
| Category保留类别/清空属性 | 22 | 33130 | GPU1第一组 |
| Attributes同自动类别属性替换 | 14 | 20392 | GPU1第二组 |

新增80轨迹/119782图像/119702 track调用；没有按正负结果挑序列。捐赠者复用、自动类别不等于物理类别、属性非人工验证错误、空词可能超出训练分布等限制继续保留。旧M58 spec SHA be4a2521…及源/banks未动，新spec SHA=2c190de16b186a3c9c97abcdabdeb76ce75428f4e32546c32d7d5e6226077cd6，父递归结果SHA54fd8445…。运行器仅改目录、负父结果诊断准入和禁止晋升状态；load_frame/prefix_parity/run/aggregate源码相同，父160推理文件及六接口不变。

真实GPU入口前缀检查通过：bag05/cup08/mobilephone02各102帧，共306图像/303 track调用，最大bbox误差0、score误差0，19.06秒，exit0。三条前缀实际模板写入均0，因此只证明这些前缀输出复现，不声明全轨迹/全部内部状态/更新分支或TraX验证。prefix结果SHA=c0ce584c6571ce579d49fa15be294385e258ea8687842b693b23ce046398f74a，诊断bundle SHA=695ac6e416a6d03c4599fcfa1bb5e1005f18c6bf078f952c02bc89de922e9dca。

22:28:47两个专用screen启动，GPU0 Empty→Mismatch，GPU1 Category→Attributes，启动前显存均1MiB。22:31:51现场PID355439/355441及GPU执行确认，两组bag05完整889帧已保存SHA。预计较长一路约3550秒，首条实测49.5秒，估计23:28—23:35结束；接近ETA再以240秒间隔观察。Worker0完成后在同一screen等Worker1，不新建额外监控服务；两边退出0且全部轨迹封存后才打开GT计算完整结果，中途不算指标。

补充对M58全部封存score按继承代码frame_id%50==0且score>0.75重建模板写入：

| M58完成路径，非新增跟踪 | 定时检查数 | 重建写入数 | score中位数 |
| --- | ---: | ---: | ---: |
| 原生 | 654 | 292 | 0.719259 |
| 同预算纯视觉 | 654 | 237 | 0.657896 |
| 文本 | 654 | 178 | 0.620413 |

该计数不读GT、不修改阈值、不插入新hook。它说明保留同一默认模板规则时，学习后的分数变化已改变实际更新频率；不是因果证明，不能据此直接降低阈值或增加模板写入。后续是否研究响应校准/记忆交互，需结合M59内容效应另立Train对照，不能把它们同时混进本次实验。

M59数字margin仅描述内容效应，无论结果如何都不自动晋升M58。全部完成后判断文字是否有独立贡献、具体损害是否受词义影响，再决定新语言模块及重新训练；通过递归后才按低22→同bundle DepthTrack Test/CDTB/VOT全量。当前无新增正式分数。

公开projects/sttrack_lachtt_v1/diagnostics/m59。准备证据包28688字节、SHA=2ed0fdf297b3ba5c2ab92fc993ab515601d66cadab5414880e813947713e0033；模板重建SHA=e4bb63cb51f12b2deebde92fe446a04ce026b8ab58efe67b5a20398f551d2fef。方案公开摘要省略真实初始化框，银行/权重仍留服务器。22:31:51磁盘余2889166848字节；Qwen3_8B与Qwen2.5-VL-3B均保留，M59新权重0、删除0。无新独立Astra/max审阅PASS；目标仍未完成。


### 5.96 M58模板写入质量补查：低质量写入减少，正确框时的更新也减少（2026-09-06）

延伸§5.95中292/237/178次模板写入重建，只对已封存M58三组轨迹配对Train开发22既有GT。M59两GPU仍按原冻结内容方案运行，本轮不读取其部分预测或计算其中途指标，不改模型/文字/默认模板阈值。GT本来属于重复使用的同一开发22，这不是新未见验证。
...
三组各654个定时检查时刻，82个GT无效单列，不能按错误写入处理。文本检查时刻当前框IoU≥0.5有457次，纯视觉453、原生424，故写入少并非因为正确定位的检查时刻更少。写入中0.1<IoU<0.5均0。

逐物理帧对齐，有146次原生写入、文本跳过，其中108次两路当前框都IoU≥0.5，7次文本框正确而原生框低重叠；相对纯视觉，83次写入而文本跳过，其中55次两框都正确、15次文本正确而纯视觉低重叠。各自轨迹已不同，不能将其当作只改变一次写入的配对因果干预。

这说明更少更新同时减少低质量写入和正确框时的外观适应机会。当前框正确仍不保证新视图的辨识性、清晰度或未来效用，287次文本跳过且框正确不能直接变成待补回的更新清单；score为Hann最大响应而非校准的身份概率，不直接据此降低阈值。等待M59固定文字内容干预完成后，再判断语言对响应/选框/模板频率的作用，必要时另做Train配对实验。

分析程序SHA=9aa8b83571dd4715342f34350513d942c6e1d85ead497c8e79785760ea8548a6，实际执行exit0；父递归结果SHA54fd8445…，质量结果SHA=ffad5952466ea14bd62b6132cae975a12b1bbe13928020918efaf694e9035f75。公开projects/sttrack_lachtt_v1/diagnostics/m58/template_quality，证据包8355字节、SHA=7b8e085812a7010116677fe1c726e3d59d53de73497a1541573add36bcbb3b4a。没有新增模型/优化调用、阈值拟合或正式三数据集分数，无新独立审阅PASS。


### 5.97 M59四组同权重内容对照完成：类别保留最好，完整文字未优于异类替换（2026-09-06）

23:27:51 CST四组运行和统一分析全部结束，worker0/worker1/analysis/finalization均exit0。新增80完整轨迹/119782图像/119702 track调用，加上封存原文字22条，均使用同一个M58文本final、原生底座、mask及默认模板/query。新网络参数/训练步/caption生成均0。完整轨迹hash、帧号/初始框/有限框及聚合算术核对通过；GT只由所有条件封存后的冻结分析器使用。
...
四行均22条/28897有效帧，指标仍是Train开发连续IoU/H10，不是正式P/R/F或VOT指标。原文字比同权重empty高0.7466pp，但比mismatch低0.6857pp；描述性内容margin empty通过、mismatch不通过，不能宣称完整语义输入稳定有效。类别保留比完整文字高4.0570pp，少1366低帧/14H10；比同权重empty高4.8036pp。对旧独立原生高10.0986pp，低帧7397→4063、H10 75→64；比同预算纯视觉头高4.5081pp。类别保留保住原生零H10三条，empty新增mobilephone02失败，异类替换也保住三条。

类别保留是在看M59结果后发现的候选协议，不追认成M58预注册主方案，也不把相对原生的全部增益归给文字。属性仍有部分可用信息，预定义14条必须同子集对照：
...
两行均17949有效帧；原属性高2.7907pp、少542低帧/1H10，9条优于替换、5条更差。不能拿14条0.788145与全22的0.712642相减。自动类别和属性不保证语义真值；全部逐序列结果在公开报告。

flower02从原文字0.218057/346低帧/6H10变为类别保留0.792491/23/0，但原自动类别为insect，不能由此认定它是正确物理类别。glass03类别保留0.854841、异类完整替换0.168263；cup10/cup13/colacan04类别保留又增加H10。所有80条干预首次框差异均在零基第1帧，证明输入影响输出，不证明正确语义理解或第一次差异独自造成后续失败。四个全22条件重建写入178/273/197/207，属性同14原132/替换133，次数与轨迹收益不单调。

M58完整属性版本仍未晋升，无公开集评测。下一项仅补类别单独置换M60，固定属性全空、mask和同一个训练权重，隔离当前mismatch同时改类别与属性的混杂。源码修改/学习参数若继续改变，仍必须用DepthTrack Train训练；本次只是既有训练权重的输入协议诊断。

M59结果SHA=6e9373d1d9252eae104421d664703490a13541353988ac8851fe2bc5c4d0756e，完成核对SHA=285c5f6c5f279900812608385aa18a538c45779dfa4f2791225c10364540a9af，证据包26573字节/SHA=7c6f692cf5e1484db54f787dad5841d4c446f6ef98cb24630067defc4d765468；公开projects/sttrack_lachtt_v1/diagnostics/m59/completed。单种子、重复开发集、无新独立审阅PASS。

### 5.98 M60单独置换类别词：准备与前缀通过，完整22已启动（2026-09-06）

M60从M59类别保留银行出发，只将第0类别槽替换为先前冻结异类捐赠者的类别向量，其余属性token、padding、mask均逐元素相同。22个类别槽均变化，22个初始化RGB/框索引准确返回新token。原mask保留自动caption槽数，不能称为一个词槽的新网络；原自动类别并非人工真值。

新增一个22条/33130图像/33108 track完整对照，原类别和empty复用M59封存结果。底座和同一M58文本final不变，新训练步/网络参数/权重/caption均0。primary描述性margin为原类别平均IoU减去置换类别至少0.001，不自动推进公开集。类别保留为M59后选择，继续标记开发用途；结果决定是否继续训练文本交互或另行冻结候选协议。

CPU准备初次因共用RGB-D读取器未加载父源码目录退出；只补与原运行器一致的代码路径后成功，错误源/log保留，未有GPU训练或推理失败。真实GPU重放原类别bank的bag05/cup08/mobilephone02各102帧，共306图像/303 track，bbox/score最大误差均0，19.53秒，exit0。三前缀均0模板写入，不声称更新分支、完整状态、整轨迹或TraX验收。

23:45:41在启动前空闲GPU0的专用screen m60_category_isolation_20260906运行；23:47:40实际PID358782存活，bag05完整889帧已保存SHA。预计次日00:16左右结束，接近ETA再检查；全22封存后才打开GT分析。GPU1空闲，两个Qwen仍保留，没有新增/删除模型权重。

程序SHA=f8c1f789a7f927121e9f50cefc1d2995774d6bd9bf73d047ccb11e7ea920303b，spec SHA=567944c9dbe2e79340bfabc737a0975563fa7e5be17d16a8b2fc3d75fd1990c5，bank SHA=90ea3f11ba34470a15ed5078177dcd4cf41d47a564c22a4f5f3d94815ecc7986。前缀结果SHA=29e792411fa469a1b3536550c4869f3d563446050db87a63a6e85b4ba1cf1b80，证据包8734字节/SHA=863ed13e956f98415f885f7678ba358bef3abb8fd3e52d813c796929dfc475dc。公开projects/sttrack_lachtt_v1/diagnostics/m60，真实初始化框及bank留服务器。无M60完整指标或独立审阅PASS，整体目标尚未完成。


### 5.99 M60完整类别词隔离完成：原自动类别有增量，仍不能宣称实例语义全面成立（2026-09-07）

00:15:40 CST完成22条/33130图像/33108次track，tracking/analysis/job均exit0。只替换第0类别词，其余空属性token、padding、mask和同一个M58文本final均不变。完成态核对验证所有22新预测和22类别参考指纹，使用已审计的独立连续IoU/H10实现重新计算，全部一致；分析在新对照完整封存后进行。
...
均是重复开发22/28897有效帧。原类别减置换类别按帧IoU为+1.4571pp，按序列等权+4.5478pp，少524低帧/12H10；12条更好、10条更差，预定0.001描述性内容margin通过。原类别比同权重empty高4.8036pp，但置换类别也比empty高3.3465pp，不能把全部非空文本收益算作正确词义。原类别保住原生零H10三条；置换类别、empty均新增mobilephone02失败。

类别保留是在M59后选择的协议，自动类别含已披露错误且mask仍保留caption槽数。它不是一个词槽的新模型，不证明同类实例消歧，不追认M58完整属性方案晋升。M60 public_evaluation_allowed=false，没有自动公开评测或新正式三数据集分数，没有新独立Astra/max审阅PASS。后续固定同一状态分析直接定位与更新资格，避免把完整递归差异全归给单一词义机制。

结果SHA=febddd93eda9f25f3530377948751c1999030db9a08656f9bb24ae151ad64287；核对SHA=2997a74d7effe050bf650147eb89c1482ed4b28084f3d7dbef82ab88ceb7b03c；证据包11919字节，SHA=e9d44a2535463f3e0b10e47ab7e6592fe4a19f276159ff7fcc0c4bd3a406383b。全部22条见projects/sttrack_lachtt_v1/diagnostics/m60/completed。

### 5.100 M61同状态文字定位与更新资格诊断：覆盖模板写入的前缀通过，完整双参考运行（2026-09-07）

按最新用户方向，将语言内容的直接作用和响应变化造成的运行副作用分开。参考为M58原完整文字与M59类别保留两条封存路径；每条固定全部654个frame_id可被50整除的更新检查时刻，共1308事件，不用GT选择失败点。

原运行器先正常推进参考路径，同时在适配器输入处克隆RGB/Depth/未加残差融合特征、初始化RoI与mask。随后在相同克隆输入上比较同状态未加适配残差、原完整文字、empty、类别保留、类别置换五种头部输出。诊断不会接管bbox、query或模板；输入克隆逐元素检查不变。记录原score/size/offset图、Hann峰、各自峰和固定参考峰的解码框、峰分数与0.75写入资格。未加残差的同状态输出不是独立原生STTrack轨迹。

所有参考帧与封存框/分数逐帧核对，实际模板写入也核对。双参考完成后才读取GT，计算当前事件IoU和正确/错误转换、换峰、写入资格翻转、同峰翻转。固定平方根/平方变换只检验保持框及RoI不变时的分数尺度耦合，不实际改变写入，不证明未来模板效用。既有M58完成态质量审计记录每序列有效检查至少9个、合计572个；本次事件集合仍完整包含654个时刻，不据此筛选。

两条参考各以bag05/container01/mobilephone02前202帧验证，共1212图像/1206 track、24事件；框/分数最大误差均0，每条实际7次模板写入，35.86/36.02秒，exit0。覆盖真实更新分支，尚不声称完整轨迹或全部内部状态已通过。

00:28:41在空闲GPU0/1专用screen m61_fixed_state_20260907启动完整66260图像/66216 track预算。00:30:21实际PID360449/360450存活，两条首序列bag05完整889帧/17事件完成，误差仍0。预计01:02左右结束，接近ETA检查。响应图留服务器，新记录预计几十MB；未新增/删除或复制模型权重，两个Qwen保留，启动前剩余约2.84GB。

采集source SHA=06e9892e24d85073f9322114401642c0dca492e127caaf07458ca1f9eb8eed84，spec SHA=2d6f2e89cc5aef32d7f3b12931ce7f699aaf35fae6980fad74dee59411249399；分析source SHA=5c856a4ffb84333706417d3efd6b3578d429bdfa5677fbb76df8937b95310e7a，分析plan SHA=c2fd97c308f03fa05a545cf7fb4433d0217ce93579b2886b9ba30cc5988d1eff。准备证据包11867字节/SHA=6fe6cc3bacdfec1ff1e0cfaba850f891ec041f770eb39f8a5ae82f2bdb3f344d，公开projects/sttrack_lachtt_v1/diagnostics/m61。

当前没有M61完成态结果、新参数训练、caption生成、独立审阅PASS或正式公开指标。结果决定下一个最小语义拒绝或独立记忆质量对照；新增可学习结构必须在DepthTrack Train训练，最终仍需同一底座/语义模块/质量模块/文字协议/运行策略验证完整三数据集。整体目标尚未完成。


### 5.101 M61完整同状态诊断与完成态复核（2026-09-07）
...
下一项[M63](../projects/sttrack_lachtt_v1/diagnostics/m63/README.md)用四组完整递归隔离定位与更新打分的类别来源。[M64](../projects/sttrack_lachtt_v1/diagnostics/m64/README.md)是单独冻结的类别保留候选，未从交叉组中挑选方案。


### 5.102 M62真实训练权重的OPE/TraX入口完成验证（2026-09-07）
...
### 5.103 M63定位与更新类别输入2×2递归消融启动（2026-09-07）
...
### 5.104 M64类别保留候选冻结、真实初始化生成及低22启动（2026-09-07）
...
### 5.105 M64全量准备程序完成源码与元数据检查，准入及实际全量运行仍待低22完成（2026-09-07）
...
### 5.106 M64全量VOT输入保真与三数据集终验代码补齐，尚未执行条件入口（2026-09-07）
...
### 5.107 M63纯对照与交叉前缀输出审计完成，完整交叉结果仍待封存（2026-09-07）
...
### 5.108 M63四组递归与结果复算完成：原类别联合路径最好，定位与写入存在状态依赖（2026-09-07）
...
### 5.109 M64低22完成负结果：129个失败，三指标下降，不进入全量（2026-09-07）
...
### 5.110 M65类别协议一致的无支持项配对训练：真实Train烟测通过，双卡正式训练启动（2026-09-07）
...
### 5.111 M65完成态复核工具就绪：历史66条封存轨迹复算一致，新训练结果仍待完成（2026-09-07）
...
### 5.112 M65同权重内容对照已在开发结果前冻结：原类别、空文本、既有替换类别（2026-09-07）
...
### 5.113 M65配对训练与递归完成：整体增量成立，glass03保护门未通过（2026-09-07）
...
### 5.114 M66同状态诊断完成：出画恢复与框内选峰分开，无支持项尚未形成拒绝能力（2026-09-07）
...
### 5.115 M67配对训练启动：固定Null架构，仅增加框级支持监督（2026-09-07）
...
### 5.116 M67后续验证已冻结：完成态审计与同权重文字内容对照（2026-09-07）
...
### 5.117 M68诊断准备：语义支持仅改变上报置信度，定位与模板仍走原路径（2026-09-07）
...
### 5.118 M67独立评测接口修正：support架构契约与152条Train初始化路由（2026-09-07）
...
### 5.119 M68历史置信度容量诊断完成：固定框下的Train长期PR差异（2026-09-07）
...
### 5.120 M67训练约四分之一的只读机制审计：目标中心与框外支持开始分离（2026-09-07）
...
### 5.121 M69独立纯诊断准备：固定M65 Null做文字内容归因，保留原失败门（2026-09-07）
...
### 5.122 M70跨区域恢复准备完成：全22窗口、crop可达性与观察成本（2026-09-07）
...
### 5.123 M70跨区域候选诊断准备：同状态观察、文本区域选择与精细定位分离（2026-09-07）
...
### 5.124 M67后续持久队列已启动：完成态审计、内容归因、置信度与跨区域诊断（2026-09-07）
